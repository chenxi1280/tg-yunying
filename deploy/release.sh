...
append_image_env_if_set() {
  local name="$1"
  local value="${!name:-}"
  if [[ -n "$value" ]]; then
    printf '%s=%s\n' "$name" "$value" >>"$image_env_path"
  fi
}

verification_runtime_vars=(
  TELEGRAM_DIRECT_EGRESS_REGION
  TELEGRAM_DIRECT_EGRESS_IP
...
for runtime_var in "${verification_runtime_vars[@]}"; do
  append_image_env_if_set "$runtime_var"
done
