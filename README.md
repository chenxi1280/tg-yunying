...
- `TEST_DATABASE_URL`：可选测试数据库连接；测试默认使用临时 SQLite fallback
...
开发环境只初始化最小本地工作区和两个后台用户，不再默认写入演示 TG 账号、群聊和素材：

- 平台管理员：`admin@demo.local` / `admin123`
- 客户操作员：`ops@demo.local` / `ops123`
