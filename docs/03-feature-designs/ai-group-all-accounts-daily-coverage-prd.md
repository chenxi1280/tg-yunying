...
Dispatcher 取件时，显式 `target_admission_retry` 仍优先处理；其余常规 Action 中，`hard_hourly_target=true` 且可直接执行的 `send_message` 必须先于 `ensure_target_membership` 和 `ensure_channel_membership`。已记录 `required_channel_admission_pending` 的发送必须排在准入 Action 之后，让前置入群先解除该账号的实际阻塞，不能反复占用取件名额；这不影响已确认 `can_send=true` 账号的硬小时发言优先级。任务列表的硬小时状态必须由当前 bucket 的 Action 成功事实定期刷新，不能沿用历史 bucket 的 `met` 快照。
