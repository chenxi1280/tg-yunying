...
同一份当前上下文可能同时形成多个彼此不冲突的成人证据，例如既讨论外观又询问服务。此时只能在“当前证据已命中且任务已授权”的路由集合内，使用稳定 Slot 序号确定该 Slot 的唯一模式；不同 Slot 可在该集合内轮换，避免所有账号重复同一种话术。多命中本身不等于 `unsafe_or_unknown`。没有任何已授权路由被证据命中，或证据互相冲突时，仍必须显式拒绝，不能静默回落到 `general_chat`。
...
- `group_ai_chat` 暂停必须先锁定 Task 并检查 `unknown_after_send`、已开始 Gateway Attempt 和 V2 `gateway_bound` slot。无远端不确定性时复用任务重规划清理，释放旧 Action 的 coverage/content-mix 占用，并把开放 GenerationJob 与 pre-Gateway window slot 显式置为 cancelled/invalidated；同时只对当前时间覆盖的 open ledger 内、`open + frozen + task_lifecycle_epoch 非空` 且未绑定 active/success/unknown Action、任一 Attempt 均未开始 Gateway 且没有 remote ID 的数量 slot 释放旧 `task_lifecycle_epoch` claim 和已过期 `release_not_before_at`，使其可由 resume 后的新 epoch 接管；同一 paused epoch 重复清理必须返回 0。已关闭或历史 ledger 不参与接管。存在任一不确定性时必须保留原事实、在 AuditLog 标记 `blocked_remote_ambiguity`，且 bootstrap 继续以 `task_open_work_present` 阻断。
...
- 已释放 owner 即使残留非空但早于恢复游标的 `release_not_before_at`，也必须以当前恢复游标后的正常 gap 重建 release；只有仍晚于恢复游标的未来 release 可以保持。写回新 epoch 前必须替换旧 release，不能因 immutable owner 的保护逻辑把过去时间重新绑定到新 Action。
- 暂停清理还必须将仅绑定 `failed/skipped` 且没有 Gateway-started、remote ID 或 typed remote fact 的 `AccountPacingReservation` 解除旧 Action 绑定并保持为可重用 `reserved`；新 epoch 复用同一 slot key 时必须以重建后的 owner release 重新计算 reservation/effective claim。任何远端边界、open Action 或 success/unknown 预约都不得重置。
- `GenerationJob` 进入 `failed/cancelled` 时必须同步把其 `claimed/candidate_ready` window slot 置为 `invalidated` 并解除 job 绑定；暂停清理还必须扫描该任务所有历史 epoch 的 pre-Gateway window slot，回收终态 Job 遗留的 obligation 唯一键。`gateway_bound` 永不由该流程清理；不得让旧 epoch 的 `claimed` slot 阻塞新 epoch 对同一数量债的 JIT 生成。
- `group_context_route` 与各 `group_realize_*` 的生产 route-set 至少配置两个按 priority 排序、模型家族可不同的健康候选。429、quota、timeout、connect error 只允许沿该冻结顺序做 transport failover；当前生产首选 MiniMax 不可用时可进入已评测的 MiMo 候选，malformed、质量门禁和 Reviewer 拒绝仍不得切 Provider 洗稿。
