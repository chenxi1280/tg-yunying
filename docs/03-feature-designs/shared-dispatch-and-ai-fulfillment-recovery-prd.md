...
| membership单一核验协议 | unknown membership权威reprobe通过同一RemoteReconcileCase expected hash/evidence CAS确认并写joined；存量无Attempt先持久建立read-only recovery Attempt/Case；超时后释放精确claim并可由新claim重入；不得遗留pending case，也不得Action先成功后case conflict |
