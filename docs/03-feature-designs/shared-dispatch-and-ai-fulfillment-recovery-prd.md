...
- 评论重规划每轮最多领取本轮 `message_comment_quantities` 已分配的义务数，不能把该消息全部未绑定义务一次性跨小时建完。含未绑定 `open|replan_required` ordinal 的源消息必须优先并入 Planner scope，即使它已滚出当前 `latest_n|dynamic_new|specific` 消息集合；reply ordinal 仍从该同租户、同频道目标、同源消息的当前评论池重新冻结 `reply_to_message_id`。池不足时保持未绑定等待，不得转到新源消息创建新 ordinal、复用已证伪快照或降级 direct。
...
| 评论近端物化与即时重建 | 首次 reply 排期跨出 context window 时只物化窗口内 Action 并记录下一 checkpoint；源消息滚出当前消息 scope 后让 reply Action 以 remote missing 释放，Planner 仍优先载入原源消息，Task 立即唤醒、同一 ordinal replacement 按当前时间创建；不得在新消息上另建 attempt 1 义务冒充补位 |
