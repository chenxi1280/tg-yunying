...
- `own_history` 分页必须先在数据库查询中按同租户、同目标群跨任务排除已被 `pending|claiming|executing|unknown_after_send|success` Action 占用的 `reply_to_message_id`，再按成功时间取本轮上限；禁止“先截取最新 N 条、再在内存排除已用目标”，否则最新窗口耗尽时会漏掉更早但仍合法未用的成功消息并制造 `reply_target_missing`。查询必须保持有界，最终候选仍在 Action 创建前复核一次占用状态以覆盖并发。
