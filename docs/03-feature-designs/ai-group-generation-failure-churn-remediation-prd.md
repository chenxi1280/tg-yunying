...
4. Provider 尚未开始且 Action/Job 仍属于被 reconcile 的旧 owner 时，才允许原义务回到 pending；历史半收口若仍是当前 Job 的精确 Action、`claim_owner/lease_owner` 均为空且 payload owner 为空或仍等于该 Job 的 expired owner，也属于无人持有的同一旧 owner 残留，不得误报为其他 worker 抢占。此时必须按 Action 当前版本和精确 Job binding 收口：Provider 未开始则恢复原 Action/Job 为 pending；已开始 Provider request 或结果持久化不确定时，即使 Action 被旧逻辑写成 pending/skipped，也必须恢复同一 Action 并保留同一 Job/request 的 `unknown` open owner，禁止新建 replacement、取消 Job 或重调 Provider。只有存在非空且不同的 live Action owner/token 时才报 `generation_reconcile_action_claim_changed` 并零写；只有已持久化的同 request 结果缓存才能把原 Action/Job 恢复到 `persist_retry`。恢复后的 Action 再被上下文新鲜度、入群或 Provider 容量延迟时，所有延迟出口必须同时清空 live/payload claim，并把 Action 状态与 `payload.ai_generation_status` 都恢复为 `pending`，Job 同步回到可重领的 pending/wait stage；禁止形成 `Action.status=pending + payload=generating` 的永久不可领取混合状态。
...
**2026-08-14 legacy route hardening：** 兼容归一只能补齐 account coverage 等非路由字段；Planner、stats、open-action gate 或任何运行时读取不得把解析到的 `OperationTarget` 写回 running Task 的 `target_group_id` / `target_operation_target_id`，更不得绕过 config revision 和 AuditLog。账号 dialog 同步收到 numeric peer ID 时，只有公开 username/link 与既有 legacy target 唯一匹配才可复用其 group/target；无匹配可新建，多个匹配必须显式失败，不能静默选一个重复目标。若历史运行时写已使当前 Task 路由偏离 open ledger，恢复只能在同一事务 preview 中证明：单一 ledger target 与 slot route、同一公开身份、无 Task-scoped admission、以及精确未挂 ledger 的当日残留。错误 route 的 Action 仅有两种可收口例外：未创建 Attempt 的 `skipped`，或 `closed_unknown` 但 Action/每个 Attempt 均明确为 `PEER_INVALID`、已进入 Gateway 却无 remote message；二者只保留审计。尚未 claim/lease 且零 Attempt 的 `pending` 错误 route Action 必须连同 id 与 action version 写入 manifest，apply 只将其 cancel 后再还原 Task 路由；任何已 claim、执行、成功、其他 unknown、Attempt 漂移或 admission 均阻断。apply 必须比较 manifest hash、写 AuditLog，绝不重发或改写终态历史。不得把残留 target 重新挂到 ledger、删除历史事实或把 current route 当作下一日变更。
