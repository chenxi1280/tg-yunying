...
`POST /api/tasks/search_rank_deboost` 的新建请求只包含 `target_operation_target_id`、`keywords` 和 `target_count`。它们分别表示一个我方目标群、一个或多个搜索关键词和任务生命周期目标次数。服务端将单一 `OperationTarget` 解析为内部 `target_group_ids`，生成任务名称，并固定以全部启用且用途一致的 `rank_deboost` 账号组作为候选范围。
...
- 编辑页只允许修改目标群、关键词和目标次数；账号范围、代理、机器人、点击节奏、停留、并发和重试均为系统托管字段。
...
创建向导固定为“任务类型 → 目标群 → 关键词与目标次数 → 确认”。运营人员只选择一个我方目标群、输入关键词和目标次数；系统自动从全部启用且用途一致的专用账号组中选择符合配额与风险闸门的账号。
...
- 新建降权任务只显示目标群、关键词和目标次数；提交体不含 `account_config`、代理、机器人、节奏、停留或重试字段，额外字段被服务端明确拒绝。
...
- 原始需求：多个黑账号组、普通任务全选排除、降权任务全选只取黑账号，以及新建只填目标群、关键词和目标次数，已覆盖。
