...
> **2026-08-25 max-30 chunk 实施合同：** `MAX_CHUNK_ACCOUNTS`、runner CLI 默认值和 SSH wrapper 用法统一为 30。每次普通生产调用必须显式 `--max-accounts 30`；非尾批成功时审计 `processed_count=30`，尾部不足 30 才允许 1–29。循环内部保持逐账号串行，首错立即返回并停批，不能为了凑足 30 跳过失败账号；A/current/Session/generation、runtime/unknown/sensitive/MY client 与 B/C/E4 业务验收门禁不变。参数 31 及以上必须在领取 item 前显式拒绝。
...
> **2026-08-24 rebind 后 rebase 与 chunk 静默边界合同：** 当生产 release 漂移与 pending plan 漂移同时存在时，正式顺序固定为 `release-rebind preview/fingerprint/apply -> pending-plan rebase preview/fingerprint/apply -> runner`。pending rebase 仅可额外接受刚由正式 release-rebind 恢复的 `running` batch：最新 batch 审计必须精确为同审批的 release rebind，execution release 必须匹配该审计，items 仍仅 `succeeded|pending` 且 running/stopped=0；SSH apply wrapper 继续拒绝 runner 进程，DB 内继续强制 runtime off/scope 空、global sensitive/unknown=0、MY client=0 和全部 A/C/operation/slot CAS。普通 running、已有新 batch 审计或任一 active item 仍拒绝。runner 每次达到 `--max-accounts` 且 frozen N 尚未完成时，必须在当前账号 B/C/E4 与 A send/readback 全部终结后，锁 batch 重验 items 守恒、runtime/sensitive/unknown/MY 静止，再写 chunk-boundary pause 审计并把 batch 置 `stopped`；下一次同 execution release、同异人审批的 `run` 只可从该最新审计原子恢复，其他 stopped 原因不得自动恢复。pause/resume 只改 batch status/version 与审计，不改 A/B/C、Session/current/App/generation、slot 或 operation。
...
- 全量 rollout 使用一个 frozen `N`，连续调用 SSH runner，每次 `--max-accounts 30`；验证非尾 chunk 精确 30、尾批 1–29、chunk account ID 不重不漏、already-qualified 不重登。第 1–30 个成功而第 31 个 unknown 时，批次停在同一第 31 项且第 32 个登录 RPC 为 0；对账后只能从第 31 项原 operation 继续。
