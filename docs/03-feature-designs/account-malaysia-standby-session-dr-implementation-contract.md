...
> **2026-08-24 frozen legacy A identity qualification 安全合同：** 全量 item 冻结后，canonical A 的 current authorization、账号 Session 摘要、authorization/authorization-fact/connection generation、authorization fact version、active/current/SV/protected 状态和权威错误均未漂移，但 `health_status=legacy` 时，只允许把它识别为“待真实身份资格探测”的 frozen A；不得直接写成 healthy、不得算作 qualified/completed，也不得绕过 Telegram。runner 必须先对原 A Session 执行现有 `preview_primary_qualification -> Telegram identity probe -> locked fingerprint CAS`；只有 probe 成功且 UID/AuthKey/hash 规则全部通过后，才可按既有资格合同写 identity/healthy/fact generation。probe 异常、hash 无法唯一解析、probe 后 fingerprint 漂移或 CAS 失败时，A 的 Session/current/App/status/health/identity/fact/generation 必须零写入，B/C/E4 operation 必须仍全部不存在，批次首错停止。`invalid|unknown` health、非空 typed authoritative error、disabled 或任何冻结维度漂移都不得进入 Telegram probe。已成功 item 的 A 漂移扫描和最终验收继续只接受 active+healthy 与真实发送读回，legacy 兼容绝不扩散到完成口径。
