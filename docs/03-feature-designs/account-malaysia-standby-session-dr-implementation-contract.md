...
> **2026-08-26 deferred recovery 实施合同：** 第二阶段只处理首轮已终态的 `deferred_reconcile`，不得把 30 个既有 `manual_required` 重新纳入自动流程。`authorization-online-abc-deferred-recovery.sh --mode manifest|preview|apply|status|readback` 提供统一入口：manifest 只读冻结 deferred item/account/original operation/stage/blocker/remote_call_state/candidate/flow/runtime/A/B/C/E4/release/version 并输出 hash；preview/apply 必须提交 expected deferred count、异人审批、fingerprint 和 `--until-exhausted`。若二阶段能力由新 release 部署，preview 的 fingerprint 必须包含旧 `execution_release_sha`、当前 runtime SHA 与 `execution_release_rebind_required`，apply 在 batch 行锁下复算后只写 execution release、status/version 和 start audit，不改账号/operation/slot/bundle，也不复用 legacy stopped release-rebind。compose 管理的 ABC supervisor 逐项持久审计，SSH 断开后按 item audit 恢复。只允许三类结果：持久 completed checkpoint 前滚为 `succeeded`；明确终态失败转 `manual_required`；仍 remote unknown 保留 `deferred_reconcile`。禁止重登、发码、重放 unknown operation 或把未证明项伪装成功。
