...
> **2026-08-26 post-login exact DEV 增量：** 现有canary/full合同不变。三条route已增加账号级full-init binding和`abc_required`；already-authorized必须fresh A probe，已有本账号ABC owner只能attach/readback/reconcile。fixed 2FA/profile完成、复查仍无owner才创建`post_login_exact` queue并异人批准；同租户其他global open batch、runtime/unknown/MY-client继续作为物化门禁。登录worker禁止内联B/C；B pre-challenge Timeout复用`stopped_b_prechallenge_unknown`。当前为本地实现且未发布，生产状态仍未证明。

> **2026-08-26 one-shot durable full sweep 实施合同：** frozen-N 全量只允许一次外部 `--mode sweep --until-exhausted` start/apply。full sweep CLI 不接受 `--max-accounts`；所有 remaining pending item 必须由同一 durable supervisor 逐账号串行推进到 `pending=0`，SSH 断开或 supervisor 重启只能从持久 item/operation checkpoint 恢复，不能等待人工续跑。内部参数统一命名 `checkpoint_interval=30`，只用于每 30 个终态 item 的审计、守恒和门禁复核，绝不是总处理上限，也不能在第 30 个之后退出。单账号确定性失败进入统一 `manual_required/deferred_issue` 队列并继续；同一 operation 对账后仍未知，且 runner=0、client=0、runtime=off、A current/Session/generation/身份无漂移、同一 operation 不可重放时，才记 `deferred_reconcile/quarantined` 并继续。A 漂移、A/B 双失效、2FA、验证码、C 制品/恢复异常全部是异常队列项；不修改 A，不以 C 在线替代。只有 A 无漂移、B/SV、C/MY、双副本、restore probe、Saved Messages remote ID 与断连门禁全部通过才计 `succeeded`。
> **2026-08-26 deferred recovery 实施合同：** 第二阶段只处理首轮已终态的 `deferred_reconcile`，不得把 30 个既有 `manual_required` 重新纳入自动流程。`authorization-online-abc-deferred-recovery.sh --mode manifest|preview|apply|pause|status|readback` 提供统一入口：manifest 只读冻结 deferred item/account/original operation/stage/blocker/remote_call_state/candidate/flow/runtime/A/B/C/E4/release/version 并输出 hash；preview/apply 必须提交 expected deferred count、异人审批、fingerprint 和 `--until-exhausted`。若二阶段能力由新 release 部署，preview 的 fingerprint 必须包含旧 `execution_release_sha`、当前 runtime SHA、`execution_release_rebind_required` 和本次 start 前 `succeeded/manual_required/deferred_reconcile` 基线；status 的真实进度只看 `business_delta.succeeded`，`processed_count` 仅是审计游标。apply 在 batch 行锁下复算后只写 execution release、status/version 和 start audit，不改账号/operation/slot/bundle，也不复用 legacy stopped release-rebind；`pause` 只写 batch pause 审计和状态，不能调用 Telegram 或创建/重放 operation，暂停后可用新的 preview/fingerprint/key 重启同一二阶段。compose 管理的 ABC supervisor 逐项持久审计，SSH 断开后按 item audit 恢复。只允许三类结果：完整持久 B/C/E4 checkpoint、A 门禁和 Saved Messages remote ID 全部通过才前滚为 `succeeded`；明确等待 2FA、终态失败或同证据人工债转 `manual_required`；remote effect 已开始但同一 operation 无法权威证明成功或 no-effect 时保留 `deferred_reconcile`。禁止重登、发码、新建 operation、重放 unknown operation 或把未证明项伪装成功。

> **2026-08-26 one-shot 初始 release 绑定：** 若历史 stopped frozen batch 的 `execution_release_sha` 与当前生产 runtime SHA 不同，且该 batch 尚无 one-shot start 审计，唯一的 sweep `preview/apply` 可在同一 fingerprint/CAS 事务中记录旧 SHA→新 SHA、先收口当前唯一异常 item，再写 start 审计并进入 `sweeping`；该绑定不连接 Telegram、不修改 A/B/C 资产，不是 legacy runner resume。已有 start 审计、运行中/暂停中的 batch 或任一并发安全门禁不满足时必须零写入并走正式 durable pause/reconcile。

> **2026-08-26 B pre-challenge Timeout unknown 实施合同：** `authorization-online-abc-release-interrupted.sh preview|apply|readback` 现在按持久边界分流：原 `release_changed_running` 行为不变；新增 `stopped_b_prechallenge_unknown` 只接受 full batch stopped、唯一 stopped/reconcile_unknown item、原 B `provision_standby_1 + reconcile_unknown/unknown + blocker=TimeoutError + effect present`，且 challenge/code/flow/candidate/owner/lease/reconcile/bundle/stage 全空，B slot 精确绑定原 op、C/E4 absent、runtime off、global unknown/sensitive 精确为原 op、MY client0、A frozen。effect 后若存在 flow，只允许一条完整空 intent，所有字段进入 fingerprint。apply 行锁重验 preview/CAS，创建 applied `b_pre_challenge_remote_unproven` case，保留 effect 与空 intent 证据，将同一 operation 置 `manual_required/reconciled_hold`、B/C/item 投影人工债务并让批次继续；不连接 Telegram、不发码、不建新 flow/op、不写 A/B candidate/远端设备。原 release-interrupted classification、blocker、idempotency 与 readback 保持兼容。

> **2026-08-26 completed A/B 双失效实施合同：** `authorization-online-abc-primary-manual.sh preview|apply|readback` 只接受 full frozen-N 中唯一 `stopped/primary_drift_after_success` 的既往 completed item。A 必须仍是同一 current/slot-current SV authorization，账号/授权 Session 摘要、authorization/connection generation 与 item 冻结值一致，fact delta 单调一致，且新鲜 online fact 为 `desired_online + login_required + last_probe_at`；B 必须仍是原独立 healthy standby 结构，但 preview 和 apply 均以其原 Session/Developer App 做即时只读 identity probe，并且只能接受 Telegram 明确 `session is not authorized`。C 必须通过 `_standby_c` 的 dormant MY、active bundle、KMS、2/2 copy、restore probe 与断连门禁；原 B/C/E4 operation 均须 succeeded，E4 audit 必须有非空 remote message id。全局必须 runner0、runtime off/scope空、unknown0、sensitive0、MY client0。batch 可额外存在一个因同次全局扫描遗留的 running item，但它必须已 B/C/E4 全 succeeded、A qualified、C 恢复证据与 E4 remote ID 完整；其 item/A/operation 版本和 remote ID 进入 fingerprint，apply 对其零写入。apply 在 batch/item 行锁内重跑全部门禁和 fingerprint，只把 B authorization 投影 invalid/needs_repair、B slot 与 item 记为 blocker=`primary_and_sv_standby_unavailable`、execution release 绑定当前生产并写幂等审计；A、C、bundle、operation 与 Telegram 均零写入。该明确 blocker 的 manual item 从后续 completed-A 扫描中只在 current/Session/generation/fact delta 仍等于已承认失败边界时排除，后续任一新 A 漂移仍停批；manual 仍计入最终债务、不计 succeeded。任一 B 恢复可用、C/E4 证据缺失、A/版本/审批漂移都零写入拒绝。

> **2026-08-26 acknowledged-primary chunk pause/rebind 合同：** `online_abc_chunk._require_manual_primary_unchanged` 与 `online_abc_release_rebind._require_manual_primary` 必须和 `online_abc_primary` 的 manual-A 漂移扫描共用同一判断。普通 manual item 仍只接受 `frozen|legacy_frozen|qualified`；仅 blocker 精确为 `primary_and_sv_standby_unavailable` 时，接受专用入口已确认的同 current/slot-current/protected、Session digest、authorization/connection generation 与一致正 fact delta。该例外只允许 chunk pause 写 batch stopped/version/audit，随后 release-rebind 仍只写 execution release/status/version/audit；不改变 item outcome、A/B/C、Session、operation、slot、bundle 或 Telegram。任一新漂移必须在各自 commit 前以 `online_abc_primary_drift` 拒绝。

> **2026-08-25 quiescent chunk release-rebind 合同：** 外部发布若发生在两个正常 max-30 chunk 之间，batch 已由 runner 原子停在 stopped 且不存在 running item，不得要求伪造 mid-chunk 中断。release-rebind preview 可把最新且同审批的“ABC runner chunk 边界停批”作为 release boundary，但必须逐字冻结旧 execution release，并使 succeeded/manual/pending 计数与当前 frozen items 完全一致；仍须复核全部 completed/manual A、completed B/C/E4、runtime off、global unknown/sensitive=0、MY client=0。apply 在行锁下重算 fingerprint 后只更新 batch execution release/status/version并写审计；不连接 Telegram、不改 A/B/C、Session、operation、slot 或 bundle。任一旧审计、计数/release/审批不匹配均零写入拒绝。

> **2026-08-25 completed-checkpoint release pause 合同：** runner 在 B/C/E4 均成功后、item/slot sync 前被生产发布替换时，只能通过专用 DB-only `preview/fingerprint/apply/readback` 收口。入口必须冻结 batch/item/slot/operation versions、旧/新 release、A Session/current/generation、E4 remote message ID、B/SV、C/MY bundle 2/2 与 restore probe，并要求 runner0、runtime off、unknown/sensitive/MY client 全零。apply 只在同一事务执行普通成功投影并写 `production_release_changed_mid_chunk` pause audit；禁止 Telegram RPC、重建 operation、修改授权/Session/bundle或直接重绑 release。现有 release-rebind 必须在该 pause 审计之后独立 preview/CAS apply。

> **2026-08-25 C post-bundle 发布中断实施合同：** `authorization-online-abc-post-bundle-interrupt.sh preview|apply|readback` 仅接受 full rollout 唯一 running item 的原 C operation 已到 `bundle_copies_verified/remote_call_state=confirmed`，stage 顺序精确止于 `central_receipt_committed`，candidate、active=false bundle、两类不可变副本、inventory 和 KMS readback 完整，而 restore probe、slot decision、E4、reconcile case 均不存在；原 owner lease 必须过期，runtime 必须精确 scope 原 C，global unknown=0、唯一 sensitive=原 C、MY ready/client=0、A frozen、B succeeded，且 current backend release 与 batch execution release 不同。preview 冻结 batch/online item+slots/migration item+batch、A/B、operation/owner/lease、node capability/image、candidate、bundle/copies/inventory 与发布/502证据；apply 需原 requester、异人 approver、approval_ref、唯一 key、fingerprint/CAS，只建立 `central_bundle_restore_forward` repair case，把 online item 置 stopped/runner_blocked=`malaysia_wake_unavailable`、batch stopped、runtime off，并保留原 C/candidate/bundle/副本。扩展后的 artifact claim 只允许该 case 以 code-source A 作为 frozen source 重领过期 lease，从原 bundle 执行无登录 restore probe 与 slot commit；成功写 reconcile applied 后，runner 才可复用原 C 从 post-C/pre-E4 恢复。禁止 Telegram login/code/2FA、新 operation、新 bundle generation、覆盖副本或改 A/B。

> **2026-08-26 C 控制面中断实施合同：** `authorization-online-abc-c-precode-interrupt.sh preview|apply|readback` 只处理一个 frozen full-batch 原 C operation，边界冻结为 `active_control_plane_interrupt|stopped_login_unknown|stopped_login_post_code_unknown`。前两者维持原 pre-code 合同；post-code 仅接受 stopped/reconcile_unknown item+slot、`provision_reconcile_unknown/unknown`、challenge、code message 与 received-at 同时存在、lease 已清、runtime off、全局 unknown/sensitive 精确为原 C，并与 pre-code 一样强制唯一 `remote_login_started` stage、无 flow/candidate/bundle/remote-confirmed、A 冻结、B 成功、MY client0。preview 冻结 batch/item/slot/migration/node/A/B/C/版本/release/code-boundary/审批引用；apply 行锁重验 fingerprint/CAS，只写独立 `c_post_code_remote_unproven` reconcile case/audit，将同一 C/migration/item 收口 `manual_required/reconciled_hold`、保留原 code/effect 证据、投影 B succeeded/C manual 并让 batch 继续。A、B authorization、Telegram、Session、远端设备和 bundle 零写入；code 半字段、额外 stage/candidate/bundle、任何漂移均拒绝。

> **2026-08-25 B pre-flow 发布中断人工结案合同：** full rollout 唯一 runner 在 B operation 已持久化 `login_remote_started/remote_call_state=started/remote_effect_started_at` 后、`start_standby_authorization_login` 返回并把 flow 关联回 operation 之前被生产 release 重启时，禁止重跑 runner、重登、重发码或新建 operation。只有 batch/item 仍为唯一 `running`、原 B operation 无关联 flow/challenge/code/candidate/owner lease/reconcile，且同一 effect 后不存在 flow，或精确只有一条同账号/standby_1/App、`intent_persisted@v1`、无 challenge/code/temporary Session/phone-code-hash/authorization/error/batch-attempt/supersede 的空本地 intent，B/C slots 均未绑定且 pending、C/E4 absent、A frozen 无漂移、runtime off、global unknown=0、MY client=0、唯一 sensitive operation 正是该 B、runner process=0，且 current release 与 batch execution release 不同时，才允许专用 DB-only `preview/fingerprint/apply/readback` 把该同一 operation 结案为 `manual_required/reconciled_hold`。空 intent 的 id/version/created-at/空字段必须进入 fingerprint；apply 将其关联回原 operation 并终止为 `superseded` typed manual debt，创建 applied reconcile case、保留 remote effect 时间、将 item 与 B/C slots 记为 manual debt、重绑 execution release并继续后续 pending。不得宣称 confirmed-no-effect 或 succeeded；任何第二 flow、challenge/code/candidate/设备新事实、A 漂移、第二 sensitive operation 或 release 未变化都必须零写入拒绝。

> **2026-08-24 全量 ABC 人工失败结案与继续合同：** 单个 full-rollout item 已因原 B 或 C operation 的 typed 失败进入 `manual_required`，且远端调用明确 `confirmed_no_effect`、unknown 已清零、后续阶段尚未创建时，不得重发码、重登、新建 operation 或把该账号伪装为成功。B 失败只接受 C/E4 均不存在；C 失败只接受 B 已 `already_qualified|succeeded`、A probe succeeded 且 E4 不存在。只有用户对精确 batch/account/blocker 明确批准后，才允许专用 DB-only `preview/fingerprint/apply` 将失败阶段及未开始的下游槽记为 `manual_required`，保留已成功上游事实、原失败 operation 及 blocker，A 的 current authorization、Session 摘要、App、状态、fact 与三组 generation 全部零写入。preview 冻结 batch/item/slot/operation 版本、失败阶段、A 快照、runtime/global unknown/sensitive/MY client、当前 release、异人审批、approval ref 与 idempotency key；apply 锁行重算 fingerprint/CAS，写 secret-free audit，并把同一 frozen batch 的 execution release 绑定到当前 release 后只恢复后续 pending item。该例外只适用于 `selection_mode=all_online_accounts`，canary 永不接受 manual。one-shot supervisor 将 `manual_required/deferred_issue` 作为真实终态债务，不退出、不等待人工补跑，最终首轮只以 `pending=0` 收口，结果必须分别报告 succeeded/manual/deferred_reconcile/unresolved。
> **2026-08-25 B 验证码不可读人工结案补充：** `verification_code_unreadable` 只允许在原 B operation=`failed/remote_call_state=started`、blocker 精确匹配、challenge/effect 时间和原 waiting-code flow 存在、operation 未绑定 code message/received-at、flow 未产生 authorization、candidate 为空、C/E4 不存在时进入同一 DB-only manual preview/apply。这里保留“challenge 已发送”的真实远端效果，不把 operation 改写成 `confirmed_no_effect`；人工结案只把 item 与 B/C 槽记为 manual debt，原 failed operation/flow 完整保留。任何 code 已绑定/收到、candidate/authorization 已出现、flow 非 waiting-code、unknown/sensitive/A 漂移都拒绝，禁止重发验证码、重登或创建 retry operation。

> **2026-08-24 frozen legacy A identity qualification 安全合同：** 全量 item 冻结后，canonical A 的 current authorization、账号 Session 摘要、authorization/authorization-fact/connection generation、authorization fact version、active/current/SV/protected 状态和权威错误均未漂移，但 `health_status=legacy` 时，只允许把它识别为“待真实身份资格探测”的 frozen A；不得直接写成 healthy、不得算作 qualified/completed，也不得绕过 Telegram。B 已冻结为 `already_qualified` 时，runner 先执行 `preview_primary_qualification -> Telegram identity probe -> locked fingerprint CAS`。若 B plan=`provision` 且 A 尚未完整 qualification，则 Telegram 对 current A 返回 `hash=0`、没有可用 observer 是合法冷启动状态：runner 必须先用原 A Session 只读探测 UID/AuthKey 并冻结进 B preview/operation，完成唯一 B 登录后，再从 B 视角唯一解析 A hash并执行同一 locked qualification；禁止把 `hash=0`、App/IP/设备名或 B hash冒充 A hash。首次 A 探测失败必须保持 B/C/E4 全不存在；B 登录或随后 A qualification 失败时必须首错停，A 的 Session/current/App/status/health/identity/fact/generation 仍零写入，已成功 B 只能作为受保护 observer 保留并由同一 item checkpoint 前滚，禁止重登。`invalid|unknown` health、非空 typed authoritative error、disabled 或任何冻结维度漂移都不得进入 Telegram probe。已成功 item 的 A 漂移扫描和最终验收继续只接受 active+healthy 与真实发送读回，legacy 兼容绝不扩散到完成口径。

> **2026-08-24 pending B/C plan DB-only rebase 合同：** 旧 release 冻结为 `pending` 的 item 仅在 batch=`stopped`、runtime=`off` 且 scope 为空、global unknown=0、MY active client=0、item status/outcome 均为 pending、B/C/E4 operation 全不存在、B/C slot result 仍为未启动 pending、A 符合前述 frozen/qualified 且其 current authorization/Session/全部 generation/fact version 精确等于 item、C source 身份与 fact/slot generation 未变化时，才可进入专用 batch-wide `preview/fingerprint/apply`。preview 必须用当前数据库 assignment/App/C facts复跑正式 `_freeze_full_target`，只选择旧 B/C plan 或 B route 与当前确定性结果不同且新 B/C plan 均非 blocked 的 item；冻结 batch version、每个 item version、A/C immutable projection、旧/新 B route 与旧/新 B/C plan、精确账号集合、预期 target count、幂等键和原批次异人审批，输出稳定 fingerprint。apply 先锁 batch 与全部 item、重算同一 target set/fingerprint 并执行 item-version CAS，只允许更新 `app_b_id/app_b_credentials_version/app_b_assignment_purpose/app_b_assignment_version/proxy_id/standby_1_plan/standby_2_plan/item.version` 并写审计；batch、A 字段、C source 字段、Session/current/App/generation、slot result 和任何 operation 均不得修改，也不得连接 Telegram。任一 item 不再合格、target count 不等、fingerprint/version 漂移时整批零写入；相同 idempotency key+fingerprint 只读回原结果，key 或 fingerprint 冲突显式失败。

> **历史兼容：2026-08-24 rebind 后 rebase 与 chunk 静默边界合同：** 仅适用于旧 runner/chunk-pause 批次；当前 one-shot sweep 的顺序是 `release gate -> one-shot sweep start -> durable supervisor -> checkpoint_interval=30 -> pending=0`，不通过 `--max-accounts` 形成外层暂停。当生产 release 漂移与旧批次 pending plan 漂移同时存在时，正式顺序固定为 `release-rebind preview/fingerprint/apply -> pending-plan rebase preview/fingerprint/apply -> legacy runner`。pending rebase 仅可额外接受刚由正式 release-rebind 恢复的 `running` batch：最新 batch 审计必须精确为同审批的 release rebind，execution release 必须匹配该审计，items 仍仅 `succeeded|pending` 且 running/stopped=0；SSH apply wrapper 继续拒绝 runner 进程，DB 内继续强制 runtime off/scope 空、global sensitive/unknown=0、MY client=0 和全部 A/C/operation/slot CAS。普通 running、已有新 batch 审计或任一 active item 仍拒绝。旧 runner 每次达到 `--max-accounts` 且 frozen N 尚未完成时，必须在当前账号 B/C/E4 与 A send/readback 全部终结后，锁 batch 重验 items 守恒、runtime/sensitive/unknown/MY 静止，再写 chunk-boundary pause 审计并把 batch 置 `stopped`；下一次同 execution release、同异人审批的 `run` 只可从该最新审计原子恢复，其他 stopped 原因不得自动恢复。pause/resume 只改 batch status/version 与审计，不改 A/B/C、Session/current/App/generation、slot 或 operation。
...
> 当前状态：`design_status=complete`、`product_resync_status=complete`、`dev_handoff_ready=true`、`implemented_scope=ten_account_canary_ready_for_primary_send_acceptance_plus_p0_local_recovery_and_one_shot_full_sweep_local_candidate`、`legacy_a_qualification_safety_design=complete`、`pending_plan_db_rebase_design=complete`、`core_deployed=50f657c03c6ad0ef60b75eced1bf1390516ce290`、`ssh_mirror_deployed=true`、`slot_canary=2/2_historical_pass`、`ten_account_acceptance_gate=primary_stable_plus_saved_message_remote_id`、`fixed_observation_duration=removed`、`p0_local_recovery=2/2_verified`、`approved_batch_runner=production_verified_run_and_status`、`full_online_abc_design=complete`、`full_online_abc_implementation=durable_sweep_local_candidate_release_pending`、`full_prd_implementation=partial`、`runtime_mode=off`、`production_fixed=false`
...
> 批次执行入口合同：GitHub Actions 仅发布代码，不执行账号登录。canary/legacy 非全量批次仍使用 `deploy/authorization-online-abc-runner.sh --mode status|run|resume`；`selection_mode=all_online_accounts` 的 frozen-N 批次必须使用 `deploy/authorization-online-abc-sweep.sh --mode preview|sweep --until-exhausted`，full sweep 不接受 `--max-accounts`。sweep 只消费已审批的 `TgAuthorizationOnlineAbcBatch`，从 item 冻结事实和确定性 key 驱动既有 canonical qualification、`apply_abc_backup/prepare_scoped_c_migration`、`apply_abc_e4`、`sync_online_abc_batch`，不得复制登录业务逻辑。supervisor 单账号串行，C 未终态时只轮询原 operation；已成功阶段不重放。确定性异常进入统一队列后继续；unknown 先同 operation 对账，仍未知才在安全门禁满足时 quarantine；A 漂移或 runtime scope 冲突显式暂停并保持原事实。相同 batch ID 重启必须从数据库阶段恢复；不得依赖 GitHub Actions workflow_dispatch、操作者手抄 MY image SHA 或临时 Python/SQL。
...
> post-B/pre-C MY readiness 恢复合同：`_create_c` 在 C operation 创建前读取 ready MY node，因此 B succeeded 后也可能以 `runner_blocked + malaysia_wake_unavailable` 停止。`resume` 必须先按 operation 形状区分断点：C 缺失且 E4 缺失时只接受原 B succeeded/candidate、互补 SV slot、同 UID/不同 AuthKey/不同 App、A qualified 且冻结 current/Session/generation 不漂移；随后再次验证 MY heartbeat fresh、client=0、runtime off/scope 空和 global unknown=0，审计 checkpoint=`post_b_pre_c`。恢复只令同一 item/batch CAS running，普通 runner 跳过既有 B 并以原确定性 key 首次创建 C；禁止再次登录 B、禁止换 key/账号，MY 仍不可用时零写入拒绝。
> pre-primary 恢复合同：canonical A 是 current 业务角色，不等于持久物理槽名；合法 `local_activate` 后 current authorization 可保持 `logical_slot=standby_1`。qualification 只接受 current/slot-current、SV、账号 Session/App、冻结 authorization/fact/connection generation 全一致的 `primary|standby_1`。旧 runner 因该硬编码或 legacy-only A 的 observer 冷启动死锁在 qualification 前落成 `runner_blocked + ValueError` 时，`resume` 仅可在 C/E4 operation 不存在、A `_primary_state=frozen|legacy_frozen`、canonical preview 成功、runtime off、global unknown=0、原审批一致时恢复原 item；B operation 必须不存在，或已精确 `succeeded` 且 candidate 是 operation 冻结 UID 相同/AuthKey 不同、互补 SV healthy/non-current 的唯一 observer。审计 checkpoint=`pre_primary_no_remote_effect|post_b_pre_primary_qualification`。这不是通用 ValueError 重试入口；其他 B 状态、任一 C/E4、A 漂移或 observer 不匹配均拒绝，已成功 B 绝不重放登录。
...
> 缺失 A UID/AuthKey 的 bootstrap 合同：仅 runner 可在 B preview/apply 上显式开启 `bootstrap_missing_primary_identity`。入口先以冻结 A Session/App/proxy做只读 identity probe，把 UID/AuthKey 摘要写入 preview fingerprint 和 B operation expected facts，不写 A 行、不增 fact version；普通 preview 默认关闭并保持 DB-only。B code-source fence 接受“本地 UID/AuthKey 全部缺失或只缺其中一项、operation expected 两者非空，且每个本地非空值均与 expected 精确相同”的 legacy 状态，同时仍强制 current/Session/App/authorization/fact/connection generation 全一致；任一已存字段与探测不一致立即失败。B candidate 必须同 expected UID、不同 expected AuthKey；成功后通过 B observer 完整 qualification A，A fact generation 只增加一次。
> pre-B approved/no-effect 恢复合同：若上述 partial legacy identity 在旧 fence 中被误判，允许留下唯一 B operation=`approved`，但必须同时满足 `remote_call_state=none`、`remote_effect_started_at/login_flow_id/candidate_authorization_id` 全为空、blocker 空、reconcile none，且 C/E4 不存在。正式 runner resume 只在同 stopped item/blocker=`code_source_changed`、A 仍 frozen/current/Session/App/三组 generation/fact 不变、每个已存 identity 字段与 operation expected 相同、runtime off/global unknown=0 和原审批一致时审计 checkpoint=`pre_b_approved_no_remote_effect`；恢复后只执行该原 operation 一次，禁止新 key、新 operation 或第二次 preview/login。任一远端效果字段已存在、identity 不一致或 A 漂移继续 stopped。
...
> post-local-activate frozen item 重基线合同：仅允许原 blocker=`c_orphan_revoked_retry_ready`、原 C operation=`migration_rolled_back_forward/compensated/reconcile applied`、B/E4 operation 均不存在的唯一 stopped item 进入 `authorization-online-abc-post-activate-rebase.sh`。preview 必须 DB-only 冻结 applied local-activate case、非空 verification operation/message ID、新 current 的 Session/App/UID/AuthKey/fact/三组 generation、旧 A typed duplicate+protected、原 source C 和剩余 SV App route；apply 复算 fingerprint、校验原 batch requester/approver/ref 和幂等键，只更新同一 item A 基线、B provision route 与 B slot pending，并审计 checkpoint。runner resume 再次验证上述事实和全局 runtime off/unknown zero，从 `post_local_activate_rebase` 创建互补 B；失败不得回切、降级或改写 current A。
> completed item 切主恢复与 pre-remote rearm 合同：`authorization-online-abc-completed-recovery.sh` 的 completed preview/apply 只接受 `primary_drift_after_success`、applied local-activate send readback、原 B operation absent、C/E4 succeeded 且均绑定旧 A；apply 保留 C 与旧 E4，重绑新 A/B pending。runner 使用 E4 retry key，只有绑定新 A 的新 message ID 才重新完成 item。若旧 runner 已先恢复另一个 item 为 running、再因 completed A 扫描停批，rearm 只在该 running item 尚无 B/E4、新 A 基线和 compensated C 完全不变、同批存在另一个 stopped primary drift 时恢复原 post-local-activate blocker。指定 account resume 前必须无 running item，并先完成 succeeded A 漂移扫描。
...
> B 旧 managed 2FA 恢复合同：只允许原 `provision_standby_1` 因 `PasswordHashInvalidError` 停在 `reconcile_unknown`，且原 flow 临时 Session、A/current/Session/generation、online item/slot、runtime off 与租户固定 2FA 版本均冻结。legacy A 的中心 UID/AuthKey 同时为空时，必须复用 operation 已冻结的 bootstrap expected identity，不得为 preview 写 A。旧 item proxy plan 与已创建 operation 的新版 `primary_regular:direct` 不同时，fingerprint 同时冻结 planned proxy 与 operation egress，但实际连接只服从 operation/flow exact match，禁止为迁就旧 plan 改 flow 或回退代理。`preview` 纯数据库生成不含明文的 fingerprint；异人审批 `apply` 先只读探测同一临时 Session，若已 authorized 则直接收口，只有明确 `session is not authorized` 才对同一 flow 做一次 password-only 提交，禁止重发验证码、创建新 flow/operation 或改 A。远端设备归属必须以恢复 Session 返回的 authorization hash、App API ID、稳定设备 fingerprint 和从 A 视角 `non-current` 四项唯一匹配；同一 App 的其他历史 Session 合法存在，不能仅按 API ID 计数并误判冲突。匹配 hash/fingerprint 只以 digest 进入 reconcile manifest。固定密码再次无效写 `manual_required/confirmed_no_effect`；其他异常保留同一 unknown。远端成功但响应丢失时，下次仅以同一 Session authorized probe 前滚，不重复提交密码。

> legacy A 首次托管 2FA 恢复合同：旧 B runner 已完成 A-bound code sign-in、Telegram 明确返回 `waiting_2FA` 并把 post-code 临时 Session 持久化，但账号安全快照为 `trusted_session_status=confirmed/two_fa_status=enabled` 且托管密码密文与 stored-at 均为空时，旧通用 verify 会以 `ValueError` 落成同一 B `reconcile_unknown`。仅在 A=`legacy_frozen`、operation 已冻结非空 expected UID/AuthKey、flow 精确 `waiting_2FA`、batch/item/slot stopped、C/E4 不存在、runtime off、该 operation 是全局唯一 unknown、MY client=0、租户固定 2FA 已配置且当前仍无 healthy B 时，允许复用上述 SV 2FA preview/apply。fingerprint 必须额外冻结 blocker、flow status、空 managed-secret 标记、固定 2FA 版本和当前 release；apply 只对同一临时 Session 做 authorized probe，明确未授权时只提交一次 password-only，不得重发 code、新建 flow/operation 或写 A。成功时原子提交互补 B、reconcile case 与托管密码快照；runner 仅凭该 reconciled B 接受 `legacy_frozen` checkpoint，并先由 B observer 完成 A locked qualification 后才创建 C。`waiting_code`、有托管密码、非 legacy A 或其他任意 `ValueError` 一律拒绝；密码无效进入 `manual_required/confirmed_no_effect`，SSH/Telegram 不明结果保持原 unknown 与临时 Session，A 全字段零写入。

> C artifact-forward 后 runner 恢复合同：原 item 保持 stopped/outcome=`reconcile_unknown`，只在同一 C operation 已由正式 case 与原 artifact 前滚为 `succeeded/remote_call_state=confirmed/reconcile_status=applied`、存在 reconcile case 与 candidate、E4 尚不存在、B/C slot 均已完成、A 仍 qualified、runtime off/global unknown=0 时，`resume` 才把原 item 恢复到 `post_c_pre_e4`。恢复必须复用原 C operation，不得重登、重发码或创建第二个 C；随后只创建原确定性 E4 并重新读回 A。未带完整 reconcile 证据的 C succeeded 仍拒绝恢复。
...
> 历史全量滚动 runner 读回：release `cd06f75ca200552d621507f492629acd721a808f`、Alembic head=`0165_online_abc_full` 曾部署 frozen-N manifest preview/apply、健康 B/C readback、旧 SV C migrate、source-less C/MY provision 和 legacy runner。该历史 runner 的 `--max-accounts` 不适用于当前 one-shot 合同。当前 batch `718657f1-6582-45e7-b0aa-40a4ea1bda3c` 为 B/C/E4 `10/10 observing`；逐账号 A 仍为原 current 且 active/healthy，10/10 Saved Messages 均有远端 ID，C 均为 MY current、双副本、KMS verified、restore probe passed/authorized。用户已取消固定 24 小时等待；v2.23 accept gate 发布后只需即时复核上述 A 与发送事实及 runtime/unknown/MY client 门槛。
...
> healthy partial-identity A 的 B 2FA 恢复补充：`sv_two_fa_resume` 对 `ValueError` 另外只接受 `A active/healthy/current/slot-current/protected + A UID 缺失 + A AuthKey 非空且精确匹配 operation expected AuthKey + expected UID 非空 + flow waiting_2FA + challenge/message/received/effect 齐全 + tenant fixed 2FA present`，且账号安全快照必须 absent 或为 nonasserting：trusted Session/2FA 均为 unknown，无托管密码/提示/stored-at、外部授权、设备扫描、2FA 检查、可信设备标签和错误事实。profile/last-hardened 等非安全断言元数据允许存在，但完整快照字段必须进入 fingerprint，敏感值只保存摘要。preview 为 DB-only；apply 前任一快照漂移均以 `reconcile_evidence_conflict` 零写入拒绝，随后仅复用原 operation/flow/temporary Session，先探测是否已授权，只在明确未授权时提交一次 tenant fixed password。成功才建 B candidate 并更新账号安全快照；`PasswordHashInvalidError` 原子转 manual/no-effect，其他异常保持 unknown。UID 已存、AuthKey 缺失/不匹配、任何 confirmed/enabled/disabled、托管密码、设备/2FA 检查、外部授权、可信标签、错误事实、waiting-code、candidate/新 flow/其他 unknown 仍全部零写入拒绝。
...
7. `P3 full online ABC rollout`：只在新的 10 账号 canary 通过即时 A/发送验收后创建 `selection_mode=all_online_accounts` preview。创建事务先冻结全部在线账号为动态 `N`，再以一次 `--mode sweep --until-exhausted` 启动 durable supervisor；它从逐项 A -> B -> C/E4 checkpoint 自动继续到 `pending=0`，不得通过 `--max-accounts 30` 或人工重复调用形成外层分段。内部 `checkpoint_interval=30` 只写检查点审计并立即继续。每次开始前仍须复核 release/schema/runtime off/global unknown/MY client/批次守恒。MY 节点仅对需要 C 登录的账号逐个串行，App/host rate bucket、账号 lease 和批次守恒同时生效。确定性失败进入统一异常队列并继续；unknown 必须先对账同一 operation，仍未知时按 `deferred_reconcile/quarantined` 合同隔离后继续。批次首轮只在 `pending=0` 且三组各自守恒为 N 时收口；只有全部账号/B/C outcome 为 `already_qualified|succeeded`、无异常且所有真实验收证据齐全时才计 ABC 完成。
...
- 全量 rollout 使用一个 frozen `N`，一次调用 `--mode sweep --until-exhausted` 后由同一 supervisor 自动覆盖全部 pending item；验证 `checkpoint_interval=30` 只产生审计、不改变总处理数，checkpoint 后第 31 个及以后仍在同一生命周期继续，account ID 不重不漏、already-qualified 不重登。第 31 个发生确定性失败时只把该 item 入统一异常队列并继续；发生 unknown 时先对账同一 operation，满足隔离门禁后记为 `deferred_reconcile/quarantined` 并继续，任何 A 漂移或安全门禁失败则显式暂停而不是重放。
