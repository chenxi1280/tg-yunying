...
> **2026-08-25 C pre-code 中断实施合同：** `authorization-online-abc-c-precode-interrupt.sh preview|apply|readback` 只处理一个 frozen full-batch 原 C operation，并将边界冻结为 `active_control_plane_interrupt` 或 `stopped_login_unknown`。前者要求 running item、`login_remote_started/started`、过期 lease、runtime exact scope、unknown=0；后者要求 stopped/reconcile_unknown item+slot、`provision_reconcile_unknown/unknown`、lease 已清、runtime off、全局 unknown/sensitive 均精确为原 C。preview 同时冻结 batch/item/slot/migration item+batch version、A 快照、已成功 B、C owner/epoch、唯一 `remote_login_started` stage、MY node capability/image、当前 backend release 与中断证据引用；apply 需要原 requester、异人 approver、approval_ref、唯一 key 和 fingerprint，并在行锁下重验。成功只写 reconcile case/audit、将 C 与 migration item收口 `manual_required/reconciled_hold`、清残余 lease/关闭 exact runtime scope、投影 B succeeded/C manual/item manual 并让 batch 继续；A、B authorization、Telegram、Session、code、远端设备和 bundle 均零写入。任何 code/flow/candidate/bundle/remote-confirmed、A/B/版本/node/runtime 漂移、额外 sensitive/unknown/MY client 或不匹配的 item/slot 状态都拒绝。
