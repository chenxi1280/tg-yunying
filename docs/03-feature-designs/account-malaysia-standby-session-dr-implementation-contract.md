...
> healthy partial-identity A 的 B 2FA 恢复补充：`sv_two_fa_resume` 对 `ValueError` 另外只接受 `A active/healthy/current/slot-current/protected + A UID 缺失 + A AuthKey 非空且精确匹配 operation expected AuthKey + expected UID 非空 + account security snapshot absent + flow waiting_2FA + challenge/message/received/effect 齐全 + tenant fixed 2FA present`。preview 为 DB-only，fingerprint 冻结空 security snapshot 和上述全量证据；apply 仅复用原 operation/flow/temporary Session，先探测是否已授权，只在明确未授权时提交一次 tenant fixed password。成功才建 B candidate 并创建账号安全快照；`PasswordHashInvalidError` 原子转 manual/no-effect，其他异常保持 unknown。UID 已存、AuthKey 缺失/不匹配、security snapshot 存在但未证明托管状态、waiting-code、candidate/新 flow/其他 unknown 仍全部零写入拒绝。
