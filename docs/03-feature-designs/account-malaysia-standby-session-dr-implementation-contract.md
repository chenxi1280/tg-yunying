...
> **2026-08-24 全量 ABC 人工失败结案与继续合同：** 单个 full-rollout item 已因原 operation 的 typed 失败进入 `manual_required`，且远端调用明确 `confirmed_no_effect`、unknown 已清零、C/E4 尚未创建时，不得重发码、重登、新建 operation 或把该账号伪装为成功。只有用户对精确 batch/account/blocker 明确批准后，才允许专用 DB-only `preview/fingerprint/apply` 将该 item、B slot 和未开始的 C slot统一记为 `manual_required`，保留原 B operation 及 blocker，A 的 current authorization、Session 摘要、App、状态、fact 与三组 generation 全部零写入。preview 冻结 batch/item/slot/operation 版本、A 快照、runtime/global unknown/sensitive/MY client、当前 release、异人审批、approval ref 与 idempotency key；apply 锁行重算 fingerprint/CAS，写 secret-free audit，并把同一 frozen batch 的 execution release 绑定到当前 release 后只恢复后续 pending item。该例外只适用于 `selection_mode=all_online_accounts`，canary 永不接受 manual。runner 每个新账号仍首错即停、每次最多 10 个；chunk 守恒把 `manual_required` 作为真实终态债务而不是成功，最终批次只能是 `completed_with_manual`，结果必须分别报告 succeeded/manual/pending，不能声明 B/C 全成功。
