...
> B 旧 managed 2FA 恢复合同：只允许原 `provision_standby_1` 因 `PasswordHashInvalidError` 停在 `reconcile_unknown`，且原 flow 临时 Session、A/current/Session/generation、online item/slot、runtime off 与租户固定 2FA 版本均冻结。`preview` 纯数据库生成不含明文的 fingerprint；异人审批 `apply` 先只读探测同一临时 Session，若已 authorized 则直接收口，只有明确 `session is not authorized` 才对同一 flow 做一次 password-only 提交，禁止重发验证码、创建新 flow/operation 或改 A。固定密码再次无效写 `manual_required/confirmed_no_effect`；其他异常保留同一 unknown。远端成功但响应丢失时，下次仅以同一 Session authorized probe 前滚，不重复提交密码。
