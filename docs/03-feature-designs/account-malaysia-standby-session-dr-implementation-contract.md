...
> **2026-08-25 C pre-code 控制面中断实施合同：** `authorization-online-abc-c-precode-interrupt.sh preview|apply|readback` 只处理一个 frozen full-batch running item 的原 C operation。preview 冻结 batch/item/slot/migration item+batch version、A 快照、已成功 B、C owner/epoch/过期 lease、唯一 `remote_login_started` stage、MY node capability/image、runtime scope、当前 backend release 与中断证据引用；apply 需要原 requester、异人 approver、approval_ref、唯一 key 和 fingerprint，并在行锁下重验。成功只写 reconcile case/audit、将 C 与 migration item 收口 `manual_required/reconciled_hold`、清过期 lease、关闭 exact runtime scope、投影 B succeeded/C manual/item manual 并让 batch 继续；A、B authorization、Telegram、Session、code、远端设备和 bundle 均零写入。任何 code/flow/candidate/bundle/remote-confirmed、A/B/版本/node/runtime 漂移、其他 sensitive/unknown/MY client 或未过期 lease 都拒绝。
