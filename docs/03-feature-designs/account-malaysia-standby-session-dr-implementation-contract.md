...
> **2026-08-25 B pre-flow 发布中断人工结案合同：** full rollout 唯一 runner 在 B operation 已持久化 `login_remote_started/remote_call_state=started/remote_effect_started_at` 后、`start_standby_authorization_login` 返回并持久化 flow 之前被生产 release 重启时，禁止重跑 runner、重登、重发码或新建 operation。只有 batch/item 仍为唯一 `running`、原 B operation 无 flow/challenge/code/candidate/owner lease/reconcile、B/C slots 均未绑定且 pending、C/E4 absent、A frozen 无漂移、runtime off、global unknown=0、MY client=0、唯一 sensitive operation 正是该 B、runner process=0，且 current release 与 batch execution release 不同时，才允许专用 DB-only `preview/fingerprint/apply/readback` 把该同一 operation 结案为 `manual_required/reconciled_hold`。apply 创建 applied reconcile case、保留 remote effect 时间、将 item 与 B/C slots 记为 manual debt、重绑 execution release 并继续后续 pending；不得宣称 confirmed-no-effect 或 succeeded。任何 flow/code/candidate/设备新事实、A 漂移、第二 sensitive operation 或 release 未变化都必须零写入拒绝。
