...
> **2026-08-26 acknowledged-primary chunk pause 合同：** `online_abc_chunk._require_manual_primary_unchanged` 必须与 `online_abc_primary` 的 manual-A 漂移扫描共用同一判断。普通 manual item 仍只接受 `frozen|legacy_frozen|qualified`；仅 blocker 精确为 `primary_and_sv_standby_unavailable` 时，接受专用入口已确认的同 current/slot-current/protected、Session digest、authorization/connection generation 与一致正 fact delta。该例外只允许写 batch stopped/version 与 chunk-boundary audit，不改变 item outcome、A/B/C、Session、operation、slot、bundle 或 Telegram；任一新漂移必须在 commit 前以 `online_abc_primary_drift` 拒绝。
