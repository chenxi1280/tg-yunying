...
6. Listener 与 Planner 访问同一任务时必须统一 `TaskPlannerWakeState -> Task` 锁序；Listener 不得先把 Task 变为 dirty 再查询 wake 行，否则 autoflush 会形成 `Task -> Wake` 反向锁并死锁。
...
- 锁序：Listener 取得 Task 对象后必须先锁/推进 wake，再写监听 stats/`next_run_at`；Planner 同样先锁 wake、后提交 Task 投影。任一死锁都视为事件到目标链失败。
...
- PostgreSQL 并发回归：Planner 持有 wake 行并准备更新 Task 时，Listener 同时处理新增消息；双方均提交、wake revision 不丢失且 deadlock=0。
