...
> **2026-08-28 逐账号频道准入补正：** `OperationTarget.auth_status/can_send` 只描述目标级能力，不能证明任务账号池中的每个账号已经加入频道。频道浏览、点赞、评论和回复必须逐账号读取已确认 membership；缺少账号-频道关系的账号先进入 `ensure_channel_membership`，只有该账号的 typed membership 结果可使其进入主互动。禁止用目标级 `can_send=true`、账号存在 Session 或 `-100` peer 形式推断账号已加入。
...
- 启动频道浏览、点赞、评论或回复任务时，任务账号范围内每个缺少已确认 membership 的账号都先关注频道；目标已授权且可发送也不能替代逐账号 membership。
...
创建频道互动任务
  ...
  -> 直接保存草稿或创建并启动
  -> 解析频道目标
  -> 读取目标能力与每个任务账号的 membership
  -> 账号已有 typed membership：该账号进入浏览 / 点赞 / 评论 / 回复主互动阶段
  -> 账号缺少 typed membership：生成 ensure_channel_membership 前置动作
  -> 按抖动和限速执行关注频道
  -> 逐账号汇总关注结果
  -> 仅已确认关注的账号进入浏览 / 点赞 / 评论 / 回复主互动阶段
  -> 任务详情持续展示前置阶段和主阶段结果
```
...
已授权且可发送的频道仍必须逐账号检查 membership；只有当前账号已经存在可信 membership 时才不生成该账号的前置关注动作。目标级授权、发送能力、Session 存在和 peer id 形式均不能代替账号级事实。
