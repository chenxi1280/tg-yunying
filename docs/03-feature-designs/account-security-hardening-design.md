...
| 策略 | 说明 | 默认 |
| --- | --- | --- |
| 平台统一托管密码 | 多个账号使用同一平台托管 2FA 密码，便于自动补齐 standby session；必须加密保存、限制查看和导出 | 默认 |
| 系统生成每账号唯一密码 | 每个账号生成不同强密码，加密托管，支持一次性导出 | 可选 |
...
- 接码专用账号在登录后自动资料初始化、账号面具初始化、批量资料预检、设置 2FA 预检、设备清理预检和 `account-security` worker 执行前都必须硬拦截；拦截状态写 `code_receiver_reserved` 或等价可读原因。备用 session 补齐 / 自愈不属于该禁用动作集合，仍可执行。
...
- 接码专用账号在设置 2FA 或轮换托管 2FA 预检中必须跳过，worker 执行前必须二次拦截；备用 session 补齐 / 自愈仍可使用已托管 2FA 完成登录。
