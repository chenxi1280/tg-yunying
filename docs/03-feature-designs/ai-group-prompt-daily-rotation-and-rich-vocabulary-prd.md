...
> **2026-09-08 二轮对照修订（设计态）：** 当前unified内容能力继承见统一引擎 §19.63.4；保留话题比例、每日词库、teacher及逐字段生效，不恢复旧数量owner。§19.13–19.14 已取消的完整画像/调用预算编排不作为当前启动前置；旧reset不适用于unified。应急内容独立统计且不扩大normal分母，合法未发内容reservation由原owner CAS交接释放。
