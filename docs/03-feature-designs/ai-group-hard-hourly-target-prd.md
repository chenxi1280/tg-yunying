...
- `allow_account_repeat` 临时视为 `true`，但 Planner 必须先保持账号轮转和可用账号扩散；只有本轮缺口超过可用账号数、连发配置明确要求或账号池只剩少数可用账号时，才允许同账号在同轮重复。
- 发送计划时间压缩到当前小时剩余窗口。
...
- 已跳过的准入失败如果错误详情包含必需频道链接或“需要关注频道”提示，硬目标模式必须按重试周期推进**前置** `group_bot_channel_follow` / 群管准入状态机与 `can_send` 复检；不能把这类账号永久留在失败池，也不得用正文发送试探。
