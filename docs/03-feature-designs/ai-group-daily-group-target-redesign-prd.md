...
保持冻结日目标、独立账号覆盖与数量槽。签到应急可完成原合法数量/覆盖，但不得冲抵普通主题和真人互动质量；当前来源排期在真实 last-call、原 release/deadline、冻结账号窗口及有效未来预约之间选最早合法空隙。 具体实现、开关、测试与发布证据以[AI活群可执行供给、主题与故障兜底修复](ai-group-executable-supply-and-fallback-20260910-prd.md)为准；本条为本切片 `resync`，不改写历史验收记录，也不扩展评论路径。

> **2026-09-10预关注事务修正：** 并行关注使用不可变快照，远端等待前结束数据库事务；返回后刷新认领、生命周期与账号可用性，成功关注事实不因认领变化丢失，旧调用不继续正文。范围和并发验收见`cpu-memory-hotpath-repair-20260910-prd.md`第三轮，原配置数量、并行方式与全部成功条件不变。
