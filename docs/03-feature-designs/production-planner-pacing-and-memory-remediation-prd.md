...
| 设计状态 | product_design_complete / dev_handoff_ready=true / resynced_2026-08-19-18 |
| 实现状态 | `faa3e3ae` 已发布且旧日 no-fact/no-Gateway 集合 3,468 条已按 hash 安全结案；继续线上读回发现 RC-P13/P14：恢复调用方丢弃已取消 source state id，账号放弃路径也只 skip Action，终态 reservation 与 current admission 共同把来源 cursor 冻结到 8 月 22 日。进入下一修复 train |
| 生产状态 | partial：旧日 stale 安全事实链、Release Gate、健康和短窗 Planner 资源均通过；来源仍有 242 条 `account_task_abandoned` 无事实 reservation、29 条已有安全事实但 admission 未取消，以及 34 条 current admission 被旧队尾推迟，尚无新日自然 Gateway，production_fixed=false |
| 当前生产基线 | 2026-08-19 01:49 北京时间；release `faa3e3ae`、migration 0154、关键容器 healthy；Planner PSS p95 约 191 MiB、MemAvailable 约 2.0 GiB，但 SwapUsed 约 660 MiB。浏览来源 cursor 分别停在 8 月 22 日 10:37 与当日 05:44，首个业务阻断是未收口的 source reservation，不是 worker 健康 |
...
| RC-P12 | 历史修复 scope 把 fact/Gateway/owner/reservation 的相关 `EXISTS` 在 5 个聚合列中重复展开；生产单 Task preview 超过 60 秒，断开 SSH 后数据库查询仍继续到 280 秒 | 先以 Task/date/current contract 构造 `stale_actions` CTE，再把 fact、Gateway、safe owner、safe reservation 各自按 action_id 去重成集合 CTE，一次 outer join 聚合 5 个计数；生成 SQL 禁止相关 `EXISTS`，preview 仍须输出与候选 manifest 相同的 blocked 语义 |
| RC-P13 | safe settlement 返回已取消的 source state id，但历史恢复调用方丢弃返回值；游标仍保留已取消队尾，且重算剩余 reservation 时未加该 admission 的冻结 gap | 游标重算成为公开事务入口；所有调用方汇总 state id 后在提交前调用。next cursor 取 `last real Gateway + last gap` 与每个 remaining reserved `call_not_before + frozen gap` 的最大值，禁止同槽碰撞 |
| RC-P14 | fact-first 账号放弃只执行 `_skip`，没有 typed fact、missed account reservation、cancelled source admission 与 cursor reconcile；存量终态预约及其后 current admission 被一同推到未来 | current `view_message/like_message` pending sibling 必须复用 no-Gateway safe settlement 并在同事务重算 cursor。存量只接受 exact Task/date/reason/release/hash：无 fact 且无任意 Gateway 的行补安全事实链；已有 `safely_not_executed` 的行只取消其 no-Gateway admission、miss reservation、释放非 confirmed current owner，不复制事实；随后按 source row lock、`max(now,last Gateway+相邻 gap,planned release,account due)` 稳定排序重排 current pending admission，并同步 Action/owner/account reservation，任何 deadline 越界、unknown、confirmed 或 manifest 漂移均阻断 |
