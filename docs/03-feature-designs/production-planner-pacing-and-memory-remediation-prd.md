...
### 10.4 2026-08-19 最终生产读回

| 验收层 | 证据 | 结论 |
| --- | --- | --- |
| 发布与运行 | `32185684995` 发布 `f49353fd` 成功，no-PostgreSQL/PostgreSQL/前端/镜像/部署全部通过。current/backend/Planner/Dispatcher/Listener/Recovery 均为该 SHA，migration 0154 head、healthy/restart=0/OOM=false；激活后至 05:25 Planner/Listener/双 Dispatcher 的 deadlock/Traceback/ERROR/CRITICAL 匹配均为 0 | pass |
...
| 发送目标守恒 | v2 manifest 已把 task/ledger/owner/account、OperationTarget peer/revision、ChannelMessage remote id、payload peer/message/quantity 与 daily/total/effective/due/source 全部纳入锁内前后指纹；15 项漂移回归通过。最终 SHA 后 33 条真实群发送的 Action chat peer、TgGroup、OperationTarget、冻结快照、Task config、daily target 与 content scope mismatch=0；旧 unknown Action 保持 closed_unknown/3 attempts/1 admission/0 新 fact | pass |
