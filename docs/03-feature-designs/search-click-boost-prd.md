...
> **直连适配合同（待实现）：** 搜索点击与排名观察按[当前 SV 授权直连设计](search-direct-current-authorization-20260911-prd.md)实施版本化迁移；旧代理合同不能借空代理字段通过。履约数量、原范围和 unknown 继续保留。
...
准入子 action 的 `join_request_pending` 不是失败成功混淆：它保留 source 的目标命中事实并使其 `join_status=membership_pending`。待审批回执不得写入 `membership_observed` 或 `membership_observed_at`；即使历史回执带有这两个矛盾字段，事实核算也必须以 `join_request_pending` / `membership_pending` 为未确认，下一次复核自动清除脏字段。同一 source 的 child 仍只进行成员关系复核，不能在 retry 中重复提交相同申请，并且必须固定使用 source 的同一账号和授权槽位，不能因全局账号容量被转派到其他账号；每次调度前和运营重试前都要把历史错绑 child 恢复为 source 账号后再复核。Dispatcher 对已到期 child 的 claim 优先级位于既有目标准入重试和 AI 硬目标之后、普通批量 action 之前，保证“已点击 → 申请/复核”链路能收口而不绕过风控。`allow_same_account_repeat_application=true` 只允许新的 source 使用同一账号在当天继续搜索并进入各自 child，不能改变既有 child 的 source 归属；新的 source 也不能被既有 pending、账号日限额或关键词日限额静默阻断。每个 child 的 Telegram 回执都必须如实落库，不能把“已申请”“待审批”或拒绝写成已加入。

`max_actions_per_day` 是 source action 的基础日预算，必须不小于 `daily_click_target_count`；旧任务未设置点击目标时继续按 `daily_target_count` 的历史校验。对于 `strict_daily_target=true` 的每日点击目标，每条当天已终态、但没有 `target_click_observed/target_found_at` 的 source 都追加一条 replacement source 预算；因此基础预算不会因“未命中目标”而提前耗尽、把已确认点击目标卡在不足值。`pending`、`claiming`、`executing` 和 `unknown_after_send` 继续占点击槽位，不产生 replacement，避免同一未知结果被重复放大。replacement 仍受截止时间、静默时段、账号/授权槽位绑定、全局容量、Gateway 风控及真实 Telegram 回执约束，不能伪造点击或成员关系成功。当天点击目标达成后 Planner 停止新的 source，但已经创建的 child 继续按真实结果收口；任务保持 `running`，下一个自然日重新计算。旧 `search_join_group.target_count` 任务保持原有生命周期完成语义；`search_rank_deboost.target_count` 同样继续是生命周期 confirmed click 目标，达到后才可写 `target_count_reached`。

### 4.1.2 严格每日目标领取顺序（2026-07-23）

当严格 `search_join_group` 的日点击目标未达成时，Dispatcher 到期领取顺序固定为：既有 `target_admission_retry`、`search_join_membership` child、严格 `search_join` source、AI 硬小时 action、普通批量 action。领取顺序不放宽账号/代理/授权槽位绑定、静默时段、截止时间或 Gateway 风控；source 基础预算只会因当天终态未命中目标而逐条增加 replacement budget，未知结果仍占槽位。它避免持续产生的 AI 硬目标使已到期的搜索点击或待审批成员复核长期得不到执行。
...
| `max_actions_per_day` | 必填，正整数 | 任务自然日内的 source action 基础预算；普通搜索必须不小于 `daily_click_target_count`。严格每日点击目标的当天终态未命中 source 会逐条追加 replacement budget，达到已确认点击目标后才停止新的 source。 |
...
`daily_target_count` 的完成语义：`search_join_group` 只统计任务时区当天 `Action.status=success` 的 `membership_observed`；`pending`、`claiming`、`executing` 和 `unknown_after_send` 在当天占用剩余槽位，`failed`、`skipped` 释放槽位。点击目标同样以 `target_found_at` 计 confirmed；在前一自然日已排程但尚未进入 Gateway 的 source `pending` / `claiming` / `executing` 会作为次日点击 target 和 source 基础预算的 carryover，避免忽略存量工作后重复补量。达到每天目标时写 `stats.search_click_target.state=daily_target_met`，但任务保持 `running`，次日重置。新建简化每日目标任务写入 `strict_daily_target=true` 与 `skip_probability_per_action=0`：可用 jitter 分散执行时间，但不得用隐藏的行为随机跳过侵蚀已校验的日容量。严格模式的 source 在 Dispatcher 中位于既有目标准入重试、AI 硬目标和 `search_join_membership` child 之后、普通批量 action 之前；若只有最旧 pending 年龄触发 Planner 背压且全局/单任务硬阈值未触发，可在剩余点击槽位内继续补量；当天 `success` / `failed` 但没有 `target_found_at` 的 source 各追加一条 replacement budget，未知结果继续占槽位。存量任务或高级任务只有在运营专用编辑中明确启用“严格每日目标”时才切换到该模式；未启用时保留其已保存的行为节奏。代理、账号资格、风控和第三方检索失败仍按真实 `skipped` / `failed` 事实记录。`search_rank_deboost.target_count` 与已有 `search_join_group.target_count` 仍以生命周期 confirmed 事实计数，达到目标后才写入 `status=completed`、`next_run_at=null` 和 `stats.completion_reason=target_count_reached`。未带两种目标字段的历史任务保持原有不封顶调度语义。
