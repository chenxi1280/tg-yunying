...
搜索 open obligation 在账号尚未求解前使用中央虚拟 source shard `(account_shard_total=1, account_shard_index=0)` 获配，不能把该虚拟 ShardAllocation 当成四个执行 worker 的归属。预绑定 plan 必须按求解后固化的 `assignment.account_id % runtime_account_shard_total` 只向唯一执行 Dispatcher 暴露 candidate，同时继续用原虚拟 ShardAllocation/Reservation 做中央 claim 计数与锁定。非归属 Dispatcher 看见同一 Action 时只从本 worker plan 排除，不能写 Action、不能释放 assignment、不能生成 exclusion；归属 worker 锁定后才按账号安全策略和实时资源继续 confirm。这样执行 shard 是并行路由，不是失败原因，也不会因中央 `(1,0)` carrier 令四个 worker 全部跳过。
...
| 普通 Dispatcher 先于搜索 Planner 创建新 Window | 首次 scope-wide allocation 输入仍包含 open search click obligation，并持久化 search TaskAllocation/Reservation；后续通用 unclaimed reconciliation 识别纯搜索专属 carrier，不能因尚无预绑定 Action、任务随后停止或 due 消失而释放该份额；搜索 Planner/恢复流程在原 Window 内创建唯一 epoch并收口 |
| 搜索 Planner 先于普通 Dispatcher 创建新 Window | 首次 scope-wide allocation 对普通 Action 仍使用生产 `runtime_account_shard_total` 和 `account_id % total`，只对账号尚未求解的 search source 使用虚拟 `(1,0)`；后续普通 Dispatcher 能在同一 epoch 找到自身 shard Reservation |
