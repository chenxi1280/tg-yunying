...
source Action 在当前 Claim Window 的 commit 事务末尾绑定其任务日 ledger、`click_obligation_ordinal`、assignment 和中央 Reservation；projection 绝不创建 Action。Gateway 不得在 `deadline_at` 后开始。Gateway 已开始但事实在 deadline 后确认时仍归原 ledger并记 late。`source_click_action_id` 只表示本次执行来源，不参与 click 义务唯一键。
