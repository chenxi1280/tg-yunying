...
> **2026-09-08 内容应急导航：** [统一引擎 PRD](unified-engagement-fulfillment-engine-prd.md) §19.61 是活群签到、回复/评论随机表情的唯一current emergency合同，§19.62为旧PRD能力继承矩阵；关联文件顶部的resync说明覆盖历史冲突，不代表已实现。异步调度/准入仍见§19.60及频道成员设计§15；二轮旧能力、轻量范围、统计/清理和配置生效审计见§19.63。
...
| [antigravity-cli-server-provider-design.md](antigravity-cli-server-provider-design.md) | **2026-09-03 design resync / implementation / local QA complete / production blocked**：生产目录移除 3.5 后，slot-01 冻结为 3.6 Flash Medium → 3.1 Pro Low → 原 Provider；隔离 OAuth/bridge、durable request reconcile、unknown 不重放、原子 route cutover 与生产 Gateway canary 合同保持不变；第 17 节新 release 双模型探针未通过前不得声明线上可用 |
| [runtime-storage-retention-and-reclaim-prd.md](runtime-storage-retention-and-reclaim-prd.md) | **历史本地实现/PG QA，2026-09-08新补正未实施**：分状态TTL与受保护清理；新增统一§19.63.3的72小时统计查询依赖、业务FK/owner/事实保护，旧测试不证明新条件通过 |
...
| [ai-group-prompt-daily-rotation-and-rich-vocabulary-prd.md](ai-group-prompt-daily-rotation-and-rich-vocabulary-prd.md) | **v1.2设计及后续局部本地实现；统一继承补正未实施、未发布/E4**：`AiGroupContentAllocationPlan` 唯一持有任务话题 `0～30%` 上限，配置无静默默认，少用话题不影响数量/coverage；当前内容owner/配置生效与应急分母见统一§19.63.4；`daily_vocabulary_theme_id` 按群可见面独立轮换，general/adult 各 120+ theme/route/act-type/stance-aware 词库，跨 Task/账号冷却与 reservation；配套低量预计、teacher 独立比例、连续问句/表面词频、成本吞吐和七日 remote-confirmed E4 合同 |
...
| [task-fulfillment-classified-recovery-prd.md](task-fulfillment-classified-recovery-prd.md) | **分类型参考**：C1-C8、C2观察、独立搜索与Recovery；unified冻结账号分母/轻量运行/应急按统一§11、§19.13、§19.61、§19.63，§19.64先排除失效账号，新分配即时校验，历史分母不抹除 |
| [task-fulfillment-contract-closure-prd.md](task-fulfillment-contract-closure-prd.md) | **分类型闭合参考**：唯一事实/CAS、搜索phase、C2-C8、显式用户删除和E4；unified切换只按统一§19.58退役保留映射，持续调度/应急/轻量范围见§19.60–19.63，旧prepared/物理删除发布链不适用 |
...
| [ai-group-daily-group-target-redesign-prd.md](ai-group-daily-group-target-redesign-prd.md) | **按route分流**：legacy动态资格仅供原身份；unified按§19.64有效账号先过滤后冻结selected，按群日数量及§19.61应急计基础覆盖，内容继承/新旧差异见§19.63 |
...
| [hourly-random-pacing-and-ai-humanization-prd.md](hourly-random-pacing-and-ai-humanization-prd.md) | **v8 本地第一阶段实现并通过定向 QA、待生产证明**：四类 stable due、跨 Task source capacity、curve-aware pairwise gap、replacement headroom、逐 slot 守恒与 typed shortfall；禁止无计划0→1；合法current应急按统一§19.61，历史静态绝对禁令不适用 |
...
| [ai-content-routing-and-quality-upgrade-runtime-contract.md](ai-content-routing-and-quality-upgrade-runtime-contract.md) | **v1.2运行附录及历史本地实现**：Provider purpose/priority、selector迁移、状态机/API/UI；当前轻量运行与应急以统一§19.13–19.15、§19.61、§19.63为准，新切片未实施 |
