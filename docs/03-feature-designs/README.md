...
| [search-dispatch-release-fence-hotfix.md](search-dispatch-release-fence-hotfix.md) | **2026-08-19 L3 热修实现完成、26 项定向 QA 通过、待发布**：Stage B 同时收口共享 claim 与无共享标记的 current 纯搜索 executing Action；pre-Gateway 保留同一 Action 重领，Gateway-started 转 unknown remote case 与 `gateway_unknown` assignment，目标与防重身份不变 |
