...
| [account-profile-group-style-initialization-prd.md](account-profile-group-style-initialization-prd.md) | **2026-08-18 Product Design Complete、待实现**：按精确批量登录 batch 初始化名字+头像，群样本仅形成匿名风格分布、不复制真实身份或头像，protected preview/apply/readback + 300 账号 E4 |
...
| [production-planner-pacing-and-memory-remediation-prd.md](production-planner-pacing-and-memory-remediation-prd.md) | **2026-08-18 本地实现与 PostgreSQL QA 通过、待发布**：Planner 有界投影/唤醒、Listener 版本化空快照、四类跨批 source cursor + Gateway 前 fail-closed admission、PSS/cgroup 自采样及 24h 原始/7d rollup；`production_fixed=unproven`，必须继续取得生产资源 E3、排期 E3 与任务 E4 |
