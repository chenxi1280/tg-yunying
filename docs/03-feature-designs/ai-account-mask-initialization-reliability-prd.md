...
> **2026-09-08 用户裁决 / emergency resync（设计完成，尚未实施）：** 面具初始化/修复状态机保持；内容应急以统一引擎 §19.61为准，固定签到/表情不依赖面具生成，允许所有仍合法的原数量义务应急；缺面具不等于账号身份禁用。旧“仅首条coverage/extra-volume必须面具”的内容资格不用于新emergency。 本切片代码/生产状态仍为未实施、未验收；旧事件记录保留。
