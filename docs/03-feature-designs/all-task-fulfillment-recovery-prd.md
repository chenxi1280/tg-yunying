...
| 场景 | 必须证明 |
| --- | --- |
| Planner 与 4 Dispatcher 并发 | PostgreSQL 并发回归无 deadlock；通用入口严格使用 `Scope -> Window -> TaskAllocation -> ShardAllocation -> Reservation -> Action`，搜索入口在 Reservation 后固定追加 `carrier（如有） -> assignment -> consumptive 子预留 -> Action`，缺失层只跳过不换序；Gateway 返回后的 claim release/confirm 即使 Session 已有 Action、Task、账号、记忆等 dirty 对象，也必须在 `no_autoflush` 中先完整取得 Scope/Window/ShardAllocation/Reservation 前缀锁，取得前禁止 ORM 自动 flush 下游业务行；前缀锁完成后才允许 flush/重算，避免 release 持下游行再等待 Scope、allocation writer 持 Scope 再等待 Task 外键的反向环；claim 热事务无 `UPDATE tasks` |
