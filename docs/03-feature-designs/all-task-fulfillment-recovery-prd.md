...
2. **runtime retention。** 过期 runtime 只清理 terminal Action，禁止把仍承载未完成业务义务的 open Action 当作空间回收对象。候选查询只投影汇总、引用清理和删除所需列，不水合 payload/result；删除 Action 前必须删除 `TaskHardHourlyDeliveryCredit` 等从属明细，并将需长期保留的 `AiCoverageVariationIntent.action_id` 等可空引用解除，不能让任一外键使整批反复回滚；独立配置 batch size 与 interval，不能继续复用 Recovery 的通用 `limit=100`。生产默认每分钟处理最多 2,000 个 terminal Action，每批独立提交并保留 `RuntimeCleanupAudit`。
...
1. **旧槽重建不再形成任务级队头阻塞。** Planner 仍优先重建当前 ledger 的 `unmaterialized|replan_required` 槽，并且只能使用该槽冻结的 `primary_quantity_slot_id`、coverage 账号、relation 与内容义务。旧槽因账号、群管准入或合法引用对象暂不可用而本轮 `created=0` 时，只保留该旧槽等待；随后必须重建独立周期蓝图。普通 coverage keyset 必须排除所有仍被 ContentMix 占用的 coverage 账号，包括旧 Cycle 仍为 `pending` 但原 Action 已失败并释放 coverage 的假 ready 行，令其他尚未被任何 Cycle 占用的 open 数量槽继续形成独立新 Cycle。禁止复用旧槽主数量位，也禁止因为一个旧槽等待就让数百个独立 ready coverage 停止建 Action。
...
1. Planner 锁定当前 target-day 的 cycle cursor，选择尚未绑定 Cycle 的 AI 主发送槽，并在一个短事务中创建 Cycle、全部 CycleSlot、`ContentMixContract` 和 `policy_min` 义务；唯一键为 `(task_id, target_operation_target_id, task_day_ledger_id, cycle_seq)`，同一 `primary_quantity_slot_id` 只能绑定一个 CycleSlot。每个账号在本轮首次 coverage 候选只能绑定 `task_account_daily_coverage_id` 与该账号 coverage 完全相同的主发送槽；该 coverage 槽已在本轮分配后，同账号后续发送才按 extra-volume 候选绑定 coverage 为空的槽。找不到精确槽时本轮等待，禁止借用另一账号的 coverage 槽或让 Action coverage 与 `primary_quantity_slot_id` 身份分裂。
...
4. pre-Gateway Action 明确终态且逻辑槽可恢复时，先以 CycleSlot 行锁/CAS 写 `slot_state=replan_required`，释放的只是旧 Action 的 runtime claim/发送占位，不释放 `primary_quantity_slot_id`、coverage 义务、关系槽或内容义务；再原子递增 `slot_attempt`、创建新的 `(cycle_slot_id, slot_attempt)` Action 并替换 `current_action_id`。reply 槽可在新 attempt 选择另一个合法引用对象，但 `relation_kind=reply` 不变，旧 Action 的 `reply_to_message_id` 永不改写。历史 terminal Action 保留。重建时只从旧 Action 解析并校验 Cycle/规则/素材合同元数据，再与本轮新生成的可发送 payload 合并后执行完整校验；`reply_target_stale` 等失败 Action 允许空正文，不能先套用“可发送正文非空”校验而阻断重建。Dispatcher 在 Gateway 前必须核对 Action `coverage_ledger_id` 与 `primary_quantity_slot_id.task_account_daily_coverage_id`；不一致时以 `content_mix_quantity_coverage_mismatch` 明确跳过旧 Action、释放错误 coverage reservation，并把其 CycleSlot 置回 `replan_required`，不能继续发送或把错绑事实确认为另一个账号。finally 收口只能以冻结的 `CycleSlot.primary_quantity_slot_id` 解析原数量槽，不能信任已判坏的 Action 字段；冻结链缺失或跨 Task/ledger 损坏时保留显式 skip 结果且禁止跨行写状态，不能因清理异常回滚后重复领取。Gateway-started/unknown/success 禁止创建替代。Planner 对旧 Action 的 takeover/retry/预检变更必须先提交并释放 Action 行锁，再重新加载仍为 running 的 Task，进入不再持有旧 Action 行锁的新短事务；禁止同一事务先锁 Action、再插入带 `OperationTarget` 外键的 Cycle/Action，与 Dispatcher 的 `OperationTarget→Action` 锁序形成反向环。Planner 每轮必须先扫描同一 Task/target-day 的 `unmaterialized|replan_required` CycleSlot；`unmaterialized` 合法地没有旧 Action，必须从冻结的 CycleSlot、主发送槽绑定 coverage 账号和本轮合法 payload 直接创建 attempt 1，不能要求 inner join 旧 Action。只要仍存在待物化/重建槽，本轮就不得因账号或引用对象暂不可用而另建 Cycle、继续占用其他主发送槽；可执行对象恢复后仍在原槽物化。旧 Cycle 已无待物化/重建槽时，才允许为其他未绑定主发送槽创建新 Cycle；两个 Cycle 不得共享 `primary_quantity_slot_id`。
5. Planner 清理旧 AI Action 时只允许选择未被领取的 `pending|retryable_failed` 候选，并使用 `FOR UPDATE OF actions SKIP LOCKED`；`claiming|executing` 只由 Dispatcher/Recovery 收口，Planner 禁止接管或用较早读取的 ORM 对象覆盖其状态和 claim token。
6. 配置修改不改写已分配关闭的 Cycle。新配置只用于生效后新建的 Cycle；旧 Cycle 的 Action、Attempt、远端事实和 content mix 继续按原 `config_revision` 收口。settled Cycle 完全只读且禁止原地 retry；若产品另有明确的人工恢复操作，必须建立独立 recovery revision，不能重开旧 Cycle、改写旧日 outcome 或再次消费旧主发送槽。
7. deadline 到达时，尚未物化、`replan_required` 或未进 Gateway 的 CycleSlot 以稳定 slot 顺序写明确 `deadline_reached_unmaterialized|deadline_reached_before_gateway`，对应数量/内容 shortfall；Gateway-started/unknown 继续保持 hold，不得为了结算强制终态。
8. Cycle 只有在所有 CycleSlot 的 `slot_state` 均为 `confirmed|terminal`，且不存在 pending/gateway_started/unknown/replan_required、全部 ContentMixObligation 已完成或形成明确 shortfall 后，才由幂等 reconciliation 写 `settlement_status=settled`、`settlement_outcome` 与 `settled_at`。unknown 在核验完成前保持 open；deadline 只改变履约结论，不允许删除 Cycle、重开分母或阻塞其他独立 Cycle。
...
同一 `SearchClickAssignmentEpoch` 为某账号创建的全部 prebound Action 是求解器一次性承诺的同一账号容量集合。claim-time 账号全局容量/冷却复检必须把该 epoch、该账号的全部 Action ID 作为同一 cohort 从 occupied scan 中排除，再让账号 in-flight/Redis 互斥只放行一个真实执行；不得只排除当前 Action，使同批 pending Action 互相制造 `account_cooldown|account_hour_limit|account_day_limit`。其他 epoch、其他任务和既有 MessageTask/Action 仍正常计入硬安全容量；cohort 总量不得超过该 epoch 快照中的 `hard_safe_remaining_capacity`。
