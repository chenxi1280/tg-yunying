...
- D 的预绑定 Action 只属于原 Window；plan 先按原 ShardAllocation 路由给唯一匹配 Dispatcher，非归属 worker 只跳过且不得释放。归属 worker 内的安全账号策略、绑定 shard 失效、运行资源、CAS 或 Window 结束导致当前分片不可执行时，才用真实原因释放原 unit并重建权重，不延后原 Action、不回落普通 claim、不改绑新 Reservation。release 与 confirm 使用同一中央锁序；release 事务必须在计算 release/outcome hash 前显式 flush batch item 与 unit exclusion，QA 必须用生产一致的 `autoflush=false` 会话验收完整原子事实。
