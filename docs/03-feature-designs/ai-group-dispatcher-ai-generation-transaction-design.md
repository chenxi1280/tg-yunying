# AI 活跃群 Dispatcher AI 生成事务边界专项设计

- 日期：2026-07-13
- 等级：L3
- 状态：`design_status=complete`、`resync=true`
- 范围：AI 活跃群 reply / normal 文本生成、质量过滤、覆盖预约与真实发送；不改变评论任务。
- 触发证据：独立 QA I1 证明 Planner 在锁定 Task、任务日游标和 coverage 后，仍通过 quality -> `generate_group` / `generate_reply` -> Grok 同步外呼。
...
- 执行无需外部网络的内容清洗、指纹、语义簇、内容政策、账号面具和 DB 记忆重复检查；
...
- AI provider/fallback 最终失败或质量最终拒绝时，Action 以明确错误终结并在同一事务释放 coverage；后续由 Planner 为仍未完成义务创建新 Cycle，不把失败 Action 伪装为成功。
- Telegram Gateway 调用后结果不明继续使用 `unknown_after_send`，保留 coverage unknown 且禁止自动重发；AI 生成未知和 Telegram 发送未知必须分开统计。

## 6. 可观测状态

`Action.payload` 保存编排快照、`ai_generation_status`、lease/attempt/request id 和生成历史，`Action.result` 保存终态失败阶段；不新增另一份 coverage 或成功事实。任务详情和运行日志至少区分 `generation_pending`、`generation_claimed`、`generation_ready`、`generation_failed`、`quality_rejected`、`reply_target_stale`、`gateway_unknown`。每次 generation attempt 记录 action、cycle/slot、provider/model、开始/结束时间、outcome、失败阶段和 lease owner；不得把 provider 成功等同于 Action 或 coverage 成功。
...
E4 仍要求发布后连续至少 3 个完整 planner/dispatcher/metrics cycles 无 `>60s` transaction、无持续 `>5s` lock waiter，覆盖从 `394/2320` 连续增长，并最终以完整北京时间自然日 2320 条 Telegram 远端成功矩阵验收。worker healthy、AI provider success 或 pending Action 增长都不能替代 E4。

Phase A/B/C 必须同一 release 启用，禁止 Planner 外呼兼容分支或失败后回退 Planner 同步生成。应用回滚保留 Action、coverage、游标和生成审计数据；回滚到旧 Planner 外呼路径只表示代码回退，不代表生产恢复，Release Gate 继续 blocked。

## 9. Product Design Complete

原始问题、三阶段职责、reply/normal 时效、pending 原子性、slot 映射、质量失败释放、生成/发送未知分层、并发幂等、事务指标、580 分母、`messages_per_round`、迁移/回滚和 E2/E4 均已定义；`design_status=complete`、`resync=true`。进入 dev 前以本文件覆盖此前“Planner 可预生成 reply 文本”及“只提交 Grok 前事务”的旧口径。
