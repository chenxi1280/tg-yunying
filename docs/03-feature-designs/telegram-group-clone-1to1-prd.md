...
### 2026-09-09 共享更新消费者锁范围修订

4d947aa8上线后仍有共享state持锁200秒的真实事务。反向检查区分第二处来源：AI/评论消费者的Delivery/Event/Subscription/AuthorizationState关联查询使用未限定表的FOR UPDATE，消费业务查询期间连只读共享state也被锁住；Clone的关联消费查询也会锁住只读Event/Subscription。前述订阅准备顺序修复只覆盖准备阶段，不能替代此消费阶段修复。
