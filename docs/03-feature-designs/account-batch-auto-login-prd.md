...
> **2026-08-28 `phone_masked` 身份误判 quick_fix Mini Bug Card：** 生产批次 #13 在新号 `bind_account` 阶段出现两个不同 E.164 手机号生成相同脱敏展示值，遗留的 `ux_tg_accounts_tenant_phone_active(tenant_id, phone_masked)` 因而抛出唯一约束冲突并反复打断 worker drain。本文既有合同已明确手机号身份只由租户级 keyed fingerprint alias 判定，`phone_masked` 仅用于展示，因此该唯一索引属于旧身份语义残留。修复边界为删除 `phone_masked` 唯一身份约束、保留同字段的非唯一查询索引；批量绑定继续先锁 fingerprint、查 alias、创建账号并原子写 alias，不能按脱敏值复用账号。迁移只改变索引、不重写账号、手机号密文、alias、Session 或批次事实；回滚若已出现活跃脱敏值碰撞必须显式阻断，不能静默恢复旧唯一索引。验收要求：同租户两个不同完整手机号即使脱敏值相同也创建为两个账号和两条不同 alias；相同完整手机号仍由 alias 唯一约束阻断；生产迁移读回旧唯一索引消失、新非唯一索引存在；受阻行从原 generation 续跑，不创建重复批次。`design_status=product_design_complete`，进入 L3 dev quick_fix。

> **2026-08-28 Telegram 登录调用租约 quick_fix Mini Bug Card：** 生产批次 #13 的单行 `send_code` 在 Telegram 数据中心迁移后的重连阶段超过 90 秒 claim lease，而通用 Telethon operation timeout 仍为 300 秒；worker 主循环先释放该轮，随后把已持久化 `started` 的调用转为 `login_remote_unknown`，原协程直到 300 秒才取消。日志与持久状态均未形成临时 Session、`phone_code_hash` 或授权事实，但该状态仍必须按 unknown 处理，禁止自动重发。修复边界仅为 code 登录 `start_login/finish_login` 的同步等待固定不超过 60 秒，严格小于 claim lease；超时继续进入既有 `remote_unknown -> reconcile`，不改 API、重试 CAS、批次计数、接码频控或成功口径。验收要求：start/finish 均显式使用 60 秒边界且小于 90 秒租约；超时协程被取消，worker 在租约内持久化 unknown；已有未解行只能在完成一次对账后携带 attempt/version/resolution 三重围栏和 `confirm_remote_unknown=true` 单行恢复；最终仍以 Session、current healthy authorization 与 online readback 为登录成功。`design_status=product_design_complete`，进入 dev quick_fix。
...
运营侧持续新增 TG 账号。当前只能逐个打开登录弹窗、手动等待验证码、手动输入 2FA 密码，账号一多无法运维。运营使用的第三方接码平台（如 tgbotchecker.com、tgapi.susubot.com、api.config2.top）为每个号码提供只读取码入口，展示或返回该号码的 Telegram 登录验证码和 2FA 密码。
...
- 每行格式：`电话号码|接码链接`，示例：
  ...
  +12025550125|https://api.config2.top/tgapi/tgapi/<uuid>/GetHTML
  ```
...
2. **多行数据输入**：`Input.TextArea rows=12`，placeholder：
   ...
   - phone 规范：`+` 开头 E.164；url 必须为 `https://` 且主机/路径/参数在接码平台白名单（当前 `tgbotchecker.com/GetHTML?uuid=<32位uuid>`、`tgapi.susubot.com/index.html?type=107&apikey=<uuid>` 与 `api.config2.top/tgapi/tgapi/<uuid>/GetHTML`，见 8.4）。
...
8. 调度/限速：多租户、多长短批次、多 worker 并发下仍一批一 phase 公平轮转；host/开发者应用持久 rate bucket 不超额、无饥饿。
   - host bucket 的 `scope_id` 必须等于行项真实平台标签；config2 的有效最小间隔为 `max(部署配置, 130 秒)`，确保 baseline 与 challenge 后首次轮询之间留出供应方冷却窗口。tgbotchecker/susubot 不继承该平台专属间隔。
...
- 2026-08-28 config2 空 baseline 与生产频控增量已覆盖原始新地址/HTTPS/重新登录/日志/登录测试诉求，前端 URL 形状与权限不变，后端增加 host-scoped parser 与真实 host rate policy，数据模型/迁移/API schema/远端调用 fence 不变；失败、超时、敏感数据、发布和真实 E4 均有验收口径。`design_status=product_design_complete`、`implementation_status=local_implemented`、`qa_status=qa_pass`、`product_status=product_accepted`、`release_status=second_release_gate_pending`、`production_status=first_attempt_failed_after_send_second_failed_at_baseline`；77 项批量登录相关回归通过。首版 `bfc4113f` 已发布并越过原 `url_error`，但 generation 2 send confirmed 后频控失败、generation 3 baseline 频控失败，尚无授权 session，不能写登录通过。

## 17. 备注

- 接码平台：`tgbotchecker.com`、`tgapi.susubot.com`、`api.config2.top`。
- 地址模板：`https://tgbotchecker.com/GetHTML?uuid=<32位uuid>`；`https://tgapi.susubot.com/index.html?type=107&apikey=<uuid>`；`https://api.config2.top/tgapi/tgapi/<uuid>/GetHTML`。
- **账号映射要求**：每一行实际 UUID 都必须在运行时加密绑定到最终账号，并在账号页显示独立接码备注；不能只把 host/模板记在文档，也不能因登录完成、账号改名或 item URL 到期而丢失映射。
- 接码备注默认格式：`tgbotchecker · <UUID前6位>…<UUID后4位>`、`susubot · <UUID前8位>…<UUID后4位>` 或 `config2 · <UUID前8位>…<UUID后4位>`。具备 `accounts.code_source_credentials.read` 的同租户用户可经显式 reveal 查看完整 UUID；本文不硬编码任何账号的真实 UUID。
