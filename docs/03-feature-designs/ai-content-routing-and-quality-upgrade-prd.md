...
- 并行 GenerationJob claim 的远端调用期间，context revision 同步、window slot 与 policy/route 冻结可以由同一 owner 合法推进 `job_version`。最终 ready、已知 Provider 失败或明确取消必须以仍相同的 `generation_owner_id + generation_lease_epoch` 读取当前 `job_version` 并做下一版单行 CAS；不得继续使用 claim 刚建立时的旧 version 而制造 `parallel_generation_job_claim_lost`。若 quality/deadline 已在生成事务内把同一 epoch 的 Job 与绑定 Action 原子写为同目标终态并清空 owner/lease，外层 settlement 只能做幂等终态读回和 slot 失效，不能把已成功的内层终结再次报成 claim lost。owner/epoch takeover、终态/绑定不一致或 CAS 竞争仍必须失败关闭。已知 429/额度耗尽不得切换另一个 key，也不得把已失败 Action 对应 Job 留在 generating；Action、Job 与 pre-Gateway slot 必须收敛到一致终态。
- 2026-08-30 生产验收补正：coverage 对应 FOP 已为 `terminal_shortfall` 时，即使兼容读模型中的 `TaskAccountDailyCoverage.state` 仍残留为 `ready`，该 coverage 也必须同时从通用 ready batch 与已绑定 content-mix 槽的 `unmaterialized|replan_required` 候选中排除，并在每日履约详情中归入显式 terminal shortfall/blocked shortfall；不得反复创建同一 coverage 的 Action 再以 `obligation_not_open` 跳过，也不得让少量终态 owner 占满 context-bound 批次、饿死其余可用 coverage。这个规则只阻止新的 pre-Gateway 物化，不重开终态义务、不改变 typed shortfall、不补发历史消息。
