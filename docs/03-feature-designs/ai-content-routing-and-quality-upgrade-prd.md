...
- 2026-08-30 生产验收补正：coverage 对应 FOP 已为 `terminal_shortfall` 时，即使兼容读模型中的 `TaskAccountDailyCoverage.state` 仍残留为 `ready`，该 coverage 也必须从 ready Planner 候选中排除，并在每日履约详情中归入显式 terminal shortfall/blocked shortfall；不得反复创建同一 coverage 的 Action 再以 `obligation_not_open` 跳过，也不得让少量终态 owner 占满 context-bound 批次、饿死其余可用 coverage。这个规则只阻止新的 pre-Gateway 物化，不重开终态义务、不改变 typed shortfall、不补发历史消息。
