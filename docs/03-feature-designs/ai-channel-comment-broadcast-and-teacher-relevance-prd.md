...
- [ ] 消息编辑 operation 保留 Gateway/unknown/confirmed 旧 assignment，只为 pre-Gateway 原 ordinal append successor 并 fence 迟到旧 worker；
- [ ] Task config revision 与 grounding revision 不混用；
- [ ] 技术批次只物化首次冻结 ordinal 的 Action，不追加 assignment 或第二个 ContentMix；
- [ ] 来源编辑不改 quantity Plan/eligible/ordinal/relation/deadline，只切未进 Gateway 内容 revision；普通 route/policy 变化不改既有 Plan；
...
- [ ] 双 source-edit worker 只提交一个 revision operation/successor 集；旧 assignment 迟到 claim 在 Gateway 前失败；
