# AI 活跃群话题、聊天对象与连发模拟设计

## 背景

运营人员需要让每个 AI 活跃群任务围绕多个可配置主题和聊天对象展开，同时让同一账号偶尔连续发送 2-4 条短消息，模拟真人补充、追问或连续表达。
...
- Web 任务详情页提供 AI 活跃群专项设置；创建 / 编辑任务表单同步支持，并以多行文本作为话题方向和聊天对象主入口。
- TG bot 只作为轻量运营入口，支持管理员选择任务、查看设置摘要、查看话题 / 聊天对象摘要和获取 Web 编辑入口；不再承担完整配置编辑。
- 执行器每轮把选中的话题方向和聊天对象写入 AI prompt、话题计划和 action payload。
...
Web 创建 / 编辑表单不要求用户手写数组或 JSON。运营人员只需要输入普通多行文本：

- 话题方向：每行一个话题，第一行权重最高，系统按行顺序写入 `weight`。
- 聊天对象：每行一个对象、称呼或描述，第一行优先级最高，系统按行顺序写入 `priority`。

运营人员可以把任务内的多行话题方向和聊天对象合称为该任务的“话题包”。当前阶段话题包随任务保存，不新增跨任务模板表；后续如需要复用到多个任务，再单独设计模板中心、版本和发布流程。
...
- 每轮根据 priority 选择一个聊天对象；若没有配置，聊天对象为空。
- AI 生成提示必须明确“围绕话题方向”和“面向聊天对象”，但不得在群聊内容中暴露系统、任务或 AI。
...
- 聊天对象：主入口为多行文本，每行一个对象，越靠前优先级越高。
- 同账号连发：支持开启/关闭、2-4 条窗口和触发概率；当 `allow_account_repeat=false` 时显示“不触发连发”的提示。
- 不得把 JSON 或数组字段作为运营设置主入口。

任务详情页必须展示 TG bot 可用状态。Bot 未配置、管理员 Chat ID 缺失、AI 活群 Bot 设置关闭或 webhook 异常时，详情页应提示当前无法通过 bot 查看轻量管理入口，并指向租户级 TG Bot 配置入口。
...
4. 选择任务后显示当前话题数、聊天对象数、连发状态、全账号日覆盖状态和“配置入口：Web 任务详情”。
5. “查看话题摘要”只展示前若干条话题方向和聊天对象，便于在移动端快速核对。
6. “打开 Web 编辑”跳转到 Web 任务中心；若服务端没有配置公网地址，必须回复“请到 Web 任务中心编辑该任务”。
7. 旧按钮或旧命令 `/ai_group_set <json>` 不得继续写任务配置，必须返回可见说明：TG bot 仅支持查看，完整配置请到 Web 任务详情编辑。

TG bot 不再创建配置会话草稿，也不再展示“确认保存”。如果数据库中存在旧草稿，用户再次发送普通文本时必须明确提示旧草稿已取消，并引导到 Web 编辑。
...
`Telegram Bot API -> POST /api/telegram-bot/webhook/{tenant_id}/{webhook_secret} -> resolve tenant -> parse/verify admin_chat_id list -> handle_group_ai_bot_update -> 任务摘要 / Web 编辑入口`
...
- TG bot 提供按钮式任务选择、设置摘要、话题摘要和 Web 编辑入口；不提供分步编辑、确认保存或 JSON 设置命令。
