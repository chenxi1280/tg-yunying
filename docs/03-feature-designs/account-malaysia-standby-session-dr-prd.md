...
> **2026-08-25 frozen-N 每批 30 项执行口径：** 用户将全量生产 runner 的正常 chunk 从最多 10 项提升为最多 30 项。非尾 chunk 必须以单次正式 SSH 调用 `run --max-accounts 30` 串行处理精确 30 个 frozen item；剩余不足 30 时才允许 1–29 个尾批。该调整只扩大一次静止边界内的账号数量，不增加 Telegram 并发：任一时刻仍只有一个账号、一个登录 operation，任何 A 漂移、unexpected device、failed/manual/unknown、SSH 不明结果或守恒异常仍在首个异常账号立即停整批，后续账号 RPC 必须为 0。不得用三次 10 项拼接后伪称一个 30 项 chunk。

> **2026-08-26 completed A/B 双失效债务的 chunk 边界：** 已通过专用异人审批/CAS 入口确认的 `manual_required/primary_and_sv_standby_unavailable` 仍是 frozen-N 中的真实人工债务，不得被算作 succeeded；但只要 A 仍满足该次确认冻结的 same current/Session/authorization+connection generation 与一致 fact delta，正常 runner 完成后必须允许原子写入 chunk-boundary stopped audit。chunk pause 与随后静止批次的 release-rebind 都必须复用 completed-A 扫描的同一 acknowledged-primary 判断，不能退回通用 active-A `primary_state` 造成“30项已成功但 batch 留在 running”或“已停批但无法绑定新 release”。A 的 current、Session、generation 或已确认 fact 边界再变化时仍须零写入拒绝并进入漂移对账。
...
- `complete_online_abc` frozen-N 控制面、账号/B/C 三重守恒、全量 manifest preview/apply、每次最多 30 项的 SSH runner，以及旧 SV C 迁移和 source-less C/MY 补建均已实现；max-30 版本待发布。新的 10 账号 canary 已取得 B/C/E4 `10/10`，逐账号 A current/healthy 与 Saved Messages 远端 ID、C 双副本和 restore probe 均已读回。v2.23 取消固定 24 小时等待，`accept` 只在当前 A 无漂移、十条发送远端 ID 完整及其余安全门槛通过时放行；该 gate 发布前仍不接受 canary、不冻结或执行全量 `N`。
...
- SSH 断连、RPC 已发而响应丢失、DB commit 结果不明或远端设备差分不唯一统一为 `reconcile_unknown`：停止全量批次，不领取当前首错后的下一账号或下一 chunk，不重发验证码、不重新登录、不创建替代设备。只能在同 operation/generation、原临时 Session 或 MY 原字节、精确远端集合、fingerprint、异人审批和 CAS 下对账收口；收口成功后从原 item 检查点继续。
- runner 的 `status|preview` 始终只读；`run --max-accounts 30` 只消费已批准的 frozen batch，输出本 chunk 精确 account/item/operation manifest、前后 A fingerprint、B/C/E4 结果、runtime/MY client/global unknown、三组 outcome 守恒和 `next_action|terminal_reason`。退出码 0 只表示本 chunk 全部达到合格终态或全量已完成，任何失败、unknown、manual、A 漂移、意外设备、runtime 冲突或执行异常必须非零。
