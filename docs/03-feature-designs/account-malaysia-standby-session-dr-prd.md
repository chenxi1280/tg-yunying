...
- B 已完成绑定验证码且同一 flow 由 Telegram 明确进入 `waiting_2FA`，但旧 runner 以 `ValueError/reconcile_unknown` 停止时，可以复用原临时 Session 提交一次租户固定 2FA。普通 legacy A 仍需账号安全快照证明 trusted Session/2FA enabled；只有当 A 已是 healthy 但仅缺持久化 UID、AuthKey 已存且与 operation expected AuthKey 精确一致、operation expected UID 非空、账号安全快照完全不存在时，才允许以 operation 冻结的 expected UID 作为 partial identity fence。preview 必须冻结安全快照缺失、A identity 缺失形状、flow/code/effect、租户固定密码版本和发布 SHA；apply 只对同一 flow 进行 authorized probe，明确未授权时最多一次 password-only 提交。密码无效转 manual/no-effect，结果不明保持原 unknown；两者均不得重发 code、新建 flow/operation 或改动 A。
