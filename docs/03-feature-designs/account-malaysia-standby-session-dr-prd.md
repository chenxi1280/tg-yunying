...
> **2026-08-26 deferred recovery second phase：** 首轮完成后，`deferred_reconcile` 只能经统一二阶段 recovery sweep 重判。preview 先生成 canonical manifest hash 并要求精确 deferred count；apply 只写批次 start audit 并由 compose 管理的 ABC supervisor 自动逐项处理到该 manifest 中的 deferred 全部有二阶段 item audit。该流程不领取 pending、不触碰既有 `manual_required`、不重试登录/验证码/发码；只有 completed checkpoint 具备 A 无漂移、B/SV、C/MY、双副本、restore probe 与 Saved Messages remote ID 时才能前滚为 `succeeded`。无法证明远端结果的同一 operation 继续保留 `deferred_reconcile`，并以 typed blocker 和重判审计收口。
