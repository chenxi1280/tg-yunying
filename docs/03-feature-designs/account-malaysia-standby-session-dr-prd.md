...
- 若生产 release 在 B 已记 `login_remote_started/remote started/effect time` 但 login flow 尚未持久化时重启并杀死唯一 runner，必须把 SSH/进程结果视为 unknown，冻结同一 operation。只在 runner=0、发布已完全收敛、current release 与 batch execution release 不同、A 无漂移、该 B 为唯一 sensitive operation且没有 flow/challenge/code/candidate/lease/reconcile、B/C slots pending、C/E4 absent 时，允许专用 DB-only preview/fingerprint/异人审批/CAS 将其记为 `manual_required/reconciled_hold` 并重绑 execution release；remote effect 事实必须保留，不能冒充 no-effect、成功或授权设备，禁止重登与新 operation。
