...
> **2026-09-11 SV 冗余补齐选择补正：** B 的排除集合必须使用该账号现有 slot-current C 的实际 Developer App，不以全局 MY 默认映射替代历史 C。冻结 C 身份和版本进入 preview/fingerprint，发码前重读；保留当前 A、C 及 MY 包，按 §1 的三 App 独立合同选剩余 App。详见[继续修复记录](account-authorization-owner-and-recovery-20260910-prd.md#2026-09-11-继续修复备用-app-按现有-c-实际身份选择)。

> **2026-09-10 存量失效恢复（维护批次进行中，owner 本地实现未发布）：** 323 个 current-invalid 含 5 个已删除账号，未删除恢复范围318个；141个符合现有备用元数据谓词，8/11受合法互补物理槽误拒绝，407为历史SV standby_2特例，174个无健康备用。健康MY、retained旧SV与needs_repair均不能直接当作local_activate资格。按用户优先级先在独占维护环境恢复账号，业务流量在连接 owner 验收后恢复；按[恢复方案](account-authorization-owner-and-recovery-20260910-prd.md)逐项fresh probe/CAS/真实验收，不将无备用推断为永久封号，不把文档中的emergency流程当作已实现入口。
...
> **2026-08-26 deferred recovery second phase：** 首轮完成后，`deferred_reconcile` 只能经统一二阶段 recovery sweep 重判。preview 先生成 canonical manifest hash 并要求精确 deferred count；若 batch 的 `execution_release_sha` 落后当前已发布 runtime，preview/fingerprint/apply 在同一 CAS start 事务中只记录旧→新 execution release 并启动二阶段，不修改账号、operation、slot 或 bundle。apply 只写批次 start audit 并由 compose 管理的 ABC supervisor 自动逐项处理到该 manifest 中的 deferred 全部有二阶段 item audit。该流程不领取 pending、不触碰既有 `manual_required`、不重试登录/验证码/发码；只有 completed checkpoint 具备 A 无漂移、B/SV、C/MY、双副本、restore probe 与 Saved Messages remote ID 时才能前滚为 `succeeded`。无法证明远端结果的同一 operation 继续保留 `deferred_reconcile`，并以 typed blocker 和重判审计收口。

> **2026-08-26 B pre-challenge Timeout unknown 人工债务合同：** frozen-N runner 在 SV B `remote_effect_started_at` 已持久化、但 Telegram challenge/flow/code/candidate 尚未出现时遇 `TimeoutError`，必须首错停批并保持原 operation `reconcile_unknown/remote unknown`；SSH 断连不得重跑登录或推导 no-effect。既有 B pre-flow 正式入口新增互斥 `stopped_b_prechallenge_unknown` 边界，只接受 batch stopped、唯一 stopped item 与唯一 unknown/sensitive 均为原 B、runtime off/scope 空、MY client0、A current/Session/三组 generation 不变、B slot 绑定原 operation、C/E4 absent、无 owner/lease/reconcile/bundle/stage，以及 effect 后没有 flow或至多一条完全空的 `intent_persisted@v1`。preview 纯数据库并冻结原 `TimeoutError`、effect/flow、batch/item/slot/operation/A/release/审批；异人 apply 只创建 `b_pre_challenge_remote_unproven` reconcile case，把同一 B/item/B槽收口 `manual_required/reconciled_hold`、C 槽投影 upstream manual、更新 execution release 并继续 pending。A、Session、Telegram、远端设备、验证码和 authorization 零写入；任何 challenge/code/temp Session/candidate/artifact/stage、第二 flow/unknown/sensitive 或 CAS 漂移都必须拒绝。

> **2026-08-26 one-shot 初始 release 绑定：** 历史 stopped frozen batch 若 `execution_release_sha` 落后当前 runtime，且尚无 one-shot start 审计，只能由同一 sweep `preview/apply` 以 fingerprint/CAS 原子记录旧→新 execution release 并继续启动；已有 start、运行中/暂停或全局安全门禁漂移均不得自动绑定，不得使用 legacy resume、临时 SQL 或账号专用入口。

> **历史兼容：2026-08-26 completed A/B 双失效债务的 chunk 边界：** 仅适用于既有 legacy chunk-pause 批次，不适用于当前 one-shot sweep。已通过专用异人审批/CAS 入口确认的 `manual_required/primary_and_sv_standby_unavailable` 仍是 frozen-N 中的真实人工债务，不得被算作 succeeded；但只要 A 仍满足该次确认冻结的 same current/Session/authorization+connection generation 与一致 fact delta，正常 runner 完成后必须允许原子写入 chunk-boundary stopped audit。chunk pause 与随后静止批次的 release-rebind 都必须复用 completed-A 扫描的同一 acknowledged-primary 判断，不能退回通用 active-A `primary_state` 造成“30项已成功但 batch 留在 running”或“已停批但无法绑定新 release”。A 的 current、Session、generation 或已确认 fact 边界再变化时仍须零写入拒绝并进入漂移对账。

> **2026-08-26 completed A/B 双失效人工债务合同：** frozen-N 已成功 item 的 current A 后续被 account-online 以新鲜 `login_required + last_probe_at` 权威判定 Session 失效时仍必须首错停批。先用正式 `local_activate preview` 即时探测既有 B；若 B 也明确返回 `session is not authorized`，不得切换该 B、不得重登 A/B、不得临时唤醒 C，也不得把历史 B/C/E4 成功冒充当前双 SV 可用。用户已批准“无法自动恢复的同类账号写入人工清单并继续全量”，因此只允许专用 `preview/fingerprint/异人审批/CAS/apply/readback` 将该 completed item 改记真实 `manual_required`：A 的 current、Session、App、三组 generation、primary authorization 全字段零写入；B 保留 Session/protected 但投影 `invalid/needs_repair` 与 typed `session_not_authorized`；C 必须仍为 dormant healthy、active bundle、KMS verified、双副本 2/2、隔离 restore probe passed 且 client=0，全部零写入；既有 E4 必须有非空 Saved Messages remote ID。若全局漂移门禁同时留下一个 B/C/E4 已完成但未投影的 running item，preview 必须把其 A/operation 版本、C 恢复证据与 E4 remote ID 一并冻结，apply 对该 item 零写入，后续 runner 只从 completed checkpoint 收口而不重放。apply 只在 runtime off、global unknown/sensitive=0、runner0、唯一 stopped item、仍有 pending 项时写 item/B-slot manual debt、batch execution release 与 secret-free audit，之后允许继续后续 item。该 manual debt 不表示 A 已恢复或三槽健康；账号必须进入人工文件，未来只能在完整 `emergency_reauthorize_primary/restore_sv_pair` 正式实现后另行恢复。

> **历史兼容：2026-08-25 正常 chunk 间发布重绑：** 仅适用于 max-30 runner 已形成的 legacy chunk-boundary audit；当前 one-shot sweep 必须在启动前完成 release gate，运行中 release 漂移进入 durable pause/reconcile，不新建外层 chunk。旧批次随后生产 release 变化时，应直接复用该最新静止边界做 release-rebind，不得把正常停批伪装成 mid-chunk 故障。preview 必须绑定同 approval_ref、旧 execution release、最新 audit ID/action 和与当前 items 一致的 succeeded/manual/pending 计数，并继续执行 A 无漂移、completed operation 完整、runtime/unknown/sensitive/MY0 门禁；异人审批 fingerprint/CAS apply 只重绑 batch execution release，零 Telegram 与授权资产写入。

> **历史兼容：2026-08-25 B/C/E4 已完成后的发布中断 checkpoint 收口：** 仅适用于旧 runner 在 item/slot sync 前被发布替换的精确边界；one-shot supervisor 运行中遇到 release drift 统一进入 durable pause/reconcile。旧边界禁止重放 B/C/E4、直接改 `execution_release_sha` 或临时 SQL。专用 DB-only `preview/fingerprint/apply/readback` 只接受 batch=`running` 且 execution release 与当前 runtime release 不同、唯一 running item、B/C/E4 全部 succeeded、B/C slot 仍 pending/unbound、A qualified 且 Session/current/generation 冻结、runtime off、global unknown/sensitive=0、MY client=0 的精确边界；preview 还必须复核当前 B/SV、C/MY、双副本、restore probe 与 E4 远端审计。apply 在一个 CAS 事务中复用普通 sync 投影完成 item，随后把 batch 停在 `production_release_changed_mid_chunk` 并写 release-pause 审计，不连接 Telegram、不改 A/B/C authorization、operation、bundle 或 execution release。之后只能复用既有 release-rebind preview/apply，再从下一个 pending item 继续。

> **2026-08-25 C bundle 已落库后的发布中断前滚：** 当 MY 已完成同一 C operation 的登录、远端身份确认、本地+独立快照双副本、inventory 与中心 bundle receipt，但 restore probe 回传精确遇到生产发布窗口的控制面 502 时，不得重登、重发码、创建新 operation 或把该账号直接记为人工失败。专用 `preview/fingerprint/apply/readback` 只接受唯一 running item、原 C=`bundle_copies_verified/confirmed`、candidate 与 bundle/copies/inventory 完整且不可变、restore probe/slot decision/E4 均不存在、旧 lease 已过期、runtime 精确 scope、MY client=0、A frozen 无漂移、B 已成功、当前 release 已变化的边界。apply 只停批、关闭原 runtime scope并为同一 operation 建立 `central_bundle_restore_forward` repair approval；随后现有 MY artifact worker 只能从原不可变 bundle 解封并执行 identity/restore probe、幂等 receipt readback 与 slot CAS，禁止调用登录和验证码路径。成功后 runner 只从 post-C/pre-E4 checkpoint 恢复；任何 bundle/copy/candidate/A/node/version 漂移均零写入停止。

> **2026-08-26 C 控制面中断收口：** 原 C operation 在 `remote_login_started` 后不得重领、重登或再次发码。专用 preview/fingerprint/异人审批/CAS 入口接受三种互斥边界：① challenge 已返回但首次取码/续租遇控制面 5xx，operation 仍 `started`、lease 已过期、runtime 精确 scope；② Telegram `start_login` 在 challenge 返回前超时，节点已上报 login-unknown；③ challenge 与冻结 code message/received-at 均已持久化，但 finish-login 结果不明，batch/item/slot 已首错停下且 operation 为全局唯一 unknown/sensitive、runtime off。三种边界都必须 MY client=0、stage 仅 `remote_login_started`、无 flow/candidate/bundle/`remote_login_confirmed`、A 冻结且 B 完整成功；②必须无 code，③必须同时具备 code message 与 received-at，半字段一律拒绝。apply 仅把同一 C 收口为带独立 `c_post_code_remote_unproven` 分类的 `manual_required/reconciled_hold`，保留 code/effect 证据、把 B 如实投影成功、C 记为人工债务并继续 frozen-N；不连接 Telegram、不读取新验证码、不生成 Session、不新建 operation，也不把 C 记为成功。账号必须进入人工清单。
...
- `complete_online_abc` frozen-N 控制面、账号/B/C 三重守恒、全量 manifest preview/apply、durable one-shot supervisor、统一异常队列，以及旧 SV C 迁移和 source-less C/MY 补建纳入本次实现；`checkpoint_interval=30` 只产生审计检查点，不限制总量。新的 10 账号 canary 已取得 B/C/E4 `10/10`，逐账号 A current/healthy 与 Saved Messages 远端 ID、C 双副本和 restore probe 均已读回。v2.23 取消固定 24 小时等待，`accept` 只在当前 A 无漂移、十条发送远端 ID 完整及其余安全门槛通过时放行；该 gate 发布前仍不接受 canary、不冻结或执行全量 `N`。
...
- B 已完成绑定验证码且同一 flow 由 Telegram 明确进入 `waiting_2FA`，但旧 runner 以 `ValueError/reconcile_unknown` 停止时，可以复用原临时 Session 提交一次租户固定 2FA。普通 legacy A 仍需账号安全快照证明 trusted Session/2FA enabled；当 A 已是 healthy 但仅缺持久化 UID、AuthKey 已存且与 operation expected AuthKey 精确一致、operation expected UID 非空，账号安全快照可以完全不存在，也可以是 nonasserting snapshot：trusted Session/2FA 均为 unknown，且无托管密码/提示/stored-at、外部授权、设备扫描、2FA 检查、可信设备标签和错误事实。此时才允许以 operation 冻结的 expected UID 作为 partial identity fence。preview 必须冻结完整快照字段（密文、提示、标签、错误和 trace 只取摘要）、A identity 缺失形状、flow/code/effect、租户固定密码版本和发布 SHA；apply 前任一快照漂移均 CAS 零写入拒绝。apply 只对同一 flow 进行 authorized probe，明确未授权时最多一次 password-only 提交。密码无效转 manual/no-effect，结果不明保持原 unknown；两者均不得重发 code、新建 flow/operation 或改动 A。任何 confirmed/enabled/disabled、托管密码、设备/2FA 检查、外部授权、可信标签或错误事实仍按未证明状态拒绝。
- 历史 A/B 角色互换账号若当前 A 已使用环境 App B，禁止再按固定 App B 新建 `standby_1`。补 B 时从 `primary_sv`/`standby_1_sv` 两条 active assignment 中选择与 current A、App C 都不同的 SV App，并冻结所选 assignment/version。若已有 App A `standby_repair`，可先做同 UID/不同 AuthKey 探测后提升；若其明确失效或探测返回 `AuthKeyDuplicatedError`，不得再使用该 Session，改由 current A 读取 challenge-bound code 在 App A 新登录 B。新 B 完成身份/AuthKey/hash 读回后，错误 App B standby 只在同事务降为 retained/protected repair，不撤销设备、不改 A，再恢复三 current 槽 App 两两不同。
- B 登录已在 Telegram 成功、但中心库插入因 current-slot 唯一索引失败时必须进入 `reconcile_unknown` 并停止后续账号。若同一 `TgLoginFlow.temporary_session_ciphertext` 仍可在原 App/代理上读回 authorized、UID 与 A 相同、AuthKey 与 A 不同，且 A 观察到该 App 恰好一个远端设备，则允许 `preview fingerprint -> 异人审批 -> generation/fact/flow/remote-set CAS apply` 原样恢复该 Session；apply 先在同事务把旧同 App standby 置为 non-current retained/protected repair，再插入新 B 并清空 flow 临时密文。禁止重新发码、重新登录或撤销 orphan；任何证据不唯一保持 unknown。
...
- C 已 succeeded、runtime 已自动关闭但 MY ready heartbeat 尚在刷新窗口时，E4 preview 的 `malaysia_wake_unavailable` 属于“远端发送尚未开始”的可等待 readiness，不得立即把 item 永久停止；runner 只轮询 readiness，操作者中断后仍从同一 running item 恢复。已由旧版本停止的 item 只允许 `resume` 一个精确检查点：`runner_blocked + malaysia_wake_unavailable + B/C succeeded + E4 operation 不存在 + A 未漂移 + runtime off + unknown=0 + 同审批`，恢复不得重登 B/C。原 manifest release SHA 不变；修复导致 current release 变化时，精确 resume 必须审计写入独立 execution release SHA，普通 run 随后继续强制 execution=current。
- B 已 succeeded、C operation 尚未创建时读取 MY readiness 也可能返回同一 `malaysia_wake_unavailable`。该状态必须被识别为独立的 `post_b_pre_c` 检查点：正式 `resume` 只有在 B candidate 仍为互补 SV healthy/non-current、A current/Session/三组 generation/身份事实均未漂移、C/E4 均不存在、runtime off/scope 空、global unknown=0、MY ready/client=0 和原审批一致时，才可恢复同一 item，并由普通 runner 使用原确定性 C key 首次创建 C。不得重放 B、不得新建平行 item/operation，也不得把该 readiness 故障误记为 2FA 人工结果。
...
- C operation 的真实 unknown 状态名为 `provision_reconcile_unknown`；runner 必须把它作为终态立即停止并输出非零，不能继续轮询，更不能领取下一个账号。对账成功后只允许从同一 operation 的持久新状态继续。
...
- B 的 challenge 已发送但冻结 A 在有效期内未读到绑定验证码时，原 operation 必须保持 `failed/verification_code_unreadable`，不得重发 code 或创建 retry operation。只有 operation 无 code message/received-at、无 candidate，原 flow 仍为 waiting-code 且未生成 authorization，A 无漂移、C/E4 不存在、全局 runtime/unknown/sensitive/MY client 静止时，才允许用户已批准的 DB-only manual preview/fingerprint/apply 跳过该账号继续 frozen-N；原 challenge/flow/failed operation 保留审计，不能伪造 `confirmed_no_effect` 或成功。
- 若生产 release 在 B 已记 `login_remote_started/remote started/effect time` 但 login flow 尚未关联回 operation 时重启并杀死唯一 runner，必须把 SSH/进程结果视为 unknown，冻结同一 operation。只在 runner=0、发布已完全收敛、current release 与 batch execution release 不同、A 无漂移、该 B 为唯一 sensitive operation，且没有 flow，或 effect 后精确只有一条同账号/standby_1/App 的空 `intent_persisted@v1`（无 challenge/code/temporary Session/phone-code-hash/authorization/error/batch-attempt/supersede），同时 operation 无 challenge/code/candidate/lease/reconcile、B/C slots pending、C/E4 absent 时，允许专用 DB-only preview/fingerprint/异人审批/CAS 将其记为 `manual_required/reconciled_hold` 并重绑 execution release；空 intent 必须进入 fingerprint、关联原 operation并终止为 `superseded` typed manual debt。remote effect 事实必须保留，不能冒充 no-effect、成功或授权设备，禁止重登与新 operation；第二 flow 或空 intent 出现任一远端/授权事实立即零写入拒绝。
...
- 10 个账号全部成功后批次进入待接受的 `observing` 状态，但不设置固定等待时长；`observation_started_at/observation_closes_at` 在新批次中取同一完成时刻，仅保留兼容读模型。正式 `accept` 必须在同一事务前后保持 runtime off、全局 unknown=0、MY active client=0，复核十个 A 的 current/Session/generation/fact/健康事实均无漂移，并确认每个 E4 succeeded operation 都有非空 Saved Messages 远端消息 ID；缺一项就拒绝全量批次。
...
- 生产新 10 账号批次 `718657f1-6582-45e7-b0aa-40a4ea1bda3c` 已取得 A/B/C `10/10`、C 双副本与 restore probe、A Saved Messages 发送读回，当前处于待接受的 `observing`。用户已明确移除原 `2026-08-24 01:31:37` 固定时间门槛；v2.23 代码发布后必须先即时复核十个 A 的正式无漂移判定、十条发送远端 ID、runtime off、全局 unknown=0、MY active client=0，再允许 accept 和全量 preview。
- 即时 A/发送验收通过后只创建一个 `selection_mode=all_online_accounts` 全量批次，在单事务冻结当时全部在线账号为动态 `N`。SSH runner 每次调用最多领取并完成 30 个尚未终态账号，称为一个执行 chunk；chunk 不是新批次、不是新分母，不得换账号填补失败项，也不设置额外固定观察时长。
- 每个 chunk 开始前必须读回 current release/schema、runtime=`off`、global unknown=0、MY client=0、批准 fingerprint、剩余 outcome 守恒和上一 chunk 终态。非尾批必须恰好 30 个；仅当全量批次剩余未终态项小于 30 时允许 1–29 个尾批。runner 每个账号继续严格串行，任何时刻最多一个 Telegram 登录 operation。
- 单账号通常顺序为 `A fresh qualify -> B readback/provision -> A generation/session/fact fence -> C readback/migrate/provision -> A send/readback -> disconnect/readback`。若 frozen current A 已有 UID/AuthKey 身份事实、Telegram 自身探测仍确认同一 UID/AuthKey，但当前设备合法返回 `hash=0`，且尚无健康中心 peer Session，则必须改为 `A frozen/current/Session/generation/UID/AuthKey preflight -> B provision -> 用新 B 作为 peer 唯一解析 A 非零 hash并完成 A qualification -> C -> A E4`。不得把 `0` 落库、不得把 B 自己的 hash 冒充 A，也不得在 B 成功和 A qualification 成功前创建 C/E4。B 失败或 unknown 只停批并保留 A；已有完整合格的 B 或 C 只读回，不重复登录。历史 A/B 角色互换按 logical current 选择码源和剩余 SV App，不按 App A/App B 名称硬编码。
- `A fresh qualify` 的 canonical 投影以账号 `current_authorization_id` 和 current/slot-current、SV、Session/App/generation 一致性为准；合法 `local_activate` 后 current A 的持久物理槽可以仍为 `standby_1`，不得因硬编码 `logical_slot=primary` 停批。若旧 runner 已在任何 B/C/E4 operation 创建前把该情况记为通用 `ValueError`，只允许显式恢复同一 item：原审批三元组一致、唯一 stopped item、三项 operation 均不存在、A 仍为 frozen 且 canonical preview 已通过、runtime off、global unknown=0。恢复只重开原 item 并审计 execution release；任一条件不满足继续 stopped，禁止改库跳项。
- 上述 B-first 仅解决 current-session `hash=0` 的观察者依赖，不降低 A 门禁：B 登录前必须先证明 A 仍是 frozen current、Session/App/generation 未漂移且已有 UID/AuthKey；B 登录仍只由冻结 A 读取本 operation challenge-bound code。B 成功后 qualification 必须通过新 B 读取远端设备集，并以 A 的 Telegram 设备指纹唯一匹配非零 hash；零匹配、多匹配、A AuthKey/UID/代次漂移全部停止，不继续 C。该顺序不修改 A 的 Session、current pointer、App/proxy 或 authorization/connection generation；qualification 只追加已核验的身份事实和对应 fact generation。
- 存量 canonical A 行若 UID/AuthKey 全部缺失或只缺其中一项，但冻结 Session 的只读 Telegram identity probe 能取得完整两者且 current hash=0/no peer，runner 不得先写“半套 A 事实”。它仅在 B bootstrap 调用中把探测到的 UID/AuthKey 摘要纳入 B preview/fingerprint 和 operation 的 expected code-source facts；A 行继续不变。B operation 后续以 current pointer、Session/App、三组 generation、fact version和非空 expected UID/AuthKey 共同 fence 码源：每个本地非空 identity 值必须与 expected 精确相同，缺失值可由 expected 临时补足校验，任何不一致 fail closed。B 身份必须与 expected UID 相同、AuthKey 不同；B 成功后由 B 解析 A 非零 hash，再一次性提交完整 A qualification 和单次 fact generation 增量。普通人工/API `preview_abc_backup` 默认仍是 DB-only，未显式启用 runner bootstrap 时缺失 A 身份必须失败。
- 若旧 fence 在 Telegram 调用前误停，只有原 B operation 仍为 `approved/remote_call_state=none` 且 remote-effect 时间、login flow、candidate、blocker、reconcile 全为空，C/E4 不存在，才属于可恢复的 pre-B no-effect checkpoint。`runner --mode resume` 必须以原 item、原 operation、原审批和原 idempotency key复核 A/partial identity 全部冻结事实，并审计 `pre_b_approved_no_remote_effect`；恢复后仅执行该原 operation，禁止创建 `retry` key、第二个 B operation或先改 A。任一远端效果痕迹、identity/A 漂移继续 stopped。
- A/B 是可互换的业务角色，`logical_slot` 是不可与当前角色混淆的物理槽。current A 物理槽为 `primary` 时，新 B 占 `standby_1`；合法 `local_activate` 后 current A 物理槽为 `standby_1` 时，`restore_sv_pair`/B provision 必须占互补的 `primary`，role 仍为 `standby_1`、`is_current=false`。创建前只释放互补目标槽内的非 current 历史 repair/current-slot 标记并保留 protected，不得释放、降级、改名或覆盖 current A。槽代次取该物理槽历史最大 generation + 1；B/C/E4/manifest/readback 均按“current A 之外、互补 SV 槽、独立 App/AuthKey、健康 non-current”识别 B，不得硬编码 B 永远位于物理 `standby_1`。
- frozen-N item 若已因 C orphan 进入 stopped，随后旧 A 出现 typed `authorization_key_duplicated`，只允许先完成正式 `local_activate` 及 generation-fenced Saved Messages 发送读回，再用专用 `post_activate_rebase` preview/fingerprint/异人 apply 重绑同一 item。rebase 必须验证原 C operation 已 exact revoke/readback 并为 compensated、全局 unknown=0、runtime off、B/E4 不存在新副作用、旧 A 仍 invalid/protected/non-current、新 current 与 applied case/远端消息/UID/AuthKey 完全一致；它只把 item 的 A 冻结基线改为新 current，把 B 计划改为使用剩余 SV App 在互补物理槽 provision，并把 B slot result 复位为 pending。不得改 source C、重写 compensated operation、删除旧 A、换 item/账号或直接伪造 B/C/E4 成功。resume 只能从 `post_local_activate_rebase` 检查点继续：先创建新 B，再 qualification 当前 A，再以原 C retry key 的下一代 generation 创建 C，最后执行新的 A E4；任一步失败只保持当前 A 在线且 redundancy degraded。
- 已 succeeded 的 frozen item 若之后 A typed duplicate，正式 `local_activate`/发送读回后必须走独立 `completed_post_activate_rebase`：保留原 C succeeded operation、MY bundle/双副本/restore probe 和旧 E4 事实，只把 A 基线重绑新 current、B 复位为互补槽 provision，并要求新的 E4 retry operation 绑定新 A/current generation；旧 E4 只能作为历史事实，不能满足新 A 验收。resume 必须支持显式 account ID，在同批多个精确恢复 item 时按 ordinal 串行且禁止存在另一 running item。resume 提交 running 前先扫描全部 succeeded A；若先发现另一账号真实漂移，当前 item 必须保持原 stopped checkpoint。历史版本已经在零远端 operation 后留下 `batch=stopped + item=running` 时，只允许专用 DB-only rearm preview/fingerprint/异人 apply 将该 item 恢复原 blocker，前提是 B/E4 仍不存在、C 仍 compensated、A/case/代次未漂移且确有另一 stopped primary drift。
- B 登录已获 Telegram 授权但因旧物理槽硬编码触发 DB `IntegrityError` 时，必须保持 `reconcile_unknown` 并停止批次。恢复只可探测原 flow 的临时 Session，证明同 UID、不同 AuthKey、目标 App 远端设备唯一、A frozen facts/runtime off/MY client=0 后，以 fingerprint+异人审批原样落入互补物理槽；禁止重新发码或新登录。恢复事务只把互补目标槽的非 current 历史 repair 降为非 current retained/protected，current A 的指针、Session、App/proxy、状态、三组 generation 和 fact version 必须逐字段不变。
- 上述 B operation 对账为 `succeeded + reconcile_status=applied` 后，原全量 item 不得跳过或新建。`runner --mode resume` 仅在唯一 stopped item 原 outcome=`reconcile_unknown`、B candidate 为互补 SV 槽 healthy/non-current/独立 AuthKey、C/E4 operation 不存在、A 仍 frozen、runtime off、global unknown=0、原审批一致时把同 item 恢复到 `post_b_reconciled_pre_primary`；随后先用 recovered B 完成 A hash qualification，再继续 C/E4。任一事实不一致继续 stopped。
...
- 历史 B 只有健康槽位/Session/App 而缺 UID、AuthKey 或可归属的非零设备 hash 时，不得直接当作 E4 合格，也不得为了补元数据重新登录 B。runner 必须先冻结 A/B 本地事实，对 B 做一次只读 Telegram identity probe，由 A peer view 解析 B hash，验证同 UID/独立 AuthKey 后仅补齐 B fact。探测失败只停当前账号，A 和已成功 C 不变。若 C 已成功而 E4 因此停止，只能从精确 post-C/pre-E4 检查点恢复，禁止重放 C。
- SSH 断连、RPC 已发而响应丢失、DB commit 结果不明或远端设备差分不唯一统一进入同一 operation 对账流程；对账成功后从原 item checkpoint 继续，仍未知时仅在 runner/client/runtime/A 无漂移门禁满足后记为 `deferred_reconcile/quarantined`，并由 durable supervisor 继续后续 item。禁止重发验证码、重新登录、创建替代设备或把未知伪装为 succeeded。
- runner 的 `status|preview` 始终只读；full sweep 只接受 `--mode sweep --until-exhausted`，输出冻结 batch、checkpoint、A fingerprint、B/C/E4 结果、runtime/MY client/global unknown、三组 outcome 守恒和 `next_action|terminal_reason`。`checkpoint_interval=30` 只触发审计并自动继续，不产生人工续跑边界；任何失败、unknown、manual、A 漂移、意外设备、runtime 冲突或执行异常必须显式分类。
- full sweep 结束条件是 frozen `N` 的 `pending=0` 且三组 outcome 各自守恒；`succeeded` 只统计全部 A/B/C/E4 真实验收通过的账号，`manual_required/deferred_issue/deferred_reconcile/unresolved` 必须留在 N 并进入统一异常队列。只有 `succeeded=N` 且无异常时才是 ABC 业务完成；首轮 pending=0 不等同于成功完成。冻结后新增在线账号进入下一次 reconciliation batch，不改写本批 `N`。
...
10 账号 canary 是全量合同的强制前置批次，不能拆成“10 个 B 成功”和“另 10 个 C 成功”来拼接。必须冻结同一 10 个账号，逐账号完成 A 前后探测、B/C 槽位结果和账号 outcome；任一 `reconcile_unknown` 立即停 claim，runtime 回到 `off`。不要求固定 24 小时：只有 10/10 账号结果均为 `already_qualified|succeeded`、B/C 各自 10/10、MY client=0、global unknown=0、正式 A 漂移判定 0/10，且每个成功 E4 operation 都能从审计读到非空 Saved Messages 远端消息 ID，才允许创建全量 `all_online_accounts` preview。任一即时 gate 失败不改写原 canary 分母，必须先按原 operation 修复或对账。
...
API、权限、敏感数据、保留清理、失败码、指标、发布、QA 和开发交接的规范性合同见 [account-malaysia-standby-session-dr-implementation-contract.md](account-malaysia-standby-session-dr-implementation-contract.md) v2.25。两份文档共同构成本 PRD；实现、QA 和发布不得只选择其中一份。

当前 `design_status=complete`、`product_resync_status=complete`、`dev_handoff_ready=true`。已部署的 `standby_2` 迁移核心、unknown 原字节对账、guarded `local_activate`、历史两账号槽位 canary、10 账号 canary 和全量在线 frozen-N 滚动执行器是不同完成层级。历史 release `cd06f75ca200552d621507f492629acd721a808f` 提供全量 manifest、source-less C/MY 补建和 max-10 runner；v2.25 将同一串行/首错停批合同的正常 chunk 提升为 30，发布验收前生产仍不得传入 30。当前 batch `718657f1-6582-45e7-b0aa-40a4ea1bda3c` 为 B/C/E4 `10/10` 且逐账号 A 发送读回成功。`restore_sv_pair`、其余完整 PRD E4 和全量账号实际执行仍未完成；不得用初始 10/10、控制面发布、既有 271 批次或数据库 ready 投影宣称全量三槽完成。
