...
> **2026-08-25 C 取码前中断收口：** 原 C operation 在 `remote_login_started` 后不得重领、重登或再次发码。专用 preview/fingerprint/异人审批/CAS 入口只接受两种互斥边界：① challenge 已返回但首次取码/续租遇控制面 5xx，operation 仍 `started`、lease 已过期、runtime 精确 scope；② Telegram `start_login` 在 challenge 返回前超时，节点已上报 `login-unknown`，batch/item/slot 已首错停下且 operation 为全局唯一 unknown/sensitive、runtime off。两种边界都必须 MY client=0、stage 仅 `remote_login_started`、无 code/flow/candidate/bundle/`remote_login_confirmed`、A 冻结且 B 完整成功。apply 仅把同一 C 收口为 `manual_required/reconciled_hold`，清除残余 lease、把 B 如实投影成功、C 记为人工债务并继续 frozen-N；不连接 Telegram、不读取验证码、不生成 Session、不新建 operation，也不把 C 记为成功。账号必须进入人工清单。
