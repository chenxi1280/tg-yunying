...
- 存量 canonical A 行若 UID/AuthKey 也尚未持久化，但冻结 Session 的只读 Telegram identity probe 能取得两者且 current hash=0/no peer，runner 不得先写“半套 A 事实”。它仅在 B bootstrap 调用中把探测到的 UID/AuthKey 摘要纳入 B preview/fingerprint 和 operation 的 expected code-source facts；A 行继续不变。B operation 后续以 current pointer、Session/App、三组 generation、fact version 和非空 expected UID/AuthKey 共同 fence 码源，B 身份必须与 expected UID 相同、AuthKey 不同。B 成功后由 B 解析 A 非零 hash，再一次性提交完整 A qualification 和单次 fact generation 增量。普通人工/API `preview_abc_backup` 默认仍是 DB-only，未显式启用 runner bootstrap 时缺失 A 身份必须失败。
