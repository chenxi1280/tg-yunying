...
Recovery 进入 stale Action claim 前必须先提交前序 Task 状态修复，确保 `claim_recovery_actions` 提交时不会顺带刷新 dirty Task。线上若 `worker drain failed` 堆栈显示 `claim_recovery_actions -> session.commit` 同时执行 `UPDATE tasks ... deadlock detected`，应检查该事务边界；不得靠扩大连接池、降低 worker 数量或限制账号总量掩盖。
