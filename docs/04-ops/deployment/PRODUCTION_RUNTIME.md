...
- Provider 创建、健康检查与六条 generation route 切换分别保留 preview/apply/readback 和完整部署 SHA 门禁；切换必须在单事务内把 3.5/3.1 前置并保留每条 route 的原后备顺序，重复 apply 为 no-op。端口健康不等于 auth/model/schema 健康。
