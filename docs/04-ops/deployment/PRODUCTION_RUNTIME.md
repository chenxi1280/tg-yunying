...
- 若健康 MiMo 已存在但 AI Action 长期保持 `ai_generation_status=pending`，先按 admission state 统计 `ready/post_follow_visibility_probe/waiting` 与最近 Reservation。waiting 正文不得排在 ready/probe 正文前反复领取 fulfillment 份额；`pending + group_bot_admission_wait + 非空 lease_owner` 表示退回 pending 未清租约的实现错误。修复验收必须同时看到 ready/probe Action 先被 claim、退回 pending 的 Action lease/claim/binding 归零，以及后续 `actual_model=mimo-v2.5` 的真实 Generation/Attempt 事实。
