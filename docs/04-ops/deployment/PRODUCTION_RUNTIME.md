...
发布替换 worker 容器后，Recovery 以过期心跳记录的 `hostname + pid` 匹配 Action lease owner；heartbeat ID 末尾的角色后缀不参与租约匹配。未进入 Telegram Gateway 的旧容器执行项应立即按 `stale_worker` 回收，已进入 Gateway 的仍按 unknown 防重复口径处理。
