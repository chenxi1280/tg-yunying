...
首轮 one-shot 完成后的 `deferred_reconcile` 二阶段使用 `deploy/authorization-online-abc-deferred-recovery.sh`。`manifest` 只读输出 canonical hash 和脱敏分组；`preview/apply` 必须携带 `--expected-deferred-count`、原 batch 审批、唯一 idempotency key、fingerprint 与 `--until-exhausted`。apply 只写 start audit 并把 batch 置为 `deferred_recovery`；`pause` 只写暂停审计和 batch 状态，不调用 Telegram、不创建或重放 operation，暂停后必须 fresh status/manifest 再用新 key 重新 preview/apply。compose 管理的 `worker-authorization-abc-sweep` 运行 `authorization_online_abc_supervisor.py`，首轮 sweep 空闲后自动跑二阶段。二阶段真实进度只看 `business_delta.succeeded` / deferred→succeeded；`processed_count` 只是审计游标，不等于 ABC 备份。该流程不处理既有 `manual_required`、不重登/发码；完整 B/C/E4 checkpoint、A 门禁和 Saved Messages remote ID 全部通过才前滚 `succeeded`，等待 2FA 或明确终态失败转 typed `manual_required`，remote effect started 但不可权威证明的同一 operation 继续保留 `deferred_reconcile`。
