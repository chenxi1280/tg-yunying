...
## 2026-09-10 18:36 任务执行修复发布

运行候选 `1129407aaffa13942c9a24a91c0683f57e003d74`，目录 `20260910182905_1129407a`。本地直传三镜像、107项定向检查、前端构建、服务器包/镜像身份、独立运行版本及34项配置同值验证通过，旧镜像精确清理通过。此前维护删除2个旧评论任务并结算11,921条已过原截止且未调用的频道记录；服务恢复且原暂停/停止状态保留。上线后过期结算继续推进，历史点赞成功投影已按真实typed事实修正、保留全部历史未知。完整任务履约尚未全部通过，详见[本轮Release Gate与证据](../../05-implementation/task-execution-repair-20260910.md)。后续仅文档验收提交不改变此运行候选。
...
首轮 one-shot 完成后的 `deferred_reconcile` 二阶段使用 `deploy/authorization-online-abc-deferred-recovery.sh`。`manifest` 只读输出 canonical hash 和脱敏分组；`preview/apply` 必须携带 `--expected-deferred-count`、原 batch 审批、唯一 idempotency key、fingerprint 与 `--until-exhausted`。apply 只写 start audit 并把 batch 置为 `deferred_recovery`；`pause` 只写暂停审计和 batch 状态，不调用 Telegram、不创建或重放 operation，暂停后必须 fresh status/manifest 再用新 key 重新 preview/apply。compose 管理的 `worker-authorization-abc-sweep` 运行 `authorization_online_abc_supervisor.py`，顺序为 full sweep、deferred recovery、`post_login_exact`；前两条空闲后自动领取已异人批准或进程重启后仍 running 的单账号 exact batch，不需要人工再跑 runner。`stopped`/unknown exact债务不自动恢复，通用手工runner拒绝该selection mode。二阶段真实进度只看 `business_delta.succeeded` / deferred→succeeded；`processed_count` 只是审计游标，不等于 ABC 备份。该流程不处理既有 `manual_required`、不重登/发码；完整 B/C/E4 checkpoint、A 门禁和 Saved Messages remote ID 全部通过才前滚 `succeeded`，等待 2FA 或明确终态失败转 typed `manual_required`，remote effect started 但不可权威证明的同一 operation 继续保留 `deferred_reconcile`。
...
- `deploy/install-antigravity-provider-slot.sh` 只建立 OS/service 边界，不代替 OAuth，并与普通发布共用 `/run/lock/tgyunying-antigravity-provider.lock`。host 必须先精确安装 Anolis AppStream 的 `python3.11` 与 `python3.11-cryptography`；bridge 固定使用 `/usr/bin/python3.11`，不得替换仍供其他 host 脚本使用的 `/usr/bin/python3` 3.6，依赖探针、Fernet key、迁移 helper、发布探针和 unit 全部使用 `-E -s`，禁止用 root user-site/PYTHONPATH 伪造依赖就绪。`check-antigravity-slot-install-state.sh` 只接受既存 unit 的 `loaded+inactive+MainPID=0` 或首次 `not-found+MainPID=0`，activating/deactivating/failed 一律拒绝；ledger 操作前后都必须复核。加密 request ledger 固定在专用 HOME 的 `.local/share/antigravity/ledger`；`migrate-antigravity-provider-ledger.py` 根据现有 env 指向决定 legacy/service authority，首次与 crash retry 分别执行 migrate/compare/verify，拒绝 claimed/started/unknown，通过 SQLite backup 纳入 WAL、比较迁移前后行数/hash/integrity，并以同目录 hard-link 原子发布；legacy 与 destination 首次都不存在时创建 canonical schema 的零行 service ledger，helper 成功后、env 切换前中断只允许 canonical schema、settled、integrity 正常的零行 staged ledger 续接，env 已指向 service 后文件缺失则失败；destination 已存在但 authority 不明或与 legacy 漂移时必须失败，禁止直接切 env 或只复制主 sqlite 文件。只有 `.gemini`、Antigravity config/cache/data/ledger 子目录可写，共享应用 `.env`、`/data` release 和授权灾备目录对 unit 不可见。bridge 四个必需文件（server、protocol、ledger、schema）由 `install-antigravity-provider-runtime.sh` 按完整 SHA 原子安装为 root-owned 只读 runtime，并在复用既有 SHA 时逐项复核非 symlink、owner 和 mode；agy 模型调用与版本探针只接收 HOME/XDG/TLS/proxy 等 allowlist 环境，不能继承 bridge token 或 ledger key。禁止用放宽 `/data` ACL 解决执行权限。必须以对应 service user 执行 `agy` 并完成模型/schema POC 后才启用 unit。`antigravity-slot-release-plan.sh` 必须在 compose/current 等发布变更前完成状态矩阵门禁；disabled/inactive 保持 skipped，disabled/active 视为漂移并在零发布写入时中止。普通发布由 `restart-antigravity-provider-slots.sh` 对 enabled slot 区分组件是否变化：四个runtime文件与候选一致、owner/mode符合原保护要求且全部active时，保留原symlink/进程/ledger，执行认证只读 `--observe-runtime` 并明确输出provider原健康及quota状态、模型探针未执行；不会把degraded标成ready。文件变化、未安装或存在inactive slot时才执行完整安装/restart/双模型探针；固定模型schema probe与 `/health=ready` 任一失败仍中止该部署完成声明，并恢复发布前runtime symlink及全部enabled unit的active/inactive起始状态。只读观测失败直接退出，不转成安装；详见发布专项PRD§11。
- 2026-09-06 01:37用户确认临时切换后的新生成route为MiniMax-M3，独立审核为MiniMax-M2.5；旧冻结Job仍保留原3.6/3.1 route，Gemini共享周额度0被明确报告。2026-09-03 authenticated模型目录已移除3.5；独立bridge发生代码变化时的完整发布探针仍要求3.6与3.1分别confirmed。started/unknown请求必须用同一request ID对账，禁止failover重放。
- Provider 创建、健康检查与六条 generation route 切换分别保留 preview/apply/readback 和完整部署 SHA 门禁；切换必须在单事务内把 3.6/3.1 前置并保留每条 route 的原后备顺序，重复 apply 为 no-op。端口健康不等于 auth/model/schema 健康。
...
账号在线保活默认使用 `ACCOUNT_ONLINE_WORKER_DRAIN_LIMIT=1000` 作为单轮分页数量，使用 `ACCOUNT_ONLINE_PROBE_CONCURRENCY=32` 控制同一时刻的 Telegram 健康探测数，并使用独立的 `ACCOUNT_ONLINE_PROBE_TIMEOUT_SECONDS=30` 限制单个健康探测，不能继承普通业务 Telegram 调用的 300 秒超时。三者只控制处理吞吐，不是账号上线名额：全部 `desired_online=true` 账号都必须进入状态机，账号池超过单页时由后续 drain 继续处理，不得在服务内部再次按前 N 个账号截断。账号在线探测必须使用当前账号 `proxy_id` 对应的代理凭据，禁止从应用服务器公网出口直连；已知Session失效、需重新登录或冻结账号按 `ONLINE_UNAVAILABLE_PROBE_INTERVAL=24小时` 自动复查，旧短排期亦受上次检查时间约束；复查再次遇到网络/凭据故障仍保持每日间隔。封禁/禁用保持退出自动保活，不因检查恢复。正常账号5分钟/低频15分钟保活不变；账号资格判断读取当前持久事实，不为任务分配额外触发健康RPC。数据库读取和状态落库留在 worker 主线程，探测线程只执行 Telegram 网络调用；结果按完成顺序流式返回，主线程逐条提交，不能等待整页全部探测结束后集中落库。`last_probe_at` 保留各账号实际完成时间；同一 drain 批次的 `next_probe_at` 和成功 stale 窗口则统一不早于本批最后一个网络探测完成后的对应间隔，避免批次耗时超过 5 分钟时早完成账号已再次到期。每个探测线程通过 `check_account_health_isolated` 在本线程独立 asyncio 事件循环中执行一次性 Telethon client，不能把 32 路探测重新提交到 process-wide 生命周期；正常发送、监听和登录仍使用原业务生命周期及持久 client cache。健康探测 client 在 `finally` 中断开，30 秒探测超时后最多等待 5 秒有界断连和 1 秒调度余量，调用返回时清理已收口，且清理错误不得覆盖原始 Telegram 错误。生产验收需同时检查 account-online 批次没有成片 `account_health_probe_failed / TimeoutError`，探测期间 TCP 连接保持在配置并发附近并在批次后回落。并发和超时参数必须为正数，非法配置会使服务明确启动失败。
...
2. Stage B：在全部业务writer仍为零写的前提下执行`reconcile-ledger`，再执行全任务takeover；AI scope必须先`takeover_ai_content_scope preview`取得`batch_id`、`classification_hash`和完整`classification_counts`，并只读输出`classification_reason_counts`定位重规划原因；apply仍只以原样提交的前三项及actor/approval作为身份合同。初始整批drift检查与chunk锁内二次校验都必须保留，可批量读取Action但不得减少校验；中途退出后继续同一batch的pending item；出现drift/conflict不得激活。
...
若 B 原 flow 已进入 2FA，但旧 managed 密码触发 `PasswordHashInvalidError/reconcile_unknown`，不得重新登录或发码。先运行 `deploy/authorization-dr-sv-two-fa-resume.sh --mode preview --operation-id <id> --tenant-id 1 --requested-by <requester>`，复核 fingerprint、A frozen、batch/item/slot stopped、runtime off、MY client=0、固定 2FA 版本晚于旧 managed 版本；legacy A 中心 UID/AuthKey 同时为空时还必须核对 operation bootstrap expected identity 非空且与 B 资格一致。若输出 planned proxy 与 operation `primary_regular:direct` 不同，必须把二者同时纳入 fingerprint，但只允许按 operation/flow direct exact match 连接，禁止回退旧代理。再由异人审批执行 `--mode apply --expected-fingerprint <fp> --actor <approver> --approval-ref <ref> --idempotency-key <key>`。入口先探测原临时 Session：已授权直接前滚，明确未授权才做一次 password-only；A 远端设备列表允许同 App 存在多个历史 Session，candidate 必须以恢复 Session 的 authorization hash、App API ID、稳定 fingerprint 与 non-current 四项唯一匹配，manifest 只读回相应 digest；匹配缺失或非唯一仍保持原 unknown。任何不明结果保持原 unknown，固定密码无效进入 manual_required，整个过程禁止修改 A；成功后由 runner 原 qualification 写入 A identity/hash，不在 reconcile 事务中提前写 A。
...
bash deploy/authorization-online-abc.sh --mode apply --tenant-id <tenant> --account-ids <id1,...,id10> --idempotency-key <batch-key> --deployed-release-sha <deployed-full-sha> --expected-fingerprint <fingerprint> --requested-by <requester> --approved-by <different-approver> --approval-ref <ticket>
```

每轮只调用一次 `start`，取得当前账号和系统生成的 B/C/E4 幂等键。随后按本节前述正式入口完成该账号 canonical A qualification、B/C 登录与 E4；禁止人工改写幂等键或并行执行下一账号。完成后调用 `sync`，并以 `status` 核对账号/B/C 三组各自仍为 10 条守恒：

```bash
bash deploy/authorization-online-abc.sh --mode start --batch-id <batch-id> --approved-by <operator> --approval-ref <ticket>
bash deploy/authorization-online-abc.sh --mode sync --batch-id <batch-id> --approved-by <operator> --approval-ref <ticket>
bash deploy/authorization-online-abc.sh --mode status --batch-id <batch-id>
```
...
2026-08-23 10 账号 runner 生产验收：batch `718657f1-6582-45e7-b0aa-40a4ea1bda3c` 对账号 `17,18,19,20,21,22,23,39,41,50` 完成 B/C/E4 `10/10` 并进入 `observing`，观察窗为 `2026-08-23 01:31:37` 至 `2026-08-24 01:31:37`（Asia/Shanghai）。A 均为 online 且 Saved Messages message ID 依次为 `183,59,54,182,1344,70,265,86,125,335`；B 均为 SV healthy/remote active；C 均为 MY healthy/remote active、active bundle 双副本且 restore probe passed/authorized。账号 23 的 C 远端登录成功后因 SV `/data` 满导致 `provision_reconcile_unknown`；只清理未被容器引用的 Docker image 后，以正式 artifact-forward reconcile 将 MY 已生成的同一 bundle 收口，未重登 C、未重发验证码、未改变 A。最终 GitHub Actions run `32589580258` 发布 release `0002d373a44b5e0bf23dd5c294dc8afc7414b32d`，线上 Alembic head=`0164_online_abc_exec_sha`，代码与 global gate 均统一统计 `reconcile_unknown|provision_reconcile_unknown`；独立读回为 backend healthy、runtime=`off`、claim scope 为空、两种 unknown 均为 0、MY node ready/active client=0、runner 进程为 0、`/data` 尚余 21G。该结果是 10/10 执行通过；观察窗未关闭前不得写最终 canary pass。

2026-08-23 全量在线 ABC 滚动执行器发布读回：GitHub Actions run `32592732387` 的 frontend、PostgreSQL、no-PostgreSQL、images 和 deploy 全部成功，生产 current release=`cd06f75ca200552d621507f492629acd721a808f`、backend healthy、Alembic head=`0165_online_abc_full`，SSH runner 已提供 `--max-accounts`。只读库存为在线 1063；runtime=`off`、claim scope 为空、global unknown=0、MY active client=0。现有 canary batch `718657f1-6582-45e7-b0aa-40a4ea1bda3c` 仍为 `observing` 且账号/B/C 各 10 项守恒；逐账号再次读回 A 仍为原 current、active/healthy、无权威错误，10/10 Saved Messages 均有远端 message ID，B/SV current `10/10`，C/MY current `10/10`，每个 C bundle active、双副本、KMS verified、restore probe passed/authorized。观察窗于 2026-08-24 01:31:37（Asia/Shanghai）关闭；在此之前未执行 `accept`、未创建全量 preview/apply、未触发新登录。
# 2026-09-10 资源存储修复运行合同（待本批发布读回）

合同`resource-storage-repair-20260910-prd.md`。准入快照保留PG和原业务身份；0232增加共享账号路径证据，旧行混合读取，禁止旧代码读取清空冗余字段后的新格式。发布沿用stop旧worker→backend迁移→新worker就绪。历史转换使用`backend/scripts/manage_admission_evidence.py preview/apply/readback`，传完整release SHA、actor、approval-ref和精确manifest；同事务审计及独立内容hash读回，无Task/Action/远端事实删除。
