...
目标详情中的“最近消息”和“账号覆盖”可以为目标画像提供来源状态，但不能在运营目标页出现画像版本、重建、清空、样本采纳、样本降权或样本剔除等治理动作。目标级发送时间窗、每日上限、账号冷却、敏感词和链接白名单属于风控中心的目标例外策略；运营目标页只展示风险状态和“去风控中心配置”入口。
...
- 风控中心：管理发送时间窗、每日上限、账号冷却、敏感词、链接白名单和目标例外策略；目标画像页不配置风控策略。
