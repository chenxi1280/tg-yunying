...
向导必须按任务类型动态切换字段。AI 活跃群展示话题、语气、AI 黑话、账号角色、无人续聊、每小时最大发送量、每轮计划发言数、参与账号比例和账号轮换说明；转发监听展示源群、目标群、转发处理方式和来源过滤；频道浏览 / 点赞 / 评论默认面向新消息流，指定消息只是快捷入口；频道评论 / 回复展示每小时最大发送量、每条评论 / 回复目标、每号每小时评论上限和讨论区不可用的预检提示。
...
AI 活跃群和频道评论 / 回复的数量规划必须先确定小时总预算，再分配到轮次、频道消息、账号和发送时间：

```text
账号范围 + 目标准入预检
  -> 可主互动账号数 / 可准备账号数 / 不可用账号数
  -> 动态推荐任务每小时最大发送量
  -> 按 24 小时曲线和当前强度切分本小时计划
  -> AI 活跃群拆成多个 Cycle / Turn
     频道评论拆到频道消息补差额
...
- AI 活跃群每轮计划发言数由任务小时量、当前曲线强度和预计 Planner 轮次推导；它可以被用户手动覆盖，但不再有 10 条这类风控硬上限。
...
数量规划不得引入静默降级或假成功。AI 供应商不可用、规则过滤、质量闸门、账号容量不足和目标不可互动都必须以明确状态、跳过原因或容量缺口展示。
...
- 自动模式必须根据任务小时上限、24 小时曲线强度、可发言账号数、当前接话 / 暖场 / 沉默模式和质量风险估算本轮计划数，不得固定为极低的 1 条或固定 2-5 个账号。
...
- AI 活跃群和频道评论 / 回复创建页必须按当前账号范围、目标准入和 24 小时曲线动态推荐数量配置；推荐值包括任务每小时最大发送量、AI 活跃群每轮计划发言数、频道评论每条消息累计目标和每号每小时评论上限。
...
- 前端必须明确展示“小时总量是硬上限，轮次 / 每条目标 / 参与比例只做分配”的说明，避免运营误以为参与比例会在每轮强制拉满或每轮字段是单账号额度。
...
- AI 活跃群每轮计划发言数不得再有 10 条这类风控硬上限；真正的发送上限来自任务每小时最大发送量、账号小时上限和全局风控。
