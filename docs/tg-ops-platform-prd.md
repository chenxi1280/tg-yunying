...
- 素材运行配置补充用户侧输入口径：普通管理员只填写缓存频道链接、`@username` 或 `t.me/c/...` 链接；系统负责解析为运行所需 peer，不要求用户理解或手填 `-100...` 内部 ID。
...
素材缓存频道是平台底座配置，归属系统设置的“提示词与素材运行配置”Tab。页面必须提供两个可独立配置的输入项：
...
- 执行缓存的 TG 账号必须已加入缓存频道，并有发消息权限；健康检查失败时页面提示“缓存频道不可访问 / 账号无权限”，而不是暴露 Telethon 原始异常。
...
`GET /api/materials/cache/config` 返回素材缓存频道和源媒体缓存频道的当前用户输入、归一化状态、配置来源和健康摘要。`PATCH /api/materials/cache/config` 保存频道链接或 `@username`，后端负责解析 `t.me/c/...`、公开链接和高级 peer id，并写入审计；普通用户页面不得要求直接填写 `-100...`。
...
- 素材运行配置必须允许普通管理员填写缓存频道链接或 `@username`，系统自动解析和保存运行所需缓存目标；不得把 `-100...` peer id 作为唯一可用输入。
...
- 系统设置只维护 TG 开发者应用、AI 供应商、提示词、素材运行配置、后台账号和权限等平台底座能力；素材运行配置必须用频道链接 / `@username` 作为主输入，由系统解析 peer 并兼容 `.env` 回退。
