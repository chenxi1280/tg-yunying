...
| 系统设置 | `/system-config` | 系统设置 | `/system-config` | 保留为平台底座；TG 开发者应用、AI 供应商、提示词、素材基础库、后台账号权限均先作为 Tab |
...
本阶段不把“素材库与 AI”拆成独立一级菜单；它是系统设置下的 AI / 素材能力 Tab。若后续重构需要单独拆出，必须同步调整路由、权限、操作手册和接口归属。
...
| 运营管理员 | 运营中心、账号同步、目标、消息、任务、监听、规则、风控、归档、数据和审计查看 |
...
| `operation_issues.manage` | 认领、忽略、标记解决、跳转处理运营异常 | 忽略和标记解决必须填写原因 |
...
| `tasks.dispatch_control` | 手动 drain、恢复 stuck action、重置 / 重排计划 | 限系统管理员，必须记录原因和影响范围 |
| `rules.publish` | 发布、回滚、复制规则版本 | 发布和回滚必须写版本差异和审计 |
| `risk.manage` | 修改风控策略、处置账号 / 代理 / 目标风险 | 解除限制、策略修改必须二次确认 |
| `materials.manage` | 上传、禁用、版本化素材，编辑提示词 | 被引用素材不能物理删除；提示词变更写版本 |
| `system.manage` | 开发者应用、AI 供应商、运行配置 | 影响全局运行的配置必须健康检查或回滚提示 |
...
| 导航升级 | 手册菜单名必须使用“运营中心”，并说明 AI、提示词、素材和后台账号权限位于系统设置 Tab |
...
- 运营中心、任务中心和系统设置关系定为：运营中心是日常工作台、运营方案和异常处理入口；任务中心是执行详情、失败事实源和调度控制台；系统设置只维护平台底座能力。
...
- 补齐账号中心作为“账号资产治理中心”的 PRD 口径，并新增全量前端页面设计章节；后续统一重构以前端页面设计、数据流转和执行流程共同作为验收基准。

### 2.8 2026-05-23 更新记录

- 按 `docs/tg-ops-platform-prd-refactor-checklist.md` 同步 PRD、总设计和前端操作手册口径；本次更新只代表文档面完成收敛，代码重构仍需按清单分批实施。
- 后续实施优先级统一改为 P0-P6：基线和口径收敛、汇总读模型与运营异常、账号可用性中心、运营方案中心、运营中心重构、任务中心收敛、系统设置/手册/验收闭环。
...
- 前端内置操作手册必须同步真实菜单和最近更新，包括运营中心、运营方案模板、任务创建动态向导、账号可用性、汇总延迟和系统设置边界。
...
- 下半部分：运营方案 / 策略模板，维护群活跃、频道互动、转发监听和账号运营方案，并生成或调整任务。
...
| 建议动作 | 重新登录账号、重新准入、同步目标、检查规则、暂停任务、调整方案 |
...
| 影响 | 受影响账号、目标能力、预计缺口 |
| 操作 | 查看任务详情、重试失败项、暂停任务、查看目标、处理账号、查看规则 |
...
- 同一目标下按 `issue_type`、`severity`、`failure_type` 和 `source_task_ids` 合并。
...
| 关联任务失败 | `task_runtime_summary` + `operation_issue.source_task_ids` | 任务名称、类型、状态、失败数、主要失败码、最近失败时间 | 任务详情、失败 action 明细 |
...
- 群活跃话题、AI 暖场间隔、频道互动节奏、转发监听方案、账号运营方案和任务失败处理入口属于运营中心，不在系统设置中编辑。

### 系统设置与 AI / 素材边界

系统设置维护“平台底座配置源”，运营中心和任务中心消费这些能力，但不反向修改底座配置。
...
| 素材基础库 | 系统设置 | 消息发送、规则、转发监听和运营方案中选择素材 | 发送记录、转发结果和任务执行明细 |
| 后台账号与权限 | 系统设置 | 其他页面按权限隐藏按钮或显示无权限状态 | 业务角色临时授权、绕过后端权限校验 |

业务页可以提供“去配置”快捷入口，但保存动作仍回到系统设置对应 Tab；运营方案只保存对配置源的引用和业务参数快照。
...
| 代理 | 代理名称、代理状态、告警状态 |
| 可用性 | 可发送、可监听、可加入、可评论、不可用原因、下次可重试时间 |
...
| 筛选 | 说明 |
| --- | --- |
| 账号状态 | 在线、待登录、等待验证码、等待扫码、等待2FA、需重新登录、受限、异常、禁用 |
| 可用性 | 可发送、不可发送、可监听、不可监听、可评论、不可评论、容量不足 |
...
| Tab | 展示内容 | 主要操作 |
| --- | --- | --- |
| 资料 | 昵称、username、完整手机号、头像、简介、账号分组、最近同步时间 | 编辑分组、同步资料、打开资料初始化 |
| 可用性 | 发送、监听、加入、评论、接码、资料修改能力，容量剩余和不可用原因 | 刷新可用性、跳转风控 / 代理 / 登录处理 |
| 账号安全 | 平台可信设备、外部登录设备、2FA、最近安全快照、最近加固结果 | 刷新安全状态、清理外部设备、设置 2FA、查看批次 |
| 资产 / 目标 | 已加入群、已关注频道、联系人、可运营目标、账号-目标准入状态 | 同步资产、查看目标、处理准入失败 |
| TG 官方验证码 | 登录验证码和官方消息验证码，展示过期时间和来源 | 同步提取官方验证码、复制验证码 |
| 验证待处理 | 入群问题、人工审批、图片验证码、挑战失败记录 | 查看问题、提交人工结果、跳转任务详情 |
| 执行记录 | 关联任务、action 摘要、最近失败、发送 / 监听 / 评论记录 | 查看任务详情、查看失败事实 |
| 审计记录 | 登录、同步、敏感查看、导出、资料和安全动作 | 查看审计详情 |

账号详情默认打开“资料”Tab；从运营中心账号异常、任务详情账号失败、风控处置队列跳入时，应按来源打开对应 Tab，例如账号掉线打开“资料 / 登录状态”，外部设备异常打开“账号安全”，准入失败打开“资产 / 目标”。
...
| 状态 | 进入条件 | 任务可用性 | 允许操作 |
| --- | --- | --- | --- |
| 在线 | session 可用，最近连接正常 | 可用，但仍要看容量、目标权限和风控 | 详情、提取验证码、移动分组、检查、同步、资料初始化、安全动作 |
...
| 异常 | 代理、权限、同步、登录、目标能力或风控异常 | 按可用性读模型判断 | 检查、同步、换代理、重新登录、查看风控 |
...
账号中心必须提供账号级汇总可用性，供任务中心预检、运营中心异常展示、风控中心处置和账号列表查询使用。
...
- 如果失败影响目标执行、任务容量或运营效果，Metrics / Recovery 上卷为运营中心异常；运营中心默认按目标展示，点开后展示关联账号批次失败和关联任务失败。
...
- 运营人员默认从运营中心看到异常，再跳转到任务详情、账号处理、目标处理或规则处理。
...
## 3.10 系统设置 - 素材库与 AI 内容

### 页面目标

统一维护 AI 供应商、提示词、黑话配置、素材和缓存状态。

本节是系统设置中的 AI / 素材能力子模块，不是当前阶段独立一级导航。它可以被消息发送、运营方案、规则中心和任务创建向导引用，但配置源、权限点和审计归属仍在系统设置。
...
| 最近更新功能 | 运营中心日常入口、运营方案模板、任务创建动态向导、账号可用性、数据汇总与延迟、导航升级、账号安全加固、批量资料初始化、任务内目标输入、账号-目标准入前置、手机号不脱敏展示 |
| 按菜单操作 | 运营中心、TG 账号管理、运营目标、消息发送、任务中心、监听中心、规则中心、风控中心、归档中心、运营数据与审计、系统设置 |
...
| 功能 | 前端展示要求 |
| --- | --- |
| 运营中心日常入口 | 展示运营人员登录后先进入运营中心：先看目标工作台和 open issue，再展开关联任务失败，按建议跳转账号、目标、规则、风控或任务详情处理，最后回到运营中心确认异常状态 |
...
| 账号可用性 | 展示账号中心不只看在线状态，还要展示可发送、可监听、可加入、可评论、可修改资料、可读取验证码、剩余容量、不可用原因和下次可重试时间 |
| 数据汇总与延迟 | 展示运营中心和任务中心列表默认读取汇总读模型，详情按目标、任务、action 或账号下钻；汇总延迟时显示最近更新时间、stale 标记和刷新入口 |
| 导航升级 | 菜单文案使用“运营中心”；AI、提示词、素材和后台账号权限位于系统设置 Tab，不作为独立一级菜单 |
...
- 现在能做什么：按钮、批量操作、抽屉、弹窗、跳转。
- 失败后去哪处理：本页处理、下钻任务中心、跳转账号 / 目标 / 规则 / 风控。
...
| 左侧导航 | 固定主导航：运营中心、TG账号管理、运营目标、消息发送、任务中心、监听中心、规则中心、风控中心、归档中心、运营数据、系统设置、审计记录、操作手册；AI、提示词、素材和后台账号权限在系统设置 Tab |
...
| --- | --- | --- | --- | --- |
| 运营中心 | 上半部分目标工作台，下半部分运营方案 / 策略模板 | 刷新、创建方案、调整方案、处理异常、查看任务详情 | 目标异常详情、方案编辑、建议动作确认 | 只读目标 / 任务 / 账号 / 异常汇总 |
| TG账号管理 | 风险 / 可用性摘要、筛选、账号表格、批量条 | 新增账号、完成登录、同步、资料初始化、设置 2FA、清理设备、移动分组 | 登录向导、账号详情、批量资料初始化、批量安全动作、批次详情 | 列表读账号汇总；详情分页读设备、验证码、执行记录 |
...
| 风控中心 | 全局策略、账号评分、代理告警、处置队列 | 编辑策略、代理检查、处置、解除限制、查看命中 | 策略编辑、代理详情、处置确认、命中详情 | 读风控汇总；命中记录分页 |
...
| 系统设置 | 配置分组 Tab：开发者应用、AI 供应商、AI 黑话、提示词与素材、后台账号权限、运行配置 | 新增、编辑、检查、启用、禁用、上传素材、批量上传、保存权限 | 配置编辑、供应商编辑、素材上传、素材详情、提示词编辑、权限编辑、健康检查结果 | 只读配置表和健康摘要；素材列表分页，大文件异步处理 |
...
目标工作台默认按运营目标展示，不按任务展示。目标行展开后展示关联任务失败、影响账号、主要 failure_type、建议动作和跳转入口。运营中心不展示 action 原始 payload；需要原始失败事实时跳转任务中心。

运营方案区用于配置业务策略模板，例如 AI 活跃群的接话 / 暖场策略、频道互动强度、转发监听规则组合、账号运营方案。保存方案后可以生成任务草稿或调整已有任务配置。
...
```text
摘要层：账号总数、可用账号、不可用账号、外部设备、未设置 2FA、最近批次失败
筛选层：分组、状态、可用性、安全、资料、批次、代理
列表层：账号表格 + 批量操作条
详情层：账号详情 Tab + 批次中心
```
...
账号详情必须按 Tab 承载不同问题：资料、可用性、账号安全、资产 / 目标、TG 官方验证码、验证待处理、执行记录、审计记录。批次中心展示资料初始化、设置 2FA、清理设备三类批次，并支持失败项重试。
...
AI 与素材 Tab 维护内容能力和 AI 底座，不作为当前阶段一级页面。

- AI 供应商：名称、模型、健康、默认状态、失败率、最近检查。
- 平台 AI 配置：默认供应商、回退策略、超时、token、温度和质量策略。
- 提示词模板：任务类型、版本、状态、测试入口。
- 素材库：文本、图片、表情包、自定义 emoji、组合消息、头像素材池。
- 素材详情：资产版本、TG 引用版本、使用任务、缓存状态和删除限制。
...
- 最近更新功能：运营中心日常入口、运营方案模板、任务创建动态向导、账号可用性、数据汇总延迟、导航升级、账号安全、资料初始化、任务内目标输入、准入前置、AI 接话 / 暖场。
...
- 运营中心、任务中心、账号中心、运营目标和风控中心之间的跳转必须携带对象 ID 和默认 Tab。
...
| 运营汇总读模型 | `target_runtime_summary`、`task_runtime_summary`、`account_runtime_summary`、`operation_issue` |
...
| `account_runtime_summary` | `account_id`、`send_available`、`listen_available`、`join_available`、`comment_available`、`profile_available`、`verification_available`、`capacity_remaining`、`unavailable_reason`、`next_retry_at`、`success_count`、`failed_count`、`flood_wait_count`、`restricted_count`、`latest_error_at`、`summary_updated_at` | 账号可用性、容量和失败趋势读模型 |
| `operation_issue` | `target_id`、`issue_type`、`severity`、`source_task_ids`、`source_action_ids`、`affected_account_ids`、`failure_type`、`failure_reason`、`suggested_action`、`status` | 运营中心按目标展示异常和处理建议 |
...
用户打开 /dashboard
  ...
  -> 展示 source_task_ids / failure_type / affected_account_ids
  -> 用户点开关联任务
  -> 跳转任务中心详情，按 task_id 查询明细
```
...
方案接口必须返回 `plan_type`、覆盖目标、关联任务、最近预览、最近生成结果、阻塞原因和最后应用时间。生成任务和应用关联任务都必须写 `operation_plan_generation_runs` 与审计记录。
...
认领、解决和忽略异常必须写原因、操作者、处理时间和审计；忽略不删除任务中心失败事实，只改变运营中心处理状态。
...
- 系统设置只维护平台底座能力，不编辑具体运营方案、任务节奏或目标异常处理。
...
| 运营中心异常 | 按目标聚合失败、展开关联任务、展示 failure_type、建议动作、跳转任务详情 / 账号 / 目标 / 规则 |
| 运营方案模板 | 创建方案、编辑方案、生成任务草稿、生成并启动、调整关联任务影响预览、暂停方案 |
| 运营方案接口 | 列表、详情、保存、生成预览、生成任务、应用到关联任务、运行记录、失败重试和审计 |
...
| 账号可用性 | 账号列表读取汇总、汇总延迟展示 stale 标记、任务预检实时重算、账号详情分页下钻安全 / 验证码 / 执行记录 |
...
- AI、提示词、素材和后台账号权限继续归入系统设置 Tab，不新增与 PRD 冲突的一级菜单。
...
- 账号中心补齐 `account_runtime_summary`，展示可发送、可监听、可加入、可评论、可修改资料、可读取验证码等能力。
...
- 前端操作手册同步真实菜单、最近更新、任务类型、异常速查和权限审计要求，覆盖导航升级、运营方案模板、任务创建动态向导、账号可用性和汇总延迟。
