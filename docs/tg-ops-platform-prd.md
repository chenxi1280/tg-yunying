...
| 任务类型 | 必填 / 常用字段 | 高级字段 |
| --- | --- | --- |
| AI 活跃群 | 规则集、规则版本、话题方向、语气、AI 黑话配置 | 每小时轮数曲线、每轮发言模式、手动每轮发言数、小时发送硬上限、准入策略、历史条数、账号记忆条数、账号角色、无人续聊开关、续聊间隔、上下文过期消息数、System Prompt 覆盖 |
...
- 频道任务和群聊任务必须在主互动前检查准入状态；频道未关注账号先关注，转发源群只要求已加入 / 可读取，AI 活跃群和转发目标群必须加入且可发言，成功后才进入主互动。AI 活跃群和转发目标群准入前置必须作为任务中心可见子任务运行。
