...
> 当前日期口径：2026-05-25（Asia/Shanghai）。数据库字段与接口以当前代码为准，本文负责统一产品和研发理解。
...
| `target_learning.view` | 查看目标画像、样本统计、画像版本和 AI 命中摘要 | 查看发送者敏感标识或样本明细时写审计 |
| `target_learning.manage` | 禁用 / 恢复学习、调整禁学规则、人工标记样本入选 / 拒绝 / 降权 | 必须填写原因，写入样本状态变更审计 |
| `target_learning.rebuild` | 重建画像、清空画像、恢复旧版本 | 危险操作二次确认；记录样本窗口、画像版本和操作者 |
...
### 2.10 2026-05-25 更新记录

- 新增“监听学习”产品边界：监听学习只为 AI 活跃群和频道评论 / 回复沉淀目标画像，不等于转发监听，也不等于素材或源媒体转存。
- 监听学习默认过滤 bot、平台托管账号、系统提示、按钮提示、风控提示、重复广告图 caption、连续刷屏内容和明显模板文案；过滤内容可以进入诊断统计，不能进入长期画像。
- 目标画像按目标群、频道或讨论区长期沉淀，记录真人短句习惯、接话方式、评论风格、常见话题、黑话、活跃时段和禁学内容；画像需要时间衰减和质量权重，不能把全量历史原文直接塞进 prompt。
- AI 活跃群生成读取“目标群画像 + 最近真人上下文 + 本轮任务话题”；频道评论 / 回复读取“目标评论画像 + 原频道消息细节”。两类 AI 复用同一套监听学习底座，但消费不同字段。
- 源媒体缓存只能由转发任务媒体规则、素材收藏或手动指定缓存动作触发。AI 学习只记录文本、caption、消息类型、发送者特征和后续真人反应，不得因为学习自动创建 `source_media_assets` 或转存 TG 媒体。
- 线上账号登录事故口径：`POST /api/tg-accounts/{account_id}/login/start` 不能只返回裸 `500`。登录启动失败必须写入登录流或账号审计，记录账号、开发者应用、trace_id、Telegram / Telethon 原始错误分类和可展示错误文案；前端必须展示“哪个账号未进入验证码流程、下一步怎么处理”，不能让运营误判为“已登录 2 个账号”。
- 线上任务创建事故口径：任务预检和 `create-and-start` 不能被前端 15 秒请求超时表现成“创建失败”，同时后端继续提交任务。长耗时预检、准入计算或创建启动必须返回可追踪状态、任务 ID 或操作 ID；前端超时文案必须能区分“服务仍在处理”和“后端明确失败”，防止重复点击和重复任务。
- 频道点赞容量口径：每条消息目标点赞数、可参与账号数和并发上限必须拆开展示。`target_per_message=30` 时不能被默认 `max_concurrent=20` 静默截断为 20 个参与账号；预检、确认页、任务详情和容量告警必须说明实际可参与账号、差额和导致截断的配置项。
- 生产 worker 健康口径：worker 容器健康检查不能复用 backend API 端口探针。OPS 和发布验证必须区分 worker 进程存活、worker 心跳、队列 drain 能力和 backend `/api/health`，避免把错误健康检查当成业务故障或反过来掩盖真实业务失败。
...
- 账号列表和批量抽屉必须支持至少 100 条 / 页，并提供“选择当前筛选前 100 个”和“追加当前筛选全部”这类快捷动作，避免运营人员逐页逐个勾选。
...
| 同步消息 | 对频道目标同步频道消息 |
| 查看目标画像 | 打开目标画像抽屉，展示群画像、评论画像、讨论区画像、版本、样本量和最近更新时间 |
| 查看学习样本 | 打开学习样本列表，按入选、拒绝、降权、已应用和过滤原因筛选 |
| 禁用 / 恢复学习 | 修改该目标后续学习开关，保留历史样本和画像审计链 |
| 重建画像 | 使用已入选样本重新生成画像版本，写审计 |
| 清空画像 | 清空画像摘要和版本，保留样本审计；危险操作需二次确认 |
| 创建任务 | 带目标预填进入任务中心 |
| 消息发送 | 带目标预填进入消息发送 |
...
目标画像抽屉必须区分三类场景：目标群画像、目标评论画像、讨论区回复画像。每类画像展示画像摘要、话题权重、常见短句、接话 / 评论模式、活跃时段、禁学内容、当前版本、来源样本数量、最近重建时间和最近一次 AI 使用记录。样本明细默认脱敏发送者标识；具备 `target_learning.view` 敏感查看权限后才显示稳定 sender 标识，并写审计。
...
| 账号明细 | 账号、状态、挑战问题、AI 答案、是否可发言、计划时间、完成时间和失败原因；任务详情只展示，不在此处处理重试 |
...
| 任务类型 | 必填 / 常用字段 | 高级字段 |
| --- | --- | --- |
| AI 活跃群 | 规则集、规则版本、话题方向、语气、AI 黑话配置、目标群画像摘要 | 每轮发言模式、手动每轮发言数、历史条数、账号记忆条数、账号角色、无人续聊开关、续聊间隔、上下文过期消息数、监听学习开关、System Prompt 覆盖 |
...
| 频道评论 / 回复 | 规则集、规则版本、预计每条评论 / 回复、互动方式、评论方向、主题方向、目标评论画像摘要 | 回复对象、每号每小时评论上限、最大评论长度、监听学习开关、System Prompt 覆盖 |

AI 活跃群必须默认使用“真人接话为主、空闲低频暖场为辅”的策略。无人续聊是低频暖场，不得在有真人上下文时刷模板话；上下文不足、事实无锚点或重复风险高时应沉默并留痕。

频道评论 / 回复必须复用 AI 活跃群的“去模板 + 语义去重 + 质量跳过”原则。评论候选必须贴频道消息里的具体词、数字、物品、场景或问题；不得用“这个内容挺有参考价值”“这个角度不错”“说得比较实在”“值得再讨论”“支持一下”等泛化评论补量。AI 不可用、候选不足、语义重复或模板化时，Planner 不创建假评论 action，并在任务详情记录可见原因。
...
| 手动刷新事件 | 拉取最近消息 / 评论，默认只更新预览和诊断，不触发转发或源媒体缓存 |
| 查看学习样本 | 打开样本列表，按场景、状态、过滤原因和时间筛选 |
| 重建目标画像 | 对指定目标和场景重新生成画像版本 |
| 禁用 / 恢复学习 | 修改目标学习开关，影响后续样本入选 |
| 清空目标画像 | 清空画像摘要和版本，保留样本审计链 |
| 收藏为素材 / 缓存源媒体 | 只有用户显式点击后才进入素材或源媒体缓存链路 |
...
Listener claim source
  ...
  -> 按唯一键去重
  -> 写 group_context_messages / channel_messages
  -> 监听学习写 target_learning_samples / target_learning_profiles
  -> 转发媒体规则、素材收藏或手动指定缓存时才写 source_media_assets
...
```text
candidate
  -> accepted       通过过滤，进入画像候选
  -> rejected       bot / 托管账号 / 系统提示 / 风控提示 / 模板等原因拒绝
  -> downweighted   重复、无人接话、过期或质量较低，仅低权重保留
  -> applied        已被某个画像版本吸收
```

每条样本必须保存 `learning_status`、`reject_reason` 或 `downweight_reason`、`quality_score`、`profile_scene`、`profile_version`、`created_by_worker` 和最近状态更新时间。状态变更不能静默覆盖，人工标记入选 / 拒绝 / 降权必须写审计。
...
| 最近更新功能 | 运营中心日常入口、运营方案模板、素材中心独立、任务创建动态向导、监听学习与目标画像、账号资产与可用性、数据汇总与延迟、导航升级、账号安全加固、批量资料初始化、任务内目标输入、账号-目标准入前置、手机号不脱敏展示 |
...
| 监听学习与目标画像 | 展示监听学习不是转发和素材转存；运营人员可在运营目标、监听中心和任务详情查看画像摘要、样本入选 / 拒绝原因、画像版本和 AI 命中摘要，可禁用学习、恢复学习、重建画像和清空画像 |
...
向导必须按任务类型动态切换字段。AI 活跃群展示话题、语气、AI 黑话、账号角色、目标群画像摘要和无人续聊；转发监听展示源群、目标群、转发处理方式和来源过滤；频道浏览 / 点赞 / 评论默认面向新消息流，指定消息只是快捷入口；评论 / 回复必须展示目标评论画像摘要和讨论区不可用的预检提示。
...
- 详情：最近事件、过滤结果、监听学习入选 / 拒绝原因、目标画像版本、源媒体缓存状态、监听账号切换记录。

监听中心不直接发业务消息；它只负责读取事件、维护水位和暴露监听异常。
监听中心必须把转发监听、监听学习和手动刷新分开展示；监听学习只写学习样本和目标画像，不得创建源媒体缓存。
...
| 监听学习与目标画像 | `target_learning_profiles`、`target_learning_samples`、`target_learning_profile_versions` |
...
| `target_learning_samples` | `target_id`、`source_message_id`、`source_scene`、`profile_scene`、`sender_peer_id`、`sender_username`、`is_bot`、`is_managed_account`、`message_type`、`text`、`caption`、`learning_status`、`reject_reason`、`downweight_reason`、`quality_score`、`observed_reply_count`、`applied_profile_version`、`sent_at` | 监听学习样本；状态为 `candidate/accepted/rejected/downweighted/applied`；保存文本、caption 和元数据，不保存或转存媒体原件 |
| `target_learning_profiles` | `target_id`、`profile_scene`、`learning_enabled`、`style_summary`、`topic_weights`、`phrase_patterns`、`reply_patterns`、`comment_patterns`、`slang_terms`、`forbidden_learning`、`active_windows`、`profile_version`、`last_rebuilt_at`、`disabled_reason` | 目标长期画像；AI 活跃群和频道评论 / 回复生成前读取 |
| `target_learning_profile_versions` | `profile_id`、`profile_version`、`source_sample_count`、`sample_window_start`、`sample_window_end`、`summary_snapshot`、`quality_snapshot`、`created_at`、`created_by` | 画像版本快照，用于解释本轮 AI 生成参考了哪版目标画像 |
...
listener_source_state
  -> group_context_messages / channel_messages
  -> target_learning_samples / target_learning_profiles
  -> source_media_assets（仅转发媒体规则、素材收藏或手动指定缓存）
```
...
| Listener | listener source、监听账号、源目标、学习订阅 | 上下文、监听水位、学习样本、目标画像、按规则触发的源媒体缓存、事件 | 不发送业务消息；监听学习不得自动转存媒体 |
| Recovery | 超时 claim、超时 lease、worker 失联、unknown | 恢复 action、任务错误摘要、审计 | 不调用 TG |
| Account Security | 账号资料初始化、设置二步密码、清理登录设备批次 | 执行 `tg_account_security_batch_items` pending/waiting 项，回写 profile / username / avatar / 2FA / device 结果 | 调用 TG，必须独立运行 |
...
- 对整个任务应用任务级每日安全上限；多个帖子同时有缺口时，按发布时间、剩余缺口比例、账号容量和曲线强度分配。
...
- 允许短窗口回补。
- 按唯一键去重。
- 区分转发监听、监听学习和手动刷新三种用途。
- 监听学习默认过滤 bot、平台托管账号、系统提示、按钮提示、风控提示、重复广告图 caption、连续刷屏内容和明显模板文案。
- 转发监听按任务规则决定是否过滤 bot、群主 / 管理员和来源不转发名单；转发过滤不等于监听学习过滤。
- 只有转发规则要求保留源媒体、素材收藏或手动指定缓存时，才创建 `source_media_assets`；监听学习不得自动转存图片、表情包、视频或文件。
...
| 主动作规划 | Planner 内部阶段 | 已满足准入账号、任务类型参数、曲线强度；频道浏览额外读取帖子池、单帖每日目标、单帖累计目标和任务级安全上限 | 浏览 / 点赞 / 评论 / AI 发言 / 转发 action | 幂等去重后不足量写 capacity_shortfall；频道浏览不足量按帖子记录缺口 |
...
Listener 采集群上下文
  ...
  -> AI Gateway 基于目标画像、最近真人上下文和事实锚点生成多账号候选
  -> 规则过滤、语义去重、幻觉风险和输出校验
  -> Dispatcher 按账号冷却发送
  -> 记录 AI generation、turn、目标画像版本、账号画像、事实锚点、质量风险和执行结果
```
...
- 目标群画像只提供风格、话题权重、黑话、接话方式和禁学内容；不能把画像当成事实来源编造经历、价格、位置、服务过程或人物评价。
- 目标画像来自过滤后的真人样本；bot、托管账号、重复广告图 caption、系统提示和风控提示不得入选画像。
...
频道评论 / 回复生成时读取目标评论画像，画像提供真人评论区的短评长度、追问方式、吐槽倾向、补充习惯、表情使用和禁用模板。画像不能替代频道原文事实；每条评论仍必须贴住原频道消息里的具体词、数字、物品、场景或问题。评论区样本不足时，系统只使用频道原文细节和通用质量闸门，不生成伪画像。
...
- `GET /api/operation-targets/{target_id}/learning-profile`
- `GET /api/operation-targets/{target_id}/learning-samples`
- `POST /api/operation-targets/{target_id}/learning/rebuild`
- `POST /api/operation-targets/{target_id}/learning/disable`
- `POST /api/operation-targets/{target_id}/learning/enable`
- `POST /api/operation-targets/{target_id}/learning/clear-profile`
- `PATCH /api/operation-targets/{target_id}/learning-samples/{sample_id}`
...
- `GET /api/operation-targets/{target_id}/learning-profile` 返回目标群画像、目标评论画像和讨论区回复画像；每类画像包含 `learning_enabled`、`profile_version`、`summary`、`topic_weights`、`phrase_patterns`、`reply_patterns`、`comment_patterns`、`forbidden_learning`、`source_sample_count`、`last_rebuilt_at` 和最近 AI 使用摘要。
- `GET /api/operation-targets/{target_id}/learning-samples` 支持 `profile_scene`、`learning_status`、`reject_reason`、`downweight_reason`、`sent_from`、`sent_to`、`page`、`page_size` 过滤；默认脱敏 sender 标识。
- `PATCH /api/operation-targets/{target_id}/learning-samples/{sample_id}` 只允许人工调整学习状态、原因和质量备注，不允许修改消息原文；必须写审计。
- `POST /api/operation-targets/{target_id}/learning/rebuild` 需要 `profile_scene`、样本时间窗口和原因；返回新画像版本、来源样本数和质量摘要。
- `POST /api/operation-targets/{target_id}/learning/disable`、`enable` 和 `clear-profile` 都必须填写原因；清空画像不删除样本和旧版本审计。
...
- AI 活跃群和频道评论 / 回复任务额外返回 `learning_profile_preview`，包含 `profile_scene`、`learning_enabled`、`profile_version`、`source_sample_count`、`sample_sufficiency`、`last_rebuilt_at`、`disabled_reason` 和 `profile_unavailable_reason`。

任务创建和预检的线上体验要求：

- `precheck`、`create` 和 `create-and-start` 必须使用同一套容量计算口径，并在响应中返回 `trace_id`、耗时、容量摘要和所有 warning / blocking reason。
- 后端预检或创建预计超过前端普通请求超时时间时，接口不得让浏览器静默中断后仍继续提交不可见任务；必须返回明确的任务 ID、异步操作 ID 或可轮询状态。
- 前端收到请求超时或连接中断时，不能直接显示“创建失败”；必须先按 `trace_id` 或最近创建记录确认后端是否已提交任务，再展示“仍在处理中 / 已创建 / 明确失败”。
- 频道点赞、频道浏览和频道评论的容量摘要必须同时展示 `target_per_message`、`max_concurrent`、`effective_account_count`、`capacity_shortfall` 和导致上限的字段来源。
- 当目标互动量大于当前有效参与账号数时，任务可以按 warning 启动，但确认页和任务详情必须展示差额；如果代码因为 `max_concurrent` 截断账号池，warning 必须明确写出“并发上限限制了参与账号数”，不能只提示“没有可新增的有效点赞账号”。
...
AI 活跃群和频道评论 / 回复任务详情还必须返回目标画像摘要和最近 AI 画像使用记录。AI generation 记录至少包含：`profile_scene`、`profile_version`、`profile_hit_summary`、`profile_unavailable_reason`、`anchor_message_ids`、`quality_risks`、`skip_reason`。
...
- `POST /api/listeners/{object_type}/{object_id}/refresh`
- `GET /api/listeners/{object_type}/{object_id}/events`
- `GET /api/listeners/{object_type}/{object_id}/learning-samples`
- `GET /api/listeners/{object_type}/{object_id}/learning-profile`
...
`POST /api/listeners/{object_type}/{object_id}/refresh` 是手动刷新，不等同于转发监听。默认只拉取最近事件、刷新诊断和可选学习候选；只有请求中显式传入 `cache_media=true` 且用户具备素材或源媒体缓存权限时，才允许进入素材收藏 / 源媒体缓存链路。

`GET /api/listeners/{object_type}/{object_id}/learning-samples` 和 `learning-profile` 与运营目标画像接口返回同一套数据结构，但入口按监听对象定位，方便监听中心查看水位、样本和画像版本是否一致。
...
素材接口归属 `materials.*` 权限；AI 供应商、提示词和黑话配置归属 `ai.*` / `prompt_templates.*` / `system.manage`，不通过素材接口保存。监听学习和目标画像接口归属 `target_learning.*` 权限；画像重建、清空、禁用学习、恢复学习和人工调整样本状态必须写审计。代理接口归属风控中心和 `proxies.manage`，系统设置只展示运行配置和健康提示。
...
- 任务创建预检、创建并启动和前端超时处理必须保证状态可追踪；前端不能显示失败而后端实际创建运行，后端也不能在客户端已中断后提交无 trace 的任务。
- 频道互动容量必须拆分目标互动量、账号选择范围、实际参与账号、并发上限和容量缺口；`max_concurrent` 截断参与账号时必须在预检确认页和任务详情直接展示。
...
- AI 活跃群和频道评论 / 回复必须读取目标画像；目标画像只能来自过滤后的真人学习样本，不能学习 bot、平台托管账号、重复广告图 caption、系统提示或风控提示。
- 监听学习不得触发 TG 源媒体转存；源媒体缓存只允许由转发任务媒体规则、素材收藏或手动指定缓存动作触发。
- 运营目标、监听中心和任务详情必须能查看目标画像摘要、版本历史、样本统计、入选 / 拒绝 / 降权原因和最近 AI 使用记录。
- 目标画像必须支持禁用学习、恢复学习、重建画像、清空画像、人工调整样本状态和查看旧版本；所有写操作必须有权限校验、原因和审计。
- 任务创建确认页必须展示当前目标画像是否可用、样本是否不足、学习是否禁用；画像不可用不能阻塞任务创建，但必须在 AI 生成记录中留不可用原因。
...
- AI generation 和 action payload / result 必须记录接话 / 暖场 / 沉默模式、事实锚点、语义簇、重复风险、幻觉风险、跳过原因、目标画像版本、画像命中摘要和画像不可用原因。
- 目标画像只能作为风格、话题和行为约束，不得作为具体事实锚点。
- Listener 压力不拖慢发送 action。
- Listener 必须证明转发监听、监听学习和手动刷新边界清晰；删除或暂停转发任务后，AI 学习不能继续创建源媒体缓存。
- 学习样本状态流必须完整保存 `candidate/accepted/rejected/downweighted/applied`，每次状态变更有原因、时间和来源。
- 画像版本必须可回溯来源样本窗口、样本数量、质量摘要和生成者；AI generation 记录引用旧版本后，重建画像不能让历史记录失去解释。
...
| 账号登录失败可观测性 | `login/start` Telethon 异常写登录流或审计、返回结构化错误、前端展示失败账号和下一步、backend 日志含 trace_id 且不只暴露裸 `500` |
...
| 监听学习与目标画像 | bot 过滤、托管账号过滤、重复广告 caption 降权、学习样本 `candidate/accepted/rejected/downweighted/applied` 状态流、入选 / 拒绝 / 降权原因、目标画像更新、画像版本、时间衰减、画像禁用 / 恢复 / 重建 / 清空、AI 活跃群和评论读取画像、学习不创建源媒体缓存 |
| AI 活跃群质量 | 真人上下文接话、目标群画像约束、空闲低频暖场、无锚点沉默、语义重复拦截、幻觉事实拦截、多账号角色分工、质量字段留痕 |
| 频道评论质量 | 频道原文细节锚定、目标评论画像约束、模板评论拦截、重复语义跳过、评论区不可用归因、画像不足时不伪造画像 |
...
| 任务创建超时一致性 | 预检超过普通请求耗时时返回可追踪状态、创建并启动超过前端超时时不产生“前端失败但后端已创建”的不一致、重复点击不会创建重复任务 |
| 频道点赞容量 | 目标每帖 30 赞、默认并发 20、有效账号大于 20、有效账号小于目标值、调高并发上限、无新增有效点赞账号、容量 warning 文案和实际参与账号一致 |
| Listener 边界 | source claim、水位、监听学习过滤、目标画像更新、转发源媒体缓存边界、手动刷新不触发转发 |
| 目标画像治理接口 | 查看画像、查看样本、人工调整样本状态、禁用学习、恢复学习、重建画像、清空画像、旧版本查看、权限不足、审计原因缺失 |
...
| Listener | source claim、水位、用途分流、手动刷新、学习样本过滤、源媒体缓存触发边界 |
