...
- “最近验证聊天为空”必须作为独立准入诊断状态。系统已判定账号需要群管理 bot / 管理员验证，但 `challenge-context` 读取不到当前验证消息时，页面不能只展示空态并让运营人员猜答案；必须展示读取状态、目标 peer、账号、最近读取时间、读取失败原因或“当前没有可用验证消息”的处理建议。
...
- 如果 `challenge-context` 返回空消息、读取失败或最近消息中没有机器人 / 管理员验证证据，必须进入 `challenge_context_empty` 或 `manual_required`，并展示“未读取到最近验证聊天”。此时不能自动调用 MiMo，也不能要求运营人员盲填验证码。
