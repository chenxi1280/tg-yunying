...
> 当前日期口径：2026-06-04（Asia/Shanghai）。数据库字段与接口以当前代码为准，本文负责统一产品和研发理解。
...
| `tasks.membership.challenge.read` | 查看验证问题、AI / OCR 答案、置信度和原始验证结果 | 敏感查看必须写审计；无权限时只展示阶段和失败类型，不展示问题原文和答案 |
...
| 账号明细 | 账号、状态、挑战问题、AI 答案、是否可发言、计划时间、完成时间和失败原因；点击账号行进入二级详情处理当前任务范围内的重试、复检、人工处理或跳过 |
...
| 准入策略 | AI 辅助验证 | 默认开启；用于文本问答、算数题和可 OCR 的验证码 |
| 准入策略 | 图形验证码失败处理 | 默认转人工处理 |
...
- 可 OCR 的图形验证码允许 AI / OCR 辅助识别；无法识别、识别低置信、人工审批或 TG 明确拒绝时，标记 `manual_required`、`challenge_failed` 或 `failed`。
- 每个账号对同一目标的同一验证问题最多自动尝试一次；重试必须记录新问题或人工确认。
- 验证问题、AI 答案、置信度、结果、失败原因和原始错误写入 action result、准入子任务详情和父任务准入汇总。
...
| `account_standby_session_provision` | 备用 session 自动补齐 / 自愈批次投影 | 只读系统任务行，显示 standby_1 / standby_2 登录补齐、激活恢复和失败原因 | 查看、刷新、跳转账号授权资产详情 |
...
| `target_membership_items` | `tenant_id`、`parent_task_id`、`membership_task_id`、`target_id`、`account_id`、`status`、`phase`、`can_send`、`phase_records`、`latest_action_id`、`failure_type`、`failure_detail`、`manual_required`、`next_retry_at`、`ready_at` | 单账号对单目标的准入事实；承载入群、关注、验证、可发言复检和失败原因 |
| `target_membership_challenge_attempts` | `membership_item_id`、`challenge_type`、`question_hash`、`question_snapshot`、`answer_source`、`answer_text`、`confidence`、`attempt_no`、`status`、`result_snapshot`、`created_by` | 入群验证尝试；记录按钮、文本、算数、OCR / AI 辅助和人工处理过程 |
...
- 准入前置账号明细必须分页；点击账号行打开二级抽屉后才能加载验证问题、AI / OCR 答案和原始错误。
...
- 文本问答、简单算数题和 OCR 验证在目标设计中必须支持 AI / 规则辅助，但在网关未返回稳定题面、答案输入位置、置信度和 `question_hash` 前，只能创建 `人工处理` 或可审计的验证辅助任务，不能伪装成已自动处理。
- 前端展示应区分“已自动处理并复检可发言”“已生成验证辅助任务待处理”“目标扩展接口待实现”三类状态，避免运营人员误判任务已经具备完整 OCR / 算数验证能力。
...
- 返回验证问题、AI / OCR 答案、置信度、结果、原始错误和处理建议。
...
| `POST /api/tasks/{task_id}/membership-items/{item_id}/challenge-answer` | `tasks.membership.challenge.handle` | 提交人工验证答案或确认允许重新 AI / OCR 尝试；必须写 `target_membership_challenge_attempts` 和审计 |
...
- 文本问题、简单算数题、固定问答和可 OCR 的验证码必须由规则 / AI 辅助尝试处理，失败、低置信、人工审批和等待管理员审批必须作为可见状态留痕。
