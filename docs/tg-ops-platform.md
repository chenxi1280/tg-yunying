...
- 频道任务启动前会自动执行关注频道阶段：运营目标可用 `@username`、公开频道链接、`https://t.me/+...` 或 `joinchat` 邀请链接、peer id 手动添加；如果任务账号范围内没有账号关注频道，先让全部可用账号按抖动节奏加入频道；如果只有部分账号已关注，只补齐未关注账号；主互动只使用已关注或刚关注成功的账号。
...
如果目标频道存在但当前账号均未关注，允许保存任务配置；启动时必须先进入关注频道前置阶段。前置阶段没有任何账号成功关注时，主互动阶段不能开始。
...
- 启动频道互动任务前，系统自动生成 `ensure_channel_membership` 前置阶段，覆盖本任务账号范围内所有候选账号。
