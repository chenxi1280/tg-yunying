...
- 救援 action 使用运营空间全局救援管理员账号直接邀请触发失败的普通账号入群；救援账号无权限、目标群不可访问、被救援账号无法解析时必须真实失败并展示原因。
...
- 第 4 次真实连续权限失败后，只创建 1 个 `invite_group_account` 救援 action。
- 修改全局救援账号后点击“重试救援”，必须使用最新配置。
- 删除测试消息失败不影响账号达标。
- AI 发言生成失败不得产生假成功。
