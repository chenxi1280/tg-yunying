...
- 群黑名单、禁言、无发言权限和 `GROUP_PERMISSION_DENIED` 归为账号-目标权限失败；同一已完成准入 action 被多次同步时只能计数一次。
- 同一账号、同一任务、同一目标群连续权限失败超过 3 次后，停止继续刷普通准入重试，转为群聊救援 action。
- 救援 action 使用运营空间全局救援管理员账号邀请配置好的 bot 入群；救援账号无权限、目标群不可访问、bot username 无效或 username 不是 bot 时必须真实失败并展示原因。
- 被配置为全局救援管理员的账号是专职处置账号，不进入群聊准入任务快照，也不参与消息发送、转发发送、频道浏览、点赞、评论或频道关注前置。
- 救援成功只表示 bot 已邀请或已在群内，账号仍必须重新验证已入群且 `can_send=true` 后才进入测试发言。
...
- 同一个权限失败 action 多次触发同步时，权限失败次数不重复增加，也不能误创建救援 action。
- 第 4 次真实连续权限失败后，只创建 1 个 `invite_group_bot` 救援 action。
- 修改全局救援账号或 bot username 后点击“重试救援”，必须使用最新配置。
- 删除测试消息失败不影响账号达标。
- AI 发言生成失败不得产生假成功。
