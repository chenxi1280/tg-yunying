...
3. 领取时不再用通用旧排期覆盖 `DispatchClaimPlan` 的 allocation/fairness 顺序；历史同群 backlog 不应反复先领取并触发群慢速模式，从而挤占已分配目标的容量。
...
5. 用 SQL `CASE` 或等价数据库顺序表达 `candidate_action_ids`，仅在该序列内做 `FOR UPDATE SKIP LOCKED`；不扩大候选集合、不放宽群冷却。
