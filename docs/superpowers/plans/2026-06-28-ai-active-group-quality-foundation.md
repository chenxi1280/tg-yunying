...
- [x] Run `run_ai_group_quality_diagnostics` on production and verify one real AI active group task shows non-duplicated send history, account online summary, AI quality funnel / rejection stats, and voice profile payload/audit evidence.
  ...
  - 2026-06-29 workflow_dispatch run `28386238570` deployed the default-provider fix and completed the production quality gate. MiniMax was healthy and set as tenant default (`provider_id=4`, `MiniMax-M3`), `AI_HOURLY_INSPECT_EXIT=0`, and `AI_GROUP_QUALITY_DONE` completed. Real active tasks showed expression-card payloads in generated actions: 郑州楼凤 `voice_profile_payload_count=14`, 天津 `voice_profile_payload_count=14`, 石家庄 `voice_profile_payload_count=14`. 天津 sampled actions carried `profile_version=1`, `quality_decision=accepted`, `generation_source=human_context`, and non-empty `memory_id`; duplicate filtering was active with `quality_rejection_counts={"duplicate_message": 23}` and `7d_semantic` rejection samples. This closes the production verification gap for the AI active group quality foundation.
  - 2026-06-30 commit `8ea4bb88` exposed the production AI 轮次 observability fields needed to prove Web / TG Bot 配置参与生成: `generation_source`, `act_type`, account expression-card version/summary/match score/reason, stance summary, memory id and semantic cluster. Push deploy run `28387897953` published release `20260629164616_8ea4bb8` with backend, workers, public API and frontend health passed. Follow-up diagnostics run `28388343713` redeployed the same commit as `20260629165255_8ea4bb8` and completed `AI_GROUP_QUALITY_DONE` at `2026-06-30T00:53:10`. Production active tasks showed non-zero expression-card and message-memory payloads: 郑州楼凤 `voice=38 memory=250 source=human_context profile_version=1 duplicate_message=1`; 青岛师范学院 active `voice=15 memory=246 source=human_context profile_version=1`; 天津 `voice=41 memory=250 source=human_context profile_version=1 duplicate_message=23`; 石家庄 `voice=28 memory=250 source=idle_continuation profile_version=1 duplicate_message=1 hallucination_risk=1`. The paused old 青岛 task had no recent sends and no payload, which is expected.
