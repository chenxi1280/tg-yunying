...
| `tasks.create.search_rank_deboost` | 创建和启动搜索排名观察任务 | 只对 `search_rank_deboost` 生效；创建仅进入草稿准备态；启动必须通过分组级代理绑定、节点健康、节点容量未超限、真实搜索结果随机豁免群、gateway 接入、协议样本已采集和灰度账号数预检 |
...
找不到目标群时，Executor 默认最多翻 70 页。翻满或搜索机器人提前没有“下一页”仍未命中目标时写 `target_not_in_results`、`pages_exhausted=true` 和最后搜索页码，并自动停止该搜索目标群点击任务；任务状态必须进入 `stopped` 或等价的不可继续终态，不能只停账号 / 关键词组合后继续消耗其他账号。恢复执行必须由运营修改目标、关键词或匹配策略后重新启动或重建任务。非目标安全浏览只允许 `navigate_only`，总量默认不超过 3，不加入、不关注、不外跳。
...
| 找不到目标群时缺少自动停止口径 | 最多 70 页；翻满或提前没有下一页仍未命中，写 `target_not_in_results` / `pages_exhausted=true` 并停止该任务或该账号关键词组合 |
...
| 搜索失败停止 | 默认最多翻 70 页；找满或提前没有下一页仍未命中目标群时写 `target_not_in_results` / `pages_exhausted=true` 并停止该搜索目标群点击任务 | §2.5、§11.2 |
...
- `pool_purpose=rank_deboost` 标记降权任务专用分组；该类型分组与 `pool_purpose=code_receiver` 接码专用分组并列，是平台内 `AccountPool`，不是 Telegram 群或频道。
- 该类型分组不可删除，只能禁用；禁用后组内账号不得被新任务选用。可以重命名展示名，但 `pool_purpose` 不可变。
- 组内账号只允许参与 `search_rank_deboost` 任务，禁止进入其他任务候选池、消息发送入口、资料初始化、账号面具初始化、2FA 设置 / 轮换和一键清理其他登录设备。
- 同一账号不得同时存在于 `pool_purpose=rank_deboost` 分组和普通分组（含未分组）；创建分组或移动账号时必须硬校验，违反时拒绝操作并返回错误「rank_deboost 分组内账号不得同时存在于普通分组」。
- 任务候选池筛选时必须硬过滤 `account_pools.pool_purpose=rank_deboost` 分组内账号，避免被其他任务通过「全部可用账号」语义误选；前端候选账号列表展示「降权专用，不可参与本任务」标识。
- 普通任务创建页（`group_ai_chat`、`group_relay`、`channel_view`、`channel_like`、`channel_comment`、`search_join_group`）选择「全部可用账号」时必须排除所有处于 `pool_purpose=rank_deboost` 分组内的账号；`search_rank_deboost` 任务创建页只允许选择 `pool_purpose=rank_deboost` 分组。
...
| 搜索排名观察任务 | `search_rank_deboost` | 在集搜机器人（首版仅 `@searchbot`/`jisou`）的搜索结果中灰度观察曝光、导航点击行为和风控边界；不得承诺“降低竞争群排名”，排名变化只作为观察指标，不计入 action success。点击语义为 `navigate_only`：只点开导航类按钮查看群资料后停留再返回，竞争群结果项含 `join_candidate` 按钮时不得点击加入按钮；账号组隔离（`pool_purpose=rank_deboost` 分组只跑该任务）；代理模型为「1 分组 = 1 代理节点」，组内多账号共享出口 IP；Dispatcher 必须使用 gateway 本次返回的 `observed_exit_ip` / `probe_exit_ip` 做出口校验，不得用绑定旧 IP 自证；任务与 `search_join_group` 平行，互不依赖；`search_join_group` 的「非目标 ≤3 navigate_only」硬约束保持不变；实时 pacing / random decision 不调用 LLM |
...
| `account_group_proxy_bindings` | `account_pool_id`、`proxy_airport_node_id`、`binding_scope='group'`、`observed_exit_ip`、`observed_exit_country`、`observed_exit_asn`、`observed_exit_isp`、`exit_ip_stability_score`、`health_score`、`is_active`、`bound_at`、`last_health_check_at` | 降权任务专用分组级代理绑定；1 分组 = 1 节点，组内多账号共享出口 IP；与授权槽位级 `account_proxy_bindings` 并行存在；同一节点不得同时被两类绑定复用；仅用于 `pool_purpose=rank_deboost` 分组与 `search_rank_deboost` 任务 |
...
- 任务创建继续收敛为快速创建 + 高级设置折叠；6 类主任务按任务类型展示动态字段、预检确认和阻塞 / warning，其中 `search_join_group` 首版必须以真实协议样本、灰度账号、授权槽位代理节点、observed exit IP、完整客户端元数据、账号执行互斥、warmup、decoy、关键词允许矩阵、公开排名规则观察指标、目标资料相关性、内容健康、极搜生态/付费关键词广告状态、小时执行量配置、仅 search_join 生效的节奏与账号上限配置、实时 pacing / random 不调用 LLM 的执行边界、全部启用订阅不可用 Bot 管理员通知配置、入群前/入群后非目标安全浏览策略、默认入群后留存和 AI 活跃群等后续任务联动预检为硬前置。
