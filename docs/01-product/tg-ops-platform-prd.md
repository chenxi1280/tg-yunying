...
| `tg_account_proxy_bindings` | `account_id`、`authorization_id`、`session_role`、`proxy_node_id`、`proxy_provider`、`proxy_type`、`proxy_host`、`proxy_country`、`proxy_asn`、`ip_reputation_score`、`last_health_check_at`、`last_failover_at`、`binding_generation`、`is_active` | 搜索目标群点击任务授权槽位代理镜像绑定；首版要求独享静态住宅 IP 或 `airport_clash` 健康节点，同账号主/备用授权不得复用节点，节点故障切换后递增绑定代际并重新 warmup |
| `account_environment_bindings` | `account_id`、`authorization_id`、`session_role`、`proxy_binding_id`、`device_model`、`system_version`、`app_version`、`platform`、`lang_code`、`system_lang_code`、`region_code`、`client_identity_key`、`fingerprint_locked`、`health_score` | 搜索目标群点击任务授权槽位环境栈；MTProto 客户端元数据运行时生成后直接镜像绑定到授权槽位，不设独立 `device_fingerprints` 主表；主/备用授权不得复用客户端元数据组合 |
