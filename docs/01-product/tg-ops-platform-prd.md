...
`group_ai_chat` 必须支持按任务配置多个话题方向和多个聊天对象老师。话题方向用于约束每轮 AI 生成的主线，老师用于描述本轮聊天面向的目标对象；老师不是账号 persona，也不要求绑定 Telegram 真实用户。

配置字段：

| 字段 | 含义 |
| --- | --- |
| `topic_directions` | 话题方向列表，每项包含 `title`、`description`、`weight`；旧 `topic_hint` 保留，并在列表为空时作为兼容回退 |
| `teacher_targets` | 聊天对象老师列表，每项包含 `name`、`description`、`priority` |
...
- 每轮规划必须先解析本轮话题方向和老师目标，再写入 AI 生成提示、话题计划、上下文过滤和 action payload。
...
- Web 任务详情页必须提供 AI 活跃群专项设置区，可查看和保存话题方向、老师、连发模拟和全账号日覆盖配置；创建 / 编辑任务表单必须同步支持同一字段。话题和老师应以结构化增删改控件为主，JSON 只作为高级编辑方式。
- TG bot 必须支持管理员在 bot 内通过按钮选择 AI 活跃群任务、查看当前设置、编辑话题方向、编辑老师、编辑连发参数、编辑全账号日覆盖参数并确认保存；不得要求管理员手写完整 JSON。`/start` 和 `/admin` 都必须返回可见菜单或状态说明，不能静默无响应。Bot 保存必须复用后端任务配置校验，不能绕过 `PATCH /api/tasks/{task_id}/group-ai-chat` 的契约。
...
- `POST /api/telegram-bot/tasks/group-ai-chat/settings`
