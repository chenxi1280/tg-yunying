...
- 共用 Dispatcher：目标准入仍优先；严格搜索 child/source 与 AI hard-hourly 必须先在真实 dispatcher scope 的 `DispatchClaimScope` 内核算跨 Window active capacity，并在每次 Allocation 前用真实 `executing + dispatch_claim_active` Action 回写当前 `DispatchClaimWindow` 与 `DispatchClaimShardAllocation` active 计数，再授予 account shard、按当前 required claims 分配 Reservation。scope capacity 来自有效并发和 active claims，不能由每个 worker、shard 或终态 Action 的历史计数估计；任一严格任务无法获得足够份额时必须显示 shared_dispatch_capacity_insufficient，不能让固定排序长期饿死另一方或重复超配。
