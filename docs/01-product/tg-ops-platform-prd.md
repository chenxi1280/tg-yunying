...
> 4. 同日跨 Task 并发必须先竞争 `ChannelViewDailyIdentityOwner(peer,message,account,date)`；只有 winner 可以创建并执行 Action。明确未触达 Gateway 的 `pre_gateway` 失败可释放，`call_issued/unknown/confirmed` 当日不可释放。完整专项合同见 `docs/03-feature-designs/channel-view-planner-starvation-remediation-prd.md` §1.3。
...
- 每个 `target_peer_id + channel_message_id` 在每个 `TaskDayLedger` 拥有唯一 `ChannelViewDailyMessageTarget`，冻结 target operation、peer、source/target revision、`effective_target_count`（无上限时等于 `daily_view_target`；有上限时等于 `min(daily_view_target, lifetime_remaining_at_attach)`）、pacing anchor、active window、累计 due 和 next due ordinal；`completed_view_count` 由当日 typed `ViewRemoteFact` 投影。
...
| `channel_view_daily_identity_owners` | `tenant/target_peer/message/account/obligation_local_date/state=available|pre_gateway|call_issued|unknown|confirmed/logical_task_id/nullable action+obligation/request_identity/version/timestamps` | peer+message+account+obligation_local_date物理唯一，非空 action、obligation 分别唯一；Planner 原子 claim，Dispatcher Gateway 前推进 call-issued；safe pre-Gateway 才释放，call-issued/unknown/confirmed 当日占位；迁移回填 legacy Action/Gateway/fact，禁止 mixed-fleet check-then-insert 重复浏览 |
