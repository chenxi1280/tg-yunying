...
- `search_join_group` 新建及专用编辑 API 接收：目标群（`target_title` + `target_link`）、搜索关键词、每日已确认目标 `daily_target_count`、单个普通 `account_group_id`、任务每日 action 硬上限 `max_actions_per_day`、完成截止时间 `scheduled_end`、日/小时抖动 `daily_jitter_percent` / `hourly_jitter_percent` 与可选 `quiet_hours`。每日目标按任务时区自然日统计 `membership_observed`；`pending`、`claiming`、`executing` 和 `unknown_after_send` 占当日待确认槽位，失败/跳过释放槽位。Planner 将尚未确认的每日目标按当天剩余 `hourly_round_curve` 权重折算为当前小时需求，并与既有 `hourly_min_successful_joins` 取较高值；每轮、每小时、账号、关键词和代理风控上限仍不得突破。达到当天目标只停止当天继续规划，不得把任务写为 `completed`；次日按新的自然日重新计算。`max_actions_per_day` 是可创建/占用 action 的硬预算，必须不小于 `daily_target_count`，两者不得混用。完成只由 `scheduled_end` 或运营显式停止触发。
