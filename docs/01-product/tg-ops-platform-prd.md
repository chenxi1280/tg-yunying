...
| 系统设置 | `/system-config` | 系统设置 | `/system-config` | 保留为平台底座；TG 开发者应用、AI 供应商、提示词、素材运行配置、全局 Clash 订阅配置、后台账号权限作为 Tab；账号面具从系统设置拆为一级菜单 |
...
| 系统设置 | `/system-config` | `system.view` | TG 开发者应用、AI 供应商、提示词、素材运行配置、全局 Clash 订阅配置、后台账号权限 |
...
| `system.manage` | 平台运行配置、素材运行配置、全局 Clash 订阅配置、后台账号权限入口 | 影响全局运行的配置必须健康检查或回滚提示；Clash 配置读取需要 `system.view`，保存、测试和同步需要 `system.manage`；Clash 订阅地址必须加密保存，普通日志和前端列表只展示脱敏摘要 |
...
| 导航升级 | 手册菜单名必须使用“运营中心”，并说明素材中心和账号面具是一级菜单；AI 供应商、提示词、素材运行配置、全局 Clash 订阅配置和后台账号权限位于系统设置 Tab |
...
- 系统设置新增“Clash 配置”Tab，只保存一个全局 Clash 订阅地址 / 接口配置，作为平台级代理节点来源。系统设置支持读取、保存、连通性测试、同步节点、显示最近同步时间、节点数量和失败原因；读取需要 `system.view`，保存 / 测试 / 同步需要 `system.manage`；不在系统设置里分配授权槽位代理。
- 全局 Clash 订阅地址必须加密保存。前端列表、审计摘要、任务 stats 和普通日志只能展示脱敏摘要、节点数量、同步状态和失败原因，不得输出完整 URL、token、节点密码或 URI 原文。
- Clash 配置页保存后必须明确展示“配置已保存 / 节点同步中 / 节点同步成功 / 节点同步失败”四类状态。保存成功但同步失败时，只能提示订阅地址已更新且节点不可用，不能把它当作代理池可用；同步失败必须展示可读失败原因和重试入口。
...
#### 2.8.2 Clash 全局订阅分阶段状态

系统设置只负责一个全局 Clash 订阅源，不能承担账号代理分配。系统设置的 Clash 页面只展示“保存、解析、同步、健康探测”四个阶段；“槽位绑定”是完整代理链路的第五阶段，但只能在“账号面具 > 账号代理”展示和操作，不能回到系统设置。

| 阶段 | 成功含义 | 页面状态 | 不能宣称 |
| --- | --- | --- | --- |
| 保存 | 订阅地址已加密写入 `proxy_airport_subscriptions` | `configured`，展示脱敏摘要 | 代理节点可用 |
| 解析 | 已拉取订阅并识别 Base64 URI 列表 / Clash YAML / JSON | 展示解析节点数和过滤伪节点数 | 节点健康或出口可用 |
| 同步 | 真实节点已标准化写入 `proxy_airport_nodes` | 展示节点总数、最近同步时间、失败原因 | 授权槽位已经绑定代理 |
...
健康节点数为 0、订阅解析失败、同步失败或出口观测失败时，`search_join_group` 创建 / 启动预检必须给出明确阻断原因；不能用“订阅已保存”或“节点已解析”绕过代理可用性校验。

#### 2.8.3 搜索目标群点击任务配置缺口补齐
...
创建和编辑页必须只在 `search_join_group` 下展示以下专属字段，其他任务类型不得出现或消费这些字段：

| 字段 | 范围 / 语义 | 闸门顺序 |
| --- | --- | --- |
| `per_account_total_action_limit` | 单账号在该任务内累计搜索目标点击上限；0 表示不设置该侧上限 | 先于小时补量 |
| `per_account_daily_action_limit` | 单账号自然日上限，按租户时区计算 | 先于小时补量 |
| `per_account_cooldown_days` | 同账号两次 search_join action 之间至少间隔天数 | 先于小时补量 |
| `per_keyword_account_daily_limit` | 同账号同关键词自然日上限 | 先于小时补量 |
| `max_actions_per_day` | 任务级自然日总上限 | 先于小时补量 |
| `hourly_skip_probability` / `daily_skip_probability` | 0 到 1 的概率，采样结果必须持久化到 pacing decision | 先于小时补量 |
| `skip_probability_per_action` | 单 action 显式跳过概率，采样结果必须可复现 | 先于真实 action 创建 |
| `hourly_jitter_percent` / `daily_jitter_percent` | 0 到 100 的抖动百分比，不得突破硬上限 | 调整补量目标，不覆盖硬上限 |
...
找不到目标群时，Executor 默认最多翻 70 页。翻满仍未命中目标时写 `target_not_in_results`、`pages_exhausted=true` 和最后搜索页码，并自动停止该搜索目标群点击任务；任务状态必须进入 `stopped` 或等价的不可继续终态，不能只停账号 / 关键词组合后继续消耗其他账号。恢复执行必须由运营修改目标、关键词或匹配策略后重新启动或重建任务。非目标安全浏览只允许 `navigate_only`，总量默认不超过 3，不加入、不关注、不外跳。
...
| 缺口 / 风险 | 修正口径 |
| --- | --- |
| 系统设置和账号面具边界容易混淆 | 系统设置只保存全局 Clash 订阅；账号代理和授权指纹都在“账号面具”一级菜单中按授权槽位配置 |
| Clash 保存成功容易被误报为代理可用 | 完整代理链路按保存、解析、同步、健康探测、槽位绑定五阶段验收；前四阶段在系统设置展示，槽位绑定只在“账号面具 > 账号代理”展示；健康节点数为 0 不可作为候选代理池 |
...
| 找不到目标群时缺少自动停止口径 | 最多 70 页，翻满仍未命中写 `target_not_in_results` / `pages_exhausted=true` 并停止该任务或该账号关键词组合 |
...
| Clash | 系统设置只保存一个全局 Clash 订阅地址；保存成功、订阅解析、节点同步、节点健康必须在系统设置分阶段展示，授权槽位绑定只在“账号面具 > 账号代理”展示 | §2.8、§8.4、§9.7、§20.2 |
...
| 搜索失败停止 | 默认最多翻 70 页；找满仍未命中目标群时写 `target_not_in_results` / `pages_exhausted=true` 并停止该搜索目标群点击任务 | §2.5、§11.2 |
...
| Clash 配置 | `proxy_airport_subscriptions` / `proxy_airport_nodes` | 保存全局订阅、测试连通性、同步节点、查看同步状态和健康节点数 |
...
系统设置维护“平台底座配置源”，运营中心、任务中心、规则中心和素材中心消费这些能力，但不反向修改底座配置。代理治理分层：全局 Clash 订阅地址是平台级节点来源，归系统设置；账号 / 授权槽位代理绑定归“账号面具 > 账号代理”；代理健康、限流、处置和告警归风控中心。系统设置不得分配账号或授权槽位代理，风控中心不得保存全局订阅密钥。
...
| 全局 Clash 订阅 | 系统设置 | 账号面具和任务预检展示节点同步状态、健康节点数和配置入口 | 授权槽位代理分配、账号级换节点、风控处置 |
...
| 授权槽位代理绑定 | 账号面具 | 任务预检、账号详情和风控中心展示代理健康提示与深链入口 | 系统设置、任务创建表单、运营方案 |
...
- “账号代理”按 `account_id + developer_app_id/api_id + authorization_id/session_role` 绑定代理节点。同一账号在不同 TG 开发者应用、不同 session key 和主 / 备用授权槽位下可以使用不同代理出口。账号代理配置入口在“账号面具”菜单内，系统设置只保存全局 Clash 订阅地址，不负责授权槽位代理分配。
...
- Clash 配置：只保存一个全局 Clash 订阅地址 / 接口配置，展示脱敏订阅名、保存状态、同步状态、最近同步时间、节点总数、健康节点数和失败原因；保存、测试和同步写审计，保存成功不等于节点同步成功，不能在系统设置里分配账号或授权槽位代理。
...
| `proxy_airport_subscriptions` | `tenant_id`、`provider`、`clash_subscription_url_ciphertext`、`subscription_format`、`status`、`last_sync_at`、`last_error`、`node_count`、`healthy_node_count`、`updated_by` | 系统设置里的全局 Clash 订阅配置；首版每租户只允许一个 active 订阅地址，作为代理节点来源；完整订阅 URL、token 和节点 URI 必须加密保存并禁止普通日志输出 |
| `proxy_airport_nodes` | `subscription_id`、`node_key`、`name`、`protocol`、`host`、`port`、`country`、`region`、`asn`、`isp`、`status`、`observed_exit_ip`、`last_health_check_at`、`capacity_limit` | 由全局 Clash 订阅同步出的标准节点池；节点入口 host 不等于真实出口 IP，必须通过健康检查观测出口事实后才能绑定授权槽位 |
...
| `proxy_airport_subscriptions` | `name`、`clash_subscription_url_encrypted`、`subscription_format`、`max_authorizations_per_node_default`、`all_nodes_down_policy`、`notify_admin_on_all_nodes_down`、`fetch_interval_minutes`、`last_fetched_at`、`last_fetch_status`、`is_active` | 系统设置全局 Clash 订阅；订阅 URL 加密存储，支持 Base64 URI 列表 / Clash YAML / JSON 自动识别，拉取失败、格式识别失败或全节点不可用必须可见，不得静默 fallback；全节点不可用时复用租户 Bot 通知全部管理员 Chat ID |
...
| `proxy_node_failover_events` | `subscription_id`、`account_id`、`developer_app_id`、`authorization_id`、`session_role`、`from_node_id`、`to_node_id`、`reason`、`outcome`、`observed_error`、`admin_notification_status`、`admin_notification_detail`、`admin_notified_at`、`triggered_at` | 搜索目标群点击任务机场节点故障切换审计；当前授权槽位绑定节点不通时按 `switch_to_next_healthy_node` 切到下一个健康节点，全订阅不可用时记录 `airport_all_nodes_unavailable`、推送管理员通知并停止真实操作 |
...
- 授权环境绑定的权威粒度是 `account_id + developer_app_id/api_id + authorization_id/session_role`。同一账号在不同 TG 开发者应用 `api_id/api_hash`、不同 session key 和主 / 备用授权槽位下可以绑定不同客户端元数据和不同代理节点；Executor 必须使用授权槽位登录时绑定的同一 TG 开发者应用、指纹配置和代理配置，不能用另一个应用的指纹或代理配置代替，也不能回退本机直连。
- `airport_clash` 节点必须来自已成功拉取、完成 Base64 URI 列表 / Clash YAML / JSON 解析、过滤伪节点、通过健康检查并完成真实出口 IP 观测的订阅；随机分配后固定到授权槽位，不按 action 轮换；每个节点绑定授权槽位数不得超过默认容量或单节点覆盖；当前节点不通时只允许按 `switch_to_next_healthy_node` 切换下一个健康节点并重置 warmup，全订阅不可用时必须暂停 / 跳过且不创建真实操作。
- 全订阅节点不可用时必须复用租户 Telegram Bot 通知链路，向 `Tenant.admin_chat_id` 配置的全部管理员 Chat ID 推送脱敏告警；通知失败只记录 `admin_notification_failed` 和审计，不允许因此继续执行或回退直连。
...
- 任务创建继续收敛为快速创建 + 高级设置折叠；6 类主任务按任务类型展示动态字段、预检确认和阻塞 / warning，其中 `search_join_group` 首版必须以真实协议样本、灰度账号、授权槽位代理节点、observed exit IP、完整客户端元数据、账号执行互斥、warmup、decoy、关键词允许矩阵、公开排名规则观察指标、目标资料相关性、内容健康、极搜生态/付费关键词广告状态、小时执行量配置、仅 search_join 生效的节奏与账号上限配置、实时 pacing / random 不调用 LLM 的执行边界、全节点不可用 Bot 管理员通知配置、入群前/入群后非目标安全浏览策略、默认入群后留存和 AI 活跃群等后续任务联动预检为硬前置。
...
- 系统设置只维护 TG 开发者应用、AI 供应商、提示词、素材运行配置、全局 Clash 订阅、后台账号和权限等平台底座能力；素材运行配置必须用频道链接 / `@username` 作为主输入，由系统解析 peer 并兼容 `.env` 回退，同时提供可搜索的缓存执行账号选择器；授权槽位代理和授权指纹配置仍在“账号面具”一级菜单内完成。
