...
- 必需频道关注完成后，若原群管提示已被删除或移出上下文窗口，确认链路必须从当前绑定账号的实时窗口寻找同一可信群管的新提示。Telegram 拉取快照必须带当前查看账号的远端 peer id；仅当提示显式收件人等于该 peer id、频道集合与当前准入规则一致且存在确认按钮时才允许重绑 callback，禁止把其他账号的相邻群管提示误归属。
