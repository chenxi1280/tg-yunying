...
> **2026-08-24 full frozen-N 人工失败继续口径：** `failed/manual_required/reconcile_unknown` 仍必须首错停批，禁止自动跳过。若同一账号经正式对账已证明原 B operation 为 `manual_required + confirmed_no_effect`、unknown 清零、C/E4 从未启动且 A 全冻结事实不变，用户可对精确账号明确批准“记人工失败并继续”。系统只允许通过 DB-only `preview/fingerprint/异人审批/CAS/audit` 将 item、B、C 三组 outcome 记为真实 `manual_required` 债务并恢复同一 frozen batch 的下一 pending；不得重发码、重登、新建 operation、改 A 或写成 succeeded。以后每个新异常仍需单独停批和批准，runner 单次仍最多 10 个。全量收口允许 `completed_with_manual`，但必须展示成功数、人工数和清单，不能声明全部 B/C 备份成功；canary 不适用该例外。
