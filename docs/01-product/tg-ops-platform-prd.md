...
> 2026-07-21 搜索点击口径优先说明：`@jisou` 的关键词回复必须先点“群聊 / 群组”类型；固定 70 页只代表旧 payload 的兼容字段，不是搜索或任务终止条件。只有精确公开 username 命中并确认成员关系才结束成功搜索；真实没有下一页时记录实际页码和 `no_next_page`，action 失败但任务不停止。
