...
| AI 活跃群小时硬目标 | 在任务中心说明 AI 活跃群必须启用每小时硬目标，`hourly_min_messages` 最低为 60 条 / 小时；专项口径以 `docs/03-feature-designs/ai-group-hard-hourly-target-prd.md` 和 `docs/04-ops/ai-group-hard-hourly-target-ops.md` 为准 |
...
历史配置、旧 action 或测试数据里出现的 `light_question`、`side_comment` 只允许在边界层兼容读取，并必须在进入 Planner 内部、AI Prompt、Action payload 和任务详情前归一为 `question`、`light_disagree`；线上观测、质量统计和 PRD 均只使用上表词表。
...
- 每个 Turn 先确定 `slot_id`、`act_type`、引用对象、账号、话题方向和讨论对象，再进入批量 Prompt。`act_type` 必须使用 PRD 词表，历史别名只能在输入边界兼容并归一；AI 只能填充 slot 内容，不能反向新增账号、增加本轮 Turn 数或改变引用关系。
