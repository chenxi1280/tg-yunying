...
| 搜索自动入群任务 | 在任务中心说明 `search_join_group` 是第 6 类主任务，首版执行模式固定为 `mtproto_userbot`，通过第三方索引机器人执行关键词搜索、翻页、匹配、入群前非目标安全浏览、MTProto callback / Telegram 内部 URL resolve、加入、停留、入群后安全浏览和入群后留存；公开排名资料只作为产品推断，任务详情观察目标群排名、搜索曝光、加入成功、留存和后续任务活跃，不承诺排名结果。调研落地字段必须覆盖目标资料相关性、内容健康、极搜生态、付费关键词广告、排名观察来源和反作弊风险；排名观察写入独立快照，不计入 action success。iOS / Android 客户端元数据用于让按钮 callback / Telegram 内部 URL resolve 在 Telegram / 目标机器人链路上呈现稳定移动端画像，不用于伪装外部 HTTP iOS WebView；外部 HTTP URL 首版必须跳过并标记 `external_url_requires_web_profile`。创建前必须通过真实协议样本、button type / button effect、proxy egress guard、API ID / session / 客户端元数据一致性、主/备用授权槽位独立代理节点、完整客户端元数据、机场订阅格式识别（Base64 URI 列表 / Clash YAML / JSON）、每节点容量、节点故障切换、全订阅不可用停手、全节点不可用 Bot 管理员通知、小时执行量模型、observed exit IP、账号执行互斥锁、warmup、decoy 占比和灰度账号数预检；支持 `airport_clash` 机场订阅解析节点、观测真实出口 IP 并随机固定到授权槽位；小时执行数量复用 AI 活跃群自然小时桶 / 24 小时曲线 / future open / overdue open / deficit 的统计思想，但指标是 `search_join` 成功 action；非目标安全浏览总量默认不超过 3 且只允许 `navigate_only`，不得加入非目标群；首版默认不发言且 `post_join_policy=stay_joined`；成功加入后可按冷却、可发言复检和新成员占比限制联动 AI 活跃群等后续任务 ready pool；专项口径以 `docs/03-feature-designs/search-click-boost-prd.md` 为准 |
...
| `tg_account_proxy_bindings` | `account_id`、`authorization_id`、`session_role`、`proxy_node_id`、`proxy_provider`、`proxy_type`、`proxy_host`、`proxy_country`、`proxy_asn`、`ip_reputation_score`、`last_health_check_at`、`last_failover_at`、`binding_generation`、`is_active` | 搜索自动入群授权槽位代理镜像绑定；首版要求独享静态住宅 IP 或 `airport_clash` 健康节点，同账号主/备用授权不得复用节点，节点故障切换后递增绑定代际并重新 warmup |
...
- 任务创建继续收敛为快速创建 + 高级设置折叠；6 类主任务按任务类型展示动态字段、预检确认和阻塞 / warning，其中 `search_join_group` 首版必须以真实协议样本、灰度账号、授权槽位独立代理节点、observed exit IP、完整客户端元数据、账号执行互斥、warmup、decoy、关键词允许矩阵、公开排名规则观察指标、目标资料相关性、内容健康、极搜生态/付费关键词广告状态、小时执行量配置、全节点不可用 Bot 管理员通知配置、入群前/入群后非目标安全浏览策略、默认入群后留存和 AI 活跃群等后续任务联动预检为硬前置。
