...
`group_ai_chat` 必须支持按任务配置多个话题方向和多个聊天对象。话题方向用于约束每轮 AI 生成的主线，聊天对象用于描述本轮群聊围绕的人物、称呼或对象；它不是账号 persona，也不要求绑定 Telegram 真实用户。

配置字段：

| 字段 | 含义 |
| --- | --- |
| `topic_directions` | 话题方向列表，每项包含 `title`、`description`、`weight`；Web 主入口只要求每行一个话题，系统按行顺序自动生成权重；旧 `topic_hint` 保留，并在列表为空时作为兼容回退 |
| `teacher_targets` | 聊天对象列表，每项包含 `name`、`description`、`priority`；Web 主入口只要求每行一个对象，系统按行顺序自动生成优先级 |
...
- 每轮规划必须先解析本轮话题方向和聊天对象，再写入 AI 生成提示、话题计划、上下文过滤和 action payload。
...
- Web 任务详情页必须提供 AI 活跃群专项设置区，可查看和保存话题方向、聊天对象、连发模拟和全账号日覆盖配置；创建 / 编辑任务表单必须同步支持同一字段。话题方向和聊天对象主入口必须是普通多行文本：每行一个，越靠前权重 / 优先级越高，不要求运营人员手写数组或 JSON。运营人员可把同一组多行话题和聊天对象当作单任务“话题包”维护；当前阶段不新增跨任务模板表，避免为紧急配置链路引入额外迁移和同步成本。
- TG bot 是轻量运营入口，不再承担完整配置编辑。管理员在 bot 内只能选择 AI 活跃群任务、查看当前设置摘要、查看话题 / 聊天对象摘要、获取 Web 任务中心编辑入口，以及看到启动 / 暂停等运营状态说明；不得在 bot 内编辑话题方向、聊天对象、连发参数或全账号日覆盖参数，也不得继续提供 `/ai_group_set <json>` 这类手写配置入口。`/start` 和 `/admin` 都必须返回可见菜单或状态说明，不能静默无响应。所有配置写入必须通过 Web 任务创建 / 编辑 / 详情页复用 `PATCH /api/tasks/{task_id}/group-ai-chat` 的契约。
