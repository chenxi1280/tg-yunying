...
- 历史 open Action 若已记录 `voice_profile_anchor_rewritten=true`，必须在 Planner 重规划或 Gateway 前写 `voice_profile_anchor_replan`、转 `skipped`，关联消息记忆转 `expired_before_send`，coverage reservation 释放回 `ready`；不得继续发送历史强制改写正文。该历史字段与索引继续用于旧债统计，但新生成 Action 的改写计数必须为 0。
