...
| 系统设置 | `/system-config` | 系统设置 | `/system-config` | 保留为平台底座；TG 开发者应用、AI 供应商、提示词、素材运行配置、账号表达卡、后台账号权限作为 Tab |
...
| 系统设置 | `/system-config` | `system.view` | TG 开发者应用、AI 供应商、提示词、素材运行配置、账号表达卡、后台账号权限 |
...
| `ai_voice_profiles.manage` | 查看、搜索、编辑、重建和停用账号表达卡 | 表达卡是账号级全局资产，修改影响该账号参与的所有 AI 活跃群；该权限必须能在后台用户权限弹窗分配，写接口必须同时被权限中间件和路由显式守卫；保存、重建和停用必须写审计 |
...
| AI 活跃群小时硬目标 | 在任务中心说明 AI 活跃群必须启用每小时硬目标，`hourly_min_messages` 最低为 10 条 / 小时；专项口径以 `docs/03-feature-designs/ai-group-hard-hourly-target-prd.md` 和 `docs/04-ops/ai-group-hard-hourly-target-ops.md` 为准 |
...
| 导航升级 | 手册菜单名必须使用“运营中心”，并说明素材中心是一级菜单；AI 供应商、提示词、素材运行配置、账号表达卡和后台账号权限位于系统设置 Tab |
| 账号表达卡管理 | 在系统设置说明账号表达卡是账号级全局真人感原则；支持按账号名、username、手机号后四位、状态、更新时间搜索，支持查看、编辑、重建、停用和审计查看 |
...
AI 活跃群必须默认使用“真人接话为主、空闲低频暖场为辅”的策略。无人续聊是低频暖场，不得在有真人上下文时刷模板话；上下文不足、事实无锚点或重复风险高时应沉默并留痕。成人交易 / 性服务描述允许作为既有群聊、频道原文或讨论区事实上下文被理解和引用；进入 AI 生成前应按安全口径概括露骨服务词、价格和联系信息，保留位置、编号等非露骨事实。生成内容不得新增联系方式、价格、邀约或交易撮合信息；经历 / 消费经历只能来自账号表达卡设定或当前上下文，不能扩展成新的具体交易事实。
...
AI 活跃群不能只在发送瞬间临时拉起账号。平台需要提供全局账号在线保活能力，默认覆盖所有 active、未禁用、未封禁、未要求重新登录且有可用 session 的运营账号；运行中的 AI 活跃群、转发和监听任务会把任务账号范围内的账号追加为强需求来源。禁用、封禁、需重新登录、session 无效或账号级安全阻断的账号不得被强行保活，但必须在账号中心、任务详情和运营异常中展示明确原因。
...
AI 活跃群的真人感目标不是让模型自由编造身份，而是让自动发言基于账号表达卡和任务上下文具备不同账号的表达差异、群聊接话关系、自然节奏、短期立场延续和低模板感。系统必须优先解决“所有账号像同一个模型”“每条都完整总结”“总是自己开话题”“前后态度断裂”等问题。

真人感生成仍采用一轮批量生成作为默认策略，不能把每条消息都拆成一次 AI 请求。Planner 先用规则生成本轮 slots，再把 slots 数组一次传给 AI。每个 slot 至少包含：账号、账号表达卡短摘要、消息行为类型、上下文锚点、话题方向、讨论老师、账号短期立场、禁止重复样例和输出长度约束。AI 负责按 slot 写具体内容，本地质量规则负责过滤、重写或减少发送量。首轮批量生成后若部分 slot 因重复、模板感、画像低分、事实锚点不足或同批次多样性不足被丢弃，允许继续发起补位 AI 调用；单轮规划最多 3 轮生成，后续补位轮只携带失败 slot、失败原因、禁用样例和必要上下文，不重写已通过 slot。

账号表达卡用于区分不同账号口气，必须结构化、缓存化、短摘要注入：

- 表达卡字段包括句长偏好、互动习惯、语气强度、用词偏好、表情倾向、是否爱追问、是否爱附和、是否偶尔轻吐槽、禁用表达。
- 表达卡允许配置年龄段、表达风格、口头习惯、互动偏好、经历设定和消费经历设定；这些内容只作为账号长期人设和表达约束使用，生成内容仍需遵守任务上下文、去重和质量规则。
- 表达卡是账号级全局资产，同一个账号在所有 AI 活跃群任务里使用同一张表达卡；任务和目标群只能追加短期上下文、话题和立场，不能为同一账号生成另一套长期人设。
- 表达卡在账号初始化流程中批量生成；存量账号缺表达卡时，由系统设置或任务启动前触发批量补齐。日常 Planner 只读取缓存，不每轮调用 AI 分析。
- 默认表达卡由 AI 按账号批量随机生成，但必须受严格提示词和差异度校验约束，避免所有账号都生成“自然、随意、真实”这类无效描述。
- 系统设置必须提供“账号表达卡”管理入口，支持列表、搜索、查看、编辑、重建、停用、版本回滚和审计查看；搜索至少覆盖账号展示名、username、手机号后四位、账号状态、表达卡状态和更新时间。
- 系统设置必须支持批量重建表达卡、批量停用 / 恢复表达卡、筛选缺表达卡账号，并展示批量生成结果的差异度和失败原因。
- Prompt 中只传一行短摘要，例如“短句、爱追问、少表情、轻微吐槽、避免绝对判断”，不能传完整历史。
- 表达卡可以包含人设设定和经历 / 消费经历设定，但不能要求账号冒充真实用户、管理员、认证身份或某个具体自然人；不能包含需要事实证明的身份背书。
- 表达卡必须以数据库为事实源，支持人工查看、编辑、版本回滚和审计；Redis 只能缓存 `short_prompt_summary` 等热读字段，缓存失效后必须能从数据库恢复。
- 表达卡编辑保存后下一轮规划生效；已经生成且带 `account_voice_profile_version` 的 action 必须继续使用 payload 中固化的旧表达卡版本，不能被后台编辑中途改写。存量缺表达卡账号补齐为 active 后，仍处于 `pending/claiming` 且没有表达卡版本的旧 AI 活跃群 action 必须显式跳过并标记 `voice_profile_replan`，对应消息记忆标为 `expired_before_send`，等待下一轮用新表达卡重新生成。

表达卡初始化提示词必须满足：

- 每张卡必须输出结构化字段，不能只给“自然、随意、真实、像真人”这类泛化词。
- 同批账号必须显式拉开差异，覆盖不同句长、互动习惯、语气强度、口头习惯、表情倾向、年龄段和消费经历设定。
- 每张卡必须给出 3-5 条可执行表达原则和 3-5 条禁用表达。
- 每张卡必须生成一行 `short_prompt_summary`，供 Planner 低 token 注入。
- 生成后必须做差异度检查；同批表达卡过于相似时，失败项重新生成，不能直接启用。
- 表达卡内容不得引用 AI 已发送消息作为正向样本，避免账号人设被 AI 自我污染。
...
| `detail_follow` | 围绕当前对象补一个小角度 | 必须贴当前讨论对象，不能编表达卡或上下文之外的具体经历 |
...
Redis 不允许作为唯一存储保存长期账号人设、表达卡版本、人工编辑结果、审计记录、消息记忆事实或短期立场事实。

轻口语化必须可控：

- 允许短句、省略、轻微停顿、自然口语词和少量半句话。
- 不允许为了像真人而批量制造错别字、乱码、低智表达、夸张情绪或表达卡 / 上下文之外的虚假经历。
...
- 每个 slot 只传短上下文、短表达卡、短画像摘要和必要禁用样例。
...
- Tab 3：AI 轮次，展示 Cycle / Turn、请求条数、AI 返回条数、质量过滤条数、最终计划条数、上下文、账号记忆、账号表达卡版本 / 摘要 / 匹配分、短期立场、消息记忆 ID、语义簇和行为类型。
...
| `ai_account_voice_profiles` | `tenant_id`、`account_id`、`version`、`age_band`、`persona_experiences`、`consumption_experiences`、`sentence_length`、`interaction_habits`、`tone_strength`、`lexical_preferences`、`emoji_policy`、`forbidden_expressions`、`short_prompt_summary`、`source`、`status`、`similarity_score`、`quality_status`、`last_rebuilt_at`、`updated_by`、`updated_at` | AI 活跃群账号表达卡事实源；同一账号在所有 AI 活跃群中使用同一张全局表达卡，按账号保存长期真人感原则、年龄段、经历 / 消费经历设定和口气差异；支持系统设置中搜索、查看、编辑、批量生成、批量重建、停用、版本回滚和审计；Redis 只能缓存短摘要，不得作为唯一存储；表达卡不得冒充真实用户、管理员或具体自然人 |
...
- 长时间没有可接真人消息且任务允许空闲续聊时进入低频暖场模式。暖场只允许少量账号抛轻量话题或延续任务主题，不能编造表达卡和任务上下文之外的具体经历、位置、回访、准点、穿着、服务过程等事实。
...
- Planner 启动本轮前先读取 ready pool、`tg_account_online_state`、账号表达卡、账号群内短期立场、最近真人上下文、同任务历史成功消息和 `ai_group_message_memory`。任一账号在线状态为 `stale`、`offline`、`login_required`、`session_invalid`、`proxy_failed` 时，不得进入文本生成候选；任一账号缺少 active 账号表达卡或表达卡短摘要时，也不得进入 AI 活群生成候选，必须记录 `voice_profile_missing` 并等待表达卡初始化 / 重建完成。
...
- 质量管线的所有决策都必须进入任务详情漏斗：候选数、AI 调用轮次、补位次数、重复命中窗口、模板壳命中、表达卡低分、立场冲突、在线状态剔除、最终 action 数和降级表情数。
...
- AI 活跃群生产质量诊断还必须检查近 24 小时有效发送 action 的真人感 payload 完整性；仍可能发送或已经作为质量样本的 action 缺少 `account_voice_profile_version`、`ai_message_memory_id`、`human_quality_decision`、`generation_source` 或 `act_type` 时，必须输出 `AI_GROUP_QUALITY_PAYLOAD_GATE_FAILED` 并阻断发布，避免 TG bot / Web 配置未真实进入 AI 讨论链路却被误报为完成。
...
- AI 活跃群默认保持一轮批量生成；引入账号表达卡和消息行为规划后，不得退化为每条消息一次 AI 请求。局部质量失败最多发起两次失败 slot 补位调用，单轮规划最多 3 轮生成，不能整轮无限重试，也不能第四轮以上循环补量。
- 同一轮多账号发言必须覆盖不同 `act_type`、账号表达卡和语义簇；若候选集中出现大比例总结型废话、同类夸赞或无锚点暖场，必须降权、重写或减少发送量。
- 同一账号在同一目标群内的短期立场必须连续；缺少新上下文时，不能在 24 小时窗口内从观望、质疑突然切换为强肯定。
- 账号表达卡和账号群内短期立场必须以数据库为事实源；Redis 缓存清空、过期或不可用时，系统必须从数据库恢复短摘要和立场，不得随机重建或静默降级为统一口气。
- 系统设置必须提供账号表达卡管理入口，至少支持搜索、查看、编辑、重建、停用、版本回滚和审计查看；同一账号的表达卡修改必须影响该账号参与的所有 AI 活跃群任务。
- 账号初始化必须批量生成账号表达卡；缺表达卡账号在系统设置中必须可筛选并可批量补齐 / 批量重建。缺表达卡账号即使在线且具备目标发言权限，也不能参与 AI 活群文本生成或表情兜底，避免新增账号退化成统一口气。
- 账号表达卡初始化提示词必须输出结构化字段、可执行表达原则和禁用表达；生成协议采用每账号一行紧凑 JSONL，服务端保留旧 pipe 行解析兼容；批量结构化输出格式错误时拆成单账号继续请求真实 AI，单账号仍不合格则暴露失败；禁止只生成“自然、随意、真实”等泛化描述；同批表达卡差异度不足时不得启用。批量补齐 / 批量重建必须返回逐账号结果明细，包含生成状态、版本、差异度、跳过原因和失败原因；批量重建已有 active 表达卡时必须创建新版本并将旧版本置为 superseded，不能覆盖旧版本或撞唯一约束。
- 表达卡编辑下一轮规划生效；已生成且带表达卡版本的 action 必须固化旧表达卡版本，不受后续编辑影响。缺表达卡时期生成、尚未发送且没有表达卡版本的 open action 在账号表达卡补齐后必须重排，不能继续按统一口气发送。
- 第三轮生成后仍不合格但账号有本轮活跃要求时，只能显式降级为 `emoji_react` 或极短低风险反应，并记录 `quality_fallback`；该兜底不得计为高质量 AI 文本。普通候选仍必须按账号表达卡校验，但已标记 `quality_fallback=emoji_react` 的低风险表情兜底不得被“少表情 / 不用表情”表达卡规则误杀，否则会破坏账号活跃补齐语义。
