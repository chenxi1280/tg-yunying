...
- 当同一 `all_accounts_daily` 任务启用硬小时目标且当前小时存在 `planning_deficit` 时，日覆盖自然节奏的 `due_debt=0` 不得把本轮可规划数压成 0；Planner 应以本轮 ready 且在线的账号数与硬小时缺口的较小值规划。没有硬小时缺口时仍严格按日覆盖 `due_debt`，两种预算都不得突破单轮 Action 限额。
