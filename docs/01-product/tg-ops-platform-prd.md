...
- Web 任务详情页必须提供 AI 活跃群专项设置区，可查看和保存话题方向、老师和连发模拟配置；创建 / 编辑任务表单必须同步支持同一字段。
- TG bot 必须支持管理员在 bot 内选择 AI 活跃群任务、查看当前设置、编辑话题方向、编辑老师、编辑连发参数并确认保存。Bot 保存必须复用后端任务配置校验，不能绕过 `PATCH /api/tasks/{task_id}/group-ai-chat` 的契约。
