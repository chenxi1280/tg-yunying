...
| AI 活跃群小时硬目标 | 在任务中心说明 AI 活跃群必须启用每小时硬目标，`hourly_min_messages` 最低为 10 条 / 小时；生产质量诊断必须即时重算 running AI 活群硬目标 stats，任一当前小时未达到 `success >= hourly_min_messages` 且 `hard_hourly_status=met` 时以 `AI_GROUP_QUALITY_HARD_HOURLY_GATE_FAILED` 阻断验收；专项口径以 `docs/03-feature-designs/ai-group-hard-hourly-target-prd.md` 和 `docs/04-ops/ai-group-hard-hourly-target-ops.md` 为准 |
