...
- AI 活跃群选择“全部可用账号”时默认启用 `全账号日覆盖模式`。系统必须在任务配置的 24 小时窗口内尽量让当前账号范围内每个已准入且可发言账号完成 1-2 条 `send_message`；该模式只改变账号覆盖目标和跨轮排程优先级，不新增任务类型，不突破小时硬上限、账号容量、准入状态、风控和 AI 内容质量过滤。
...
- `全账号日覆盖模式` 属于 `group_ai_chat` 的配置模式，不是新的任务类型。任务账号范围选择“全部可用账号”时必须默认启用该模式，旧 all 账号任务即使存量配置仍为 `natural`，运行和统计也按全账号日覆盖有效口径处理；新录入平台且满足 active、normal、有 session、未禁用 / 未封禁 / 未删除、目标群已准入可发言的账号，必须在下一轮账号范围扫描、在线保活和覆盖统计中自动纳入，不要求运营手工追加到任务配置。该模式默认每个账号每日最少 1 条、最多 2 条，统计窗口使用任务的当日 24 小时运行窗口。Planner 每轮优先选择 `coverage_remaining > 0`、已准入、可发言、冷却已过且健康状态可用的账号；已达到上限的账号只在补量仍有小时预算且用户允许重复时参与普通 AI 活群分配。
