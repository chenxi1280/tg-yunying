...
- `fact_first_v3` AI 正文生成前必须按当前 `task_id + account_id + target_group_id` 读取/推进 `TaskGroupBotAdmission`；旧 `GroupBotAdmission` 和 Action 内旧 `group_bot_admission_state` 不能作为新 Task 的准入门。C2 未 ready 时立即释放生成 claim 且不得读取 Provider 凭据，ready 后固化 Task admission id/version 再生成；不可发送账号只在当前 Task 内放弃。
