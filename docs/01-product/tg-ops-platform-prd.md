...
- TG bot 是租户级能力，必须先在 Web 系统设置中完成 Bot Token、管理员 Chat ID 和 AI 活群 Bot 设置开关配置；Bot Token 保存后不得明文回显，配置和测试发送必须有明确成功/失败状态。
- Web 任务详情页必须提供 AI 活跃群专项设置区，可查看和保存话题方向、老师和连发模拟配置；创建 / 编辑任务表单必须同步支持同一字段。话题和老师应以结构化增删改控件为主，JSON 只作为高级编辑方式。
- TG bot 必须支持管理员在 bot 内通过按钮选择 AI 活跃群任务、查看当前设置、编辑话题方向、编辑老师、编辑连发参数并确认保存；不得要求管理员手写完整 JSON。Bot 保存必须复用后端任务配置校验，不能绕过 `PATCH /api/tasks/{task_id}/group-ai-chat` 的契约。
- TG bot 入站必须通过租户级 webhook secret 路由到对应运营空间，不能依赖 Telegram update 体内携带业务 `tenant_id`。非管理员 Chat ID、未启用 AI 活群 Bot 设置、任务不存在或配置非法时必须显式拒绝。
