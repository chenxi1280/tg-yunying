...
- **2026-08-03 准入与数量槽收敛 supersede：** 正文 Planner 不得在无 ready 账号时用 `admission_waiting` 回填正文候选；非 ready 账号只保留 coverage 分母和 `pending_group_bot_admission` blocker，由独立 observation/follow/confirmation/probe lane 推进。ContentMix 冻结必须在 target-day/数量槽锁内按精确 coverage 身份对齐；旧 replan 槽等待不阻塞与其 coverage、数量槽均无交集的新 Cycle，但同一主数量槽永不复用。槽位变化、旧 Cycle 绑定、extra 不足和数据不变量损坏必须分码展示，禁止继续用 `quantity_slots_unavailable` 或捕获全部 `ValueError` 掩盖。完整合同见 `docs/03-feature-designs/ai-group-admission-quantity-slot-convergence-prd.md`。
