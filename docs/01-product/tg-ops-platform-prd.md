...
- 登录设备清理口径调整为“只保留 primary session、standby_1 session、standby_2 session 对应的我们的 TG 应用设备；普通运营账号一键清理所有非我们的 TG 应用设备”。Telegram 官方手机端、桌面端、网页版和未知 API 客户端均属于非平台设备；接码专用账号禁止自动清理其他登录设备，只展示风险和明细。
...
- “我们的 TG 应用”只按 Telegram 远端授权会话的 `api_id` 绑定识别。远端会话的 `api_id` 必须等于平台保存的 `primary`、`standby_1` 或 `standby_2` 授权资产所关联 `telegram_developer_apps.api_id`，才算我们的 TG 应用授权会话。
- 如果远端授权会话 `api_id` 不在账号三槽位授权资产的 `api_id` 集合内，一律算“非我们的 TG 应用设备”，包括 Telegram 官方手机端、桌面端、网页版、历史废弃平台授权和未知 API 客户端。
- 如果远端授权会话 `api_id` 命中多个槽位使用的同一个 Developer App，设备分类仍为“我们的 TG 应用设备”，槽位映射展示为“api_id 匹配多个槽位”；该设备必须保留，不允许被非平台设备清理。
