...
- AI 活跃群必须在 action payload / result 中记录 `slot_id`、`act_type`、上下文锚点、账号表达卡版本、账号表达卡摘要、短期立场摘要、重写次数和真人感过滤结论。
...
- 第三轮生成后仍不合格但账号有本轮活跃要求时，只能显式降级为 `emoji_react` 或极短低风险反应，并记录 `quality_fallback`；该兜底不得计为高质量 AI 文本。
