...
> 2026-07-20 生效说明：上表“搜索目标群点击任务”中的账号上限、跳过和抖动字段仅描述系统策略与历史任务的已保存事实；`search_join_group` 与 `search_rank_deboost` 的新建页及新建 API 一律只接收**单个目标群、搜索关键词、目标次数**。账号、账号组、代理、机器人、节奏、停留、配额、重试和资源准备均为系统托管字段，不得再作为创建控件或新建请求参数。
...
`search_join_group` 与 `search_rank_deboost` 是例外：二者使用“任务类型 → 目标群 → 关键词与目标次数 → 确认”的四步极简创建向导，只提交单个目标群、关键词和目标次数；下表的五步字段不适用于这两类任务的新建。
...
| 任务创建向导 | 普通任务保留基础信息、目标来源、任务配置、账号与节奏、预检确认；`search_join_group` 与 `search_rank_deboost` 固定为任务类型、目标群、关键词与目标次数、确认，创建时由服务端校验系统资源和 blocker，不开放策略调参。 |
