...
- 2026-07-31 生产只读评估新增「优化 AI 活群真人话术待修复」范围：`chat_mode` 单一真相源、collective should-speak、租户全局 persona 区分度/AI self-history 消融、真人校准 selector、reply relation、静默 holdout、真人节奏指纹、实际成稿趋同监测与 single-message late binding。固定连续条数 cap 已被生产回放否决；当前 batch=10/共享上下文刷新/一次多 slot 输出仅有静态合同证据，节奏、实际风格和 late-binding 效果测试受生产 ingress 超时阻断。以上项目在专项 PRD §14 标记为 `online_shadow_in_progress|pending|blocked`，不得借用 2026-07-27 既有 `design_status=complete` 直接开发或宣称上线。
