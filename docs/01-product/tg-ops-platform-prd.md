...
> 2026-07-20 生效说明：上表“搜索目标群点击任务”中的账号上限、跳过和抖动字段仅描述系统策略与历史任务的已保存事实；`search_join_group` 与 `search_rank_deboost` 的新建页及新建 API 一律只接收**目标群（完整名称 + 公开 Telegram 链接）、搜索关键词、目标次数**。账号、账号组、代理、机器人、节奏、停留、配额、重试和资源准备均为系统托管字段，不得再作为创建控件或新建请求参数。目标链接必须可归一化为公开 username；邀请链接、peer id 和内部 `OperationTarget` ID 不能作为这两类任务的输入。
...
新建操作固定为“任务类型 → 目标群 → 关键词与目标次数 → 确认”。`search_join_group` 使用“创建并启动”，`search_rank_deboost` 使用“创建草稿”；二者都不得要求运营补填账号、代理、机器人或节奏。服务端负责检查公开 username、候选账号、代理/授权绑定、协议样本和配额，失败时返回可读 blocker 并在任务详情展示事实。
...
- `search_join_group` 与 `search_rank_deboost` 新建页和新建 API 只接收目标群、搜索关键词、目标次数；名称、账号/账号组、代理、机器人、节奏、停留、配额、重试和风险参数均由系统托管，调用方传入这些系统字段必须显式拒绝。`target_count` 是任务生命周期的已确认成功数；open/unknown action 占槽，失败/跳过释放槽，达到目标后写 `completed + target_count_reached`。历史无 `target_count` 任务保持原行为。
...
| 任务创建向导 | 普通任务保留基础信息、目标来源、任务配置、账号与节奏、预检确认；`search_join_group` 与 `search_rank_deboost` 固定为任务类型、目标群、关键词与目标次数、确认，目标群填写完整名称和公开 Telegram 链接，创建时由服务端校验系统资源和 blocker，不开放内部目标选择或策略调参。 |
...
- 任务创建继续收敛为快速创建 + 高级设置折叠；`search_join_group` 与 `search_rank_deboost` 例外为三字段极简创建，真实协议样本、账号/代理环境、observed exit IP、账号互斥、warmup、关键词矩阵、配额、停留和不调用 LLM 的节奏策略由系统托管并作为预检 blocker/事实展示，不能回退为创建者可调参数。
