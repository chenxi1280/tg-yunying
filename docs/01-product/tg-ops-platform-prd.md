...
- `search_join_group` 新建及专用编辑 API 接收：目标群（`target_title` + `target_link`）、搜索关键词、每日已确认目标 `daily_target_count`、单个普通 `account_group_id`、任务每日 action 硬上限 `max_actions_per_day`、完成截止时间 `scheduled_end`、日/小时抖动 `daily_jitter_percent` / `hourly_jitter_percent` 与可选 `quiet_hours`。source `search_join` 命中目标后必须创建唯一 `search_join_membership` 准入子 action；子 action 继承 source 的授权槽位、开发者应用、代理绑定和客户端 metadata，禁止回退账号主 session。申请待审批时，若本租户已配置该目标群的救援管理员，可由管理员以自身 session 审批，再立即用 source 授权槽位复核成员关系；审批成功本身不能计入完成。否则子 action 只做成员复核，不重复提交；同任务、同账号、同目标的未决申请跨自然日持续阻断重复申请。每日目标按任务时区自然日统计经成员关系复核的 `membership_observed`；`pending`、`claiming`、`executing`、`unknown_after_send` 和 source `membership_pending` 占当日待确认槽位，失败/跳过释放槽位。成功应按 `membership_observed_at` 的本地日期计数。Planner 将尚未确认的每日目标按当天剩余 `hourly_round_curve` 权重折算为当前小时需求，并与既有 `hourly_min_successful_joins` 取较高值；每轮、每小时、账号、关键词和代理风控上限仍不得突破。达到当天目标只停止当天继续规划，不得把任务写为 `completed`；次日按新的自然日重新计算。`max_actions_per_day` 是可创建/占用 source search action 的硬预算，必须不小于 `daily_target_count`，两者不得混用。完成只由 `scheduled_end` 或运营显式停止触发；每日任务因旧 `scheduled_end` 进入 `completed` 后，运营将截止时间改为未来时必须重新入队，避免既不能继续当天补量、也不能在次日重新计算每日目标。
...
- 只处理 `search_join_group` 任务，按账号授权槽位、搜索机器人、关键词、目标群匹配策略生成 source `action_type=search_join`；命中目标后由 Dispatcher 创建 `action_type=search_join_membership`。后者不是普通账号准入 action，必须复用 source 的 scoped runtime authorization，并在成员关系观察成功后回写 source、刷新日目标和创建后续任务联动。
