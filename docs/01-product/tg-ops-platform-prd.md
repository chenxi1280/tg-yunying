...
| `account_environment.manage` | 按账号 + TG 开发者应用 + 授权槽位配置代理节点和授权指纹 | 保存、解绑、批量修改、重排代理、修改指纹、刷新观测状态都必须写审计；缺代理或指纹时任务执行必须 fail closed，不得回退账号默认代理或本机直连 |
...
| `tasks.create.search_join_group` | 创建和启动搜索目标群点击任务 | 只对 `search_join_group` 生效；必须通过授权槽位环境栈、decoy 占比、代理健康和灰度账号数预检 |
...
| `system.manage` | 平台运行配置、素材运行配置、全局 Clash 订阅配置、后台账号权限入口 | 影响全局运行的配置必须健康检查或回滚提示；Clash 订阅地址必须加密保存，普通日志和前端列表只展示脱敏摘要 |
...
### 2.8 账号面具、Clash 和授权环境入口更新

- “账号面具”升级为一级菜单，承载账号对外表现和授权环境配置。一级菜单下按 Tab 分为“面具管理 / 账号代理 / 授权指纹 / 异常与审计”，不再放在系统设置里。
- 系统设置新增“Clash 配置”Tab，只保存一个全局 Clash 订阅地址 / 接口配置，作为平台级代理节点来源。系统设置支持保存、连通性测试、同步节点、显示最近同步时间、节点数量和失败原因；不在系统设置里给单个账号分配代理。
- 全局 Clash 订阅地址必须加密保存。前端列表、审计摘要、任务 stats 和普通日志只能展示脱敏摘要、节点数量、同步状态和失败原因，不得输出完整 URL、token、节点密码或 URI 原文。
- 单个账号代理配置在“账号面具 > 账号代理”完成，绑定粒度为 `account_id + developer_app_id/api_id + authorization_role`。同一账号在不同 TG 开发者应用 `api_id/api_hash` 下可以使用不同代理；主授权和备用授权必须使用不同代理节点。
- 授权指纹配置在“账号面具 > 授权指纹”完成，绑定粒度同样为 `account_id + developer_app_id/api_id + authorization_role`。同一账号在不同 TG 开发者应用和不同授权槽位下可以使用不同客户端元数据；缺失或重复时 search_join 等真实执行任务必须 fail closed。
...
- “账号代理”按 `account_id + developer_app_id/api_id + authorization_role(primary/standby_1/standby_2)` 绑定代理节点。不同 TG 开发者应用 `api_id/api_hash` 下，同一账号可以绑定不同代理节点；同一账号的主 / 备用授权槽位不得复用同一个代理节点。账号代理配置入口在“账号面具”菜单内，系统设置只保存全局 Clash 订阅地址，不负责单账号代理分配。
- “授权指纹”按 `account_id + developer_app_id/api_id + authorization_role(primary/standby_1/standby_2)` 绑定客户端元数据，字段至少包含 `platform/device_model/system_version/app_version/lang_code/system_lang_code/lang_pack/region_code/client_identity_key`。不同 TG 开发者应用和不同授权槽位可以拥有不同指纹；同一账号的主 / 备用授权槽位不得复用同一 `client_identity_key` 或同一 `device_model + system_version + app_version` 组合。
...
- “授权指纹”必须同时展示“配置指纹”和“远端观测指纹”，并计算一致性状态：`not_connected`（未连接）、`pending_effect`（待生效）、`observed_matched`（已观测一致）、`observed_mismatch`（观测不一致）。不一致时提示运营重登 / 刷新授权 / 人工检查，不能静默改写现有 session。
- “异常与审计”展示代理缺失、指纹缺失、同应用多账号指纹重复、主备授权复用代理、配置与远端观测不一致、最近保存 / 解绑 / 重建 / 刷新操作。所有写操作必须记录操作者、原因、影响账号、TG 开发者应用、授权槽位、旧值摘要和新值摘要。
...
| `proxy_airport_nodes` | `subscription_id`、`node_key`、`name`、`protocol`、`host`、`port`、`country`、`region`、`asn`、`isp`、`status`、`observed_exit_ip`、`last_health_check_at`、`capacity_limit` | 由全局 Clash 订阅同步出的标准节点池；节点入口 host 不等于真实出口 IP，必须通过健康检查观测出口事实后才能绑定账号授权槽位 |
| `account_environment_bindings` | `tenant_id`、`account_id`、`authorization_id`、`session_role`、`developer_app_id`、`developer_app_api_id_snapshot`、`proxy_binding_id`、`proxy_id`、`device_model`、`system_version`、`app_version`、`platform`、`lang_code`、`system_lang_code`、`lang_pack`、`region_code`、`client_identity_key`、`fingerprint_locked`、`status` | 账号授权环境绑定；绑定粒度是账号 + TG 开发者应用 + 授权槽位。不同应用和不同槽位可以绑定不同代理节点与客户端元数据；search_join 等真实执行任务按本表取环境，缺失或冲突时 fail closed |
...
| `proxy_airport_nodes` | `subscription_id`、`node_id`、`node_name`、`protocol`、`uri_scheme`、`source_format`、`proxy_host`、`proxy_port`、`node_capacity`、`assigned_authorization_count`、`failover_rank`、`consecutive_failures`、`observed_exit_ip`、`observed_exit_country`、`observed_exit_asn`、`observed_exit_isp`、`exit_ip_stability_score`、`latency_ms`、`health_score`、`is_active` | 订阅解析出的代理节点池；过滤套餐/流量伪节点，节点入口只作为连接配置，风控以真实出口 IP 观测为准，节点按容量随机分配并固定到账号授权槽位，出口漂移、健康分低于阈值或节点不通时触发故障切换或暂停 |
| `proxy_node_failover_events` | `subscription_id`、`account_id`、`authorization_id`、`session_role`、`from_node_id`、`to_node_id`、`reason`、`outcome`、`observed_error`、`admin_notification_status`、`admin_notification_detail`、`admin_notified_at`、`triggered_at` | 搜索目标群点击任务机场节点故障切换审计；当前节点不通时按 `switch_to_next_healthy_node` 切到下一个健康节点，全订阅不可用时记录 `airport_all_nodes_unavailable`、推送管理员通知并停止真实操作 |
...
| `tg_account_proxy_bindings` | `account_id`、`developer_app_id`、`developer_app_api_id_snapshot`、`authorization_id`、`session_role`、`proxy_node_id`、`proxy_provider`、`proxy_type`、`proxy_host`、`proxy_country`、`proxy_asn`、`ip_reputation_score`、`last_health_check_at`、`last_failover_at`、`binding_generation`、`is_active` | 授权槽位代理镜像绑定；绑定粒度是账号 + TG 开发者应用 + 授权槽位。首版要求独享静态住宅 IP 或 `airport_clash` 健康节点，同账号主/备用授权不得复用节点，节点故障切换后递增绑定代际并重新 warmup |
| `account_environment_bindings` | `account_id`、`developer_app_id`、`developer_app_api_id_snapshot`、`authorization_id`、`session_role`、`proxy_binding_id`、`device_model`、`system_version`、`app_version`、`platform`、`lang_code`、`system_lang_code`、`region_code`、`client_identity_key`、`fingerprint_locked`、`health_score` | 授权槽位环境栈；绑定粒度是账号 + TG 开发者应用 + 授权槽位。MTProto 客户端元数据按配置在下一次连接 / 重登 / 新 session 初始化时生效；主/备用授权不得复用客户端元数据组合 |
| `account_proxy_warmup_states` | `account_id`、`authorization_id`、`session_role`、`proxy_binding_id`、`stage`、`stage_started_at`、`first_action_at`、`daily_actions_count`、`total_actions`、`reset_reason` | 搜索目标群点击任务 `(账号, 授权槽位, 代理)` 三元组 warmup 进度；换代理节点后重新进入 warmup |
...
- 创建 action 前必须实时校验真实协议样本、`execution_mode=mtproto_userbot`、授权槽位环境栈、代理绑定、代理健康、observed exit IP、客户端元数据镜像绑定、关键词允许矩阵和 `(account_id, developer_app_id/api_id, authorization_id/session_role, proxy_binding_id)` warmup 阶段。
...
- 授权环境绑定的权威粒度是 `account_id + developer_app_id/api_id + authorization_id/session_role`。同一账号在不同 TG 开发者应用 `api_id/api_hash` 下可以绑定不同代理节点和不同客户端元数据；Executor 必须使用授权槽位登录时绑定的同一 TG 开发者应用，不能用另一个应用的代理或指纹配置代替。
