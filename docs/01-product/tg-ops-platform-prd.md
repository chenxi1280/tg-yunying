...
- “授权指纹”必须同时展示“配置指纹”和“远端观测指纹”，并计算一致性状态：`not_connected`（未连接）、`pending_effect`（待生效）、`observed_matched`（已观测一致）、`observed_mismatch`（观测不一致）。不一致时提示运营重登 / 刷新授权 / 人工检查，不能静默改写现有 session，也不能把“配置已保存”当成“远端已观测一致”。
...
- Clash 配置：只保存一个全局 Clash 订阅地址 / 接口配置，展示脱敏订阅名、最近同步时间、节点总数、健康节点数和失败原因；保存、测试和同步写审计，不能在系统设置里分配账号或授权槽位代理。
...
| 搜索入群环境与结果 | `bot_protocol_samples`、`proxy_airport_subscriptions`、`proxy_airport_nodes`、`proxy_exit_ip_observations`、`account_proxy_bindings`、`account_environment_bindings`、`account_proxy_warmup_states`、`fingerprint_combo_history`、`account_authorization_execution_locks`、`ip_reputation_history`、`search_join_action_stats` |
