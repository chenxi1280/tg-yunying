...
> **2026-09-03 互动执行所有权终审（设计完成 / 未实施）：** response 日计划不再预填未来真人 `planned_call_at`，只冻结 capacity window 与不重叠的 tentative supply。公共 `TurnIntentClassifier` 对每个 canonical turn revision 最多一次分类调用；各 Task 只应用自己的 participation policy。唯一 owner 后先冻结 natural window，再在其与 compatible account/relation slot、Timeline legal interval 的交集中原子建立 `InteractionServiceBinding + planned_call + effective reservation`；交集为空记 admitted capacity miss，不拉动未来 slot。每 binding 最多两次 response Provider 调用，数量义务 pre-Gateway 归还后的 successor 继续扣同一 Task/source-plan 总 binding/call budget，旧调用、unknown 和成本不清零。materialization horizon、generation latest-safe、response release cutoff 与 protected slack 全部从冻结 `ExecutionTimingProfileRevision + path-start stage` 派生，缺 profile 时 unified route 不激活。
