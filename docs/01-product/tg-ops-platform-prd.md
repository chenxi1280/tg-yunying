...
- `search_join_group` 与 `search_rank_deboost` 新建及专用编辑 API 接收：目标群（`target_title` + `target_link`）、搜索关键词、任务总目标 `target_count`、单个 `account_group_id`、任务每日执行上限 `max_actions_per_day`、完成截止时间 `scheduled_end`、日/小时抖动 `daily_jitter_percent` / `hourly_jitter_percent` 与可选 `quiet_hours`。`target_count` 是任务生命周期的已确认成功数，`max_actions_per_day` 是自然日内可创建/占用的 action 预算，二者不得混用。普通搜索只能选择启用的普通账号组；搜索排名观察只能选择启用的 `pool_purpose=rank_deboost` 黑账号组。代理、机器人、授权环境、单账号安全上限、停留、重试和风险参数继续由系统托管，调用方传入这些系统字段必须显式拒绝。open/unknown action 占槽，失败/跳过按各任务事实语义释放槽，达到目标后写 `completed + target_count_reached`；历史无 `target_count` 任务保持原行为。
