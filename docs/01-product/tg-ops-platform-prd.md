...
> **2026-09-08 完成量与全操作设计修复：** 统一引擎§19.65、频道成员设计§16、群成员恢复设计§14为本轮当前合同：独立任务分别履约、真实时间容量、随机小窗后合法重排、全操作节奏及逐账号准入/救活。只修订PRD，`design_status=complete`、`resync=true`、`implementation_status=not_started_for_this_slice`、`production_status=unproven`。原切片的实现/发布记录不证明本修订已生效。 本轮完成量复核进一步同步统一§7.8/§11/§14/§16的容量不足处理，移除默认每账号5～10条和组合欠量全局禁用；剩余路径含router时按实际步骤计准备量。合同标识/能力状态见§19.65.9，完整周期完成量验收见§19.65.10。下方旧版本的P95/预算平台、随机小时过期即永久短缺与组合不可激活表述仅作历史记录，不得用于当前实现。
...
> **2026-09-03 unified 生命周期、时间与退避唯一合同（Product Design Complete / 未实施）：** 四类 unified Task 共用 append-only start/update/pause/resume/stop/delete 与配置 successor；当前 task-day/source plan 的目标、selected、due、deadline 不因编辑、恢复或运行时故障重写。日目标调小只对下一完整任务日生效，立即停止必须记 `terminated_by_operator`，不能把欠量改成 completed；resume 只恢复仍在原窗口的同一义务，不建立新 anchor、不追赶。current task-day 固定 `Asia/Shanghai`，其他 timezone 写入 typed 拒绝，legacy 时区只在旧 period 完整结束后无重叠接管。同 adapter 且 source predicate 有重叠的 canonical target 只能一个 quantity writer；所有 active/proposed Task 必须做跨 account/peer/source/Provider/Gateway portfolio feasibility，不能各自单算后超卖。FloodWait/SlowMode 使用结构化 authorization/session/peer transport availability，不打开错误 circuit、不暂停整 Task、不改原 due；manual retry 只接受 safely-not-called 且原窗口仍有效的同一 obligation，不存在 force-send-now。互动 peer 使用 primary/standby observer、epoch fencing 与 gap closure；无 caption 媒体、未批准语音转写、forward origin/语言不确定 turn 明确不参与，禁止万能接话。活群默认 40% response reserve；只有连续 7 个 observer-complete 日证明 `avg_daily_human_turns<5` 才按冻结公式降为 15%～20%，日内不重算、不降低 candidate/服务目标；未用 reserve 的释放提前量为版本化 `max(15 分钟 floor, 完整剩余链 P95 + quiet P95 + margin)`。上述 current 合同优先于后文 generic `next_run_at=now`、任意 timezone PATCH、同 Action retry、静态/图片 fallback 和运行时换号描述。

> **2026-09-03 远端在途 fence 补正（Product Design Complete / 未实施）：** unified engine 将可归还的本地 `ExecutionBulkheadLease(worker|stage|fair_share)` 与 durable `RemoteInvocationFence` 分开。5/10/15 秒 hard timeout 在 typed 状态持久化后只立即释放本地 Worker；尚未由当前隔离 runner termination acknowledgement 或同 invocation 权威终态证明结束的 Telethon/Provider transport，继续按 account/group/proxy route/verified egress 或 Provider route/lane 计 remote hard in-flight。TTL、Worker 重启、Future timeout、cancel-requested 均不能释放该占用。Telegram 已 call-issued 即使 transport 后续终止，业务 identity 仍保持 unknown/dedupe 直到 reconcile，防止超时后绕过并发上限或重复发送。

> **2026-09-03 群聊真人原生回复权威补正（Product Design Complete / 未实施）：** unified AI 活群 response 支持 `semantic_direct|native_reply_external_human|native_reply_owned_fact` 三类显式关系。external-human 只接受 canonical stream 中同 tenant/peer/thread/topic、精确 remote message/revision、作者确认为外部真人、current 未删除且 gap 已闭合的事件；owned 继续只接受同 Task/群的 bound typed remote fact。每个 response binding 冻结唯一 `ConversationReplyAuthorityDecision`，Provider、Action 与 Gateway call-issued 前复用同一 decision/hash；旧上下文行、昵称、正文或 Action.result 不能授权。一个真人 turn 跨 Task 仍最多一个平台账号响应，Gateway call-issued/unknown 后不能换 target 或让其他账号补答。该合同优先于旧 own-only reply pool 口径。

> **2026-09-03 分组、出口与比例口径补正（Product Design Complete / 未实施）：** legacy `all` 的迁移比较对象是旧路径与全部兼容 normal groups 在同一稳定资格规则下得到的 `policy_eligible` account-id set/hash，不是原始成员数，更不能假定默认组等于全租户。需要代理的 allocation 同时检查 current binding route 与 verified exit identity，并在 durable remote invocation fence 中同时占用 route/egress hard in-flight；不同 proxy IDs 指向同一出口仍共享默认并发上限。mask generation、turn classification、interaction response、proactive/reviewer 子泳道共同服从 Provider 父真配额，response/classification 保留容量。浏览 sampled ratio 与整数 realized ratio 分列，小账号组的量化偏差必须可见。

> **2026-09-04 账号分组、工业韧性与 JIT 终审（Product Design Complete / 未实施，优先于下方隐式 all/全账号动态健康分母口径）：** 四类 unified Task 必须显式绑定 1..N 个 enabled、用途一致的普通运营账号分组并冻结 binding-set/member/group-state revisions、成员并集与 origin group；接码/搜索降权等专用组拒绝绑定，legacy `all` 只有在全部兼容组并集 set/hash 精确等于旧 operational scope 时才能切换，不能假定默认组等于全租户。configured、policy eligible、planning admissible、task-day selected、runtime admitted/sendable、typed-fact confirmed 六层分账；规划健康不缩小业务分母，部分可服务从启动起显示 `running_partial` 并让健康分区继续。参与模式由 adapter 拥有：活群按群每日全员，评论按任务日全员并在单帖 55%～65% 内跨来源轮转，点赞按每来源数量选 distinct accounts，浏览每天冻结 80%～95% cohort 后再冻结账号—来源自然子集，默认每号 2～4 条而非逐帖全刷。任务日基础量允许 `daily_target_jitter_bps=0..3000` 的稳定波动，故障不能触发重抽。中途启动来源只有在剩余 strata 能容纳其冻结 exposure 时纳入当日，否则 `pending_first_full_day`。运行时再次检查 group state、Session、account/proxy circuit、quarantine、target access/capability 与类型依赖；面具/Provider 不进入点赞和浏览。connect/Telegram RPC/LLM hard ceiling 分别为 5/10/15 秒，call-issued timeout 必须 unknown/reconcile。舱壁和断路器只限制影响面；`ResilienceTopologyPlanRevision` 还必须证明发送 egress、Provider 与 Observer 的单故障域失效后仍有可服务路径，不能让全部账号共享一个会拖停全局的出口。互动内容按完整剩余链 P95+margin 倒推 JIT，冻结 active turn/reply chain 后取最新 10～20 条相关消息，并在 Gateway call-issued 前不超过 1 秒做 revision/CAS 终审。

> **2026-09-03 四类任务统一履约引擎与拟人化高互动核心（Product Design Complete / 未实施）：** 四类任务共用“任务专用义务真相 + obligation projection + 稳定 pacing slot + account/peer/conversation/source-message 多级时间线 + JIT materialization + immutable ready Action + Attempt/Gateway journal + 类型化事实结算”，但 workload 分层：AI 活群、频道评论是 `interactive_content`；点赞、浏览是 `passive_operation`，不读取 ContextTurn、不调用 LLM、不做语义回复。群聊和 linked discussion update 复用唯一 authorization update ingress，经单 owner cursor 原子写 ConversationEvent/outbox；各 stage 用 durable wake 低延迟推进，普通轮询只补 gap/恢复。互动型任务先冻结 eligible subscription set，再按策略形成 participation candidate，以 `ConversationTurnClaim` 保证同 tenant、同会话、同真人 turn 跨 Task 只有一个响应 owner；当前账号/slot/Provider 容量只能在 owner admitted 后影响 served/missed。仍在真人连续表达或等待真人回答时 admission 前进入 deferred-wait；planned call 前真人已解决可记正确沉默，容量或 Provider 迟到不能伪装。既有数量内冻结 response capacity：活群按每群每日 effective target 的约 40% 形成 flexible 总池后稳定抽样到预测有人活动的小时，评论以 30% 为 response 基线且 hard reply=`reply_min` 可抬高总占比；未用柔性容量到 cutoff 后在原窗口释放，禁止超发或改成 now。Timeline 按真人 response、到期互动内容、点赞、浏览做 deadline-aware 仲裁；发送时点读取真人 tempo 并冻结 natural window/planned call，不形成固定秒回指纹。互动 peer 另以 `ConversationAttentionState` 的可重叠 blocker 阻断无关主动内容：活群真人 P90 等待窗限制为 60～180 秒、评论为 180～900 秒，小样本使用上界；typed revision/wake 保证既不抢话也不永久占用。评论真人 turn 只回复真人，自有异号 `owned_peer_followup` 由我方 confirmed fact 和独立 pacing 触发，加入后仍须真人≥80%/owned≤20%，并在 Task/policy 锁内原子 admission；Gateway call-issued 前出现新真人目标必须释放 owned reservation 并归还 hard 义务。每个 Task 的全部选择账号必须取得本任务 typed fact；活群进一步按每个“目标群×账号”覆盖单元计算每群 effective target，评论在单帖 55%～65% 和 Daily Cap 内跨 source 轮转完成任务日逐账号发言覆盖，无适用来源时显式 shortfall；portfolio activity 只展示，不能替代 Task completion。互动型任务还必须满足 stream gap 收口、watermark 新鲜、candidate terminal coverage≥99% 和无双 writer 的 observation integrity，不能在监听失效时借 admitted 零分母完成。高互动用 observed/eligible/ineligible/deferred-wait/candidate/admitted/coalesced/served/validly-superseded/missed 固定漏斗、容量兑现率、上下文、关系、重复和预注册盲评样本验收；真人对我方 confirmed 消息的权威 reply、推断续聊与负向 outcome 另由 `HumanEngagementObservation` 和批准基线比较。实时 response 与主动内容使用分离的 Provider 预算，评论 response 固定一次 realizer 加一次 reviewer；新统一评论 route 是 obligation-first、质量后才建 Action，存量 v1.2 Action-first 只按原 identity 收口。当前 `dev_handoff_ready=true`、`implementation_authorized=false`、`production_status=unproven`。顶层合同见 `docs/03-feature-designs/unified-engagement-fulfillment-engine-prd.md`，AI 与评论适配器分别见 `ai-group-dual-lane-send-chain-redesign-prd.md`、`ai-channel-comment-broadcast-and-teacher-relevance-prd.md`。

> **2026-09-04 拟人互动最终业务补正（Product Design Complete / 未实施）：** 账号“每天 2～4 个 Session”只控制可见动作，不关闭 Telegram 连接或实时 Listener；明确点名只能由 required account 通过有次数、休息债务和 deadline 的 response-only wake。来源旅程为 read-only/reaction/comment/reaction+comment 分支，评论不强迫先点赞；本地 `SourceContentReadEvidence` 不等于远端 view fact。真人碎片消息只由冻结 `BurstAssemblyPolicyRevision` 关闭 turn，AttentionEngine 不另设固定 3 秒。所有 normal 活群/评论正文均经过普通 15 秒、准入/风险 90 秒的 post-send visibility；Observer gap 为 unknown 且禁止替代发送。跨午夜 turn 只有在次日计划完成后通过 `CrossDayConversationCarryover` 新绑次日 obligation 才可发送。无来源先证明 `SourceObservationCompleteness`，再区分 neutral、发布方未发和采集未证明。负反馈须先分类并按版本化阈值、滞回和 scope circuit 处理；单次删除不等于 AI 暴露。点赞必须将内容意图安全集合与 Telegram capability 取交集，`all_available` 不再表示全池等概率随机。规范对象、状态与验收以统一引擎 §19.1 为准。

> **2026-09-04 深层组合业务补正（Product Design Complete / 未实施）：** unified current 进一步以 AccountPool 全局 lease 和逐账号跨 Task 行为预算阻止共享账号组被多个 Task 分别超卖；Task 的分组并发只表示 share ceiling。活群/评论新增自然机会与受管存在感承诺状态，依赖未来真人/来源时只能显示 `forecast_conditional`，并受 peer/source 的绝对上限、managed-to-external ratio 与 consecutive rule 共同约束。同一来源的 view/reaction/comment 账号边由 `CrossAdapterSourceJourneyPlanRevision` 联合求解，禁止各任务独立选出同一批账号后形成机械三连。目标数量完成后，仅明确点名/native reply 可消费独立有界 continuity capacity，事实不增加 quantity。账号核心身份事实有 provenance，Task 面具不得改写年龄/职业/地区/经历；未绑定平台 Action 的受管账号外发占用统一 Timeline/行为预算且不计任务成功。Fleet 活跃必须分列 passive/reaction/authored/human-linked。点赞不再以 `1_000_000` 关闭任务日保护，而以 `daily_reaction_cap + ReactionCapacityAllocationEpoch` 在动态来源间公平分配，并在 Gateway 前复核 source/intent revision。完整规范以统一引擎 §19.2 为准。

> **2026-09-03 互动执行所有权终审（设计完成 / 未实施）：** response 日计划不再预填未来真人 `planned_call_at`，只冻结 capacity window 与不重叠的 tentative supply。公共 `TurnIntentClassifier` 对每个 canonical turn revision 最多一次分类调用，并在 3/5 秒 cutoff 内预留 Task candidate/claim finalize 尾部；各 Task 只应用自己的 participation policy。唯一 owner 后先冻结 natural window，再扣除冻结 permit 队列、完整准备链 P95、Gateway prepare 与 margin，只在 compatible account/relation slot、Timeline legal interval 的可达交集中原子建立 `InteractionServiceBinding + planned_call + effective reservation + Provider admission`；active binding、Task/source-plan 总预算 CAS 与 Provider reservation 同事务，预测来不及时不建 binding、不拉动未来 slot。每 binding 最多两次 response Provider 调用，数量义务 pre-Gateway 归还后的 successor 继续扣同一总预算，旧调用、unknown 和成本不清零。真人效果每个 human event revision 最多归因一个正向 fact，native reply 优先且不限语义推断窗；owned unresolved unknown 即使滑出比例窗口仍占位。materialization horizon、generation latest-safe、response release cutoff 与 protected slack 全部从冻结 `ExecutionTimingProfileRevision + path-start stage` 派生，缺 profile 时 unified route 不激活。

> **2026-09-03 评论拟人化去指纹补充（设计完成 / 未实施）：** unified 评论 route 退休 plan-time response speech act、固定 20%/60%/20% 字数配比、固定 ordinal 风格轮转和旧 persona 决策。首次 source-plan 事务只冻结 grounding assignment、evidence、允许的 intent/speech-act 集合、稳定 rank 与 style reservation，不创建依赖未来 turn 的 realization assignment；真实 relation/turn 后才形成 `CommentRealizationIntentAssignment`。明确问题不能被 reaction/附和敷衍，纠错/投诉不能被无依据反问或调侃。外部真人评论只投影同 discussion peer、`time_band_v1`、`source_content_cluster_v1` 的长度/问句/标点/emoji/voice-style 分布并冻结 `CommentStyleProfileRevision`；样本不足时每个 source plan/time band 使用稳定宽区间 cold-start simplex。每个账号另冻结不含人物故事的 `AccountCommentVoiceRevision`，保持统计声线但禁止固定口头禅、账号专属模板、虚构职业/地点/消费/经历或从既有 AI 成稿自学习。top-level 在 source intent 与 `planned_call_at` 已冻结后、互动在真实 intent/relation/turn 与 `planned_call_at` 已冻结后才形成具体 style assignment；合法 pre-Gateway 换号必须 append 新 account-binding/voice reservation，stale 只以 successor 重绑，Gateway-started 后不换意图/风格。assigned、accepted、remote-confirmed 三阶段必须回贴同一 active intent/style/voice/binding revision，并以真人基线和预注册盲评验收。

> **2026-09-03 互动准备竞态终审（设计完成 / 未实施）：** late-bound style assignment 首次创建即进入带 `preparation_timing_revision` 的准备边界；同一 revision 内 planned-call/time-band 不可原地移动。真人 attention 在 slot 未准备时可在原窗口合法 reflow；在 proactive/grounded top-level 已 preparing/ready 但 Telegram 尚未 call-issued 时，必须原子 fence 旧 Job/candidate/Action、supersede 旧 style、释放可安全释放的 reservation 并递增 preparation-timing revision，随后重新走 generation/quality/reviewer/dedupe；旧正文和 request identity 不得复用，原窗口不足即 typed shortfall。call-issued 后只记录 interruption observation，不撤销或 replacement。
...
> 2. `post_send_intercepted` 明确失败并关闭当前 hold。存在合法自动恢复路径时账号进入 `recovering` 并保留本任务 coverage；没有合法恢复路径时进入 `abandoned_for_day`，终结该账号尚未进 Gateway 的执行项，但当前任务日 policy-eligible/selected 必达分母保持不变并形成 typed shortfall。任何 abandon 都不得改写或重发 Gateway-started/pending/unknown。
...
保存时必须区分“结构配置冲突”和“外部容量不足”。`1_000_000` 软上限归一只服务未接管的 legacy route 历史兼容，不能进入 unified current 的容量真相。unified 活群/评论/点赞/浏览必须在 create/start 和任务日冻结前共同校验 AccountPool global concurrency、逐账号跨 Task behavior budget、Timeline/Session、peer/source managed-presence、类型 Cap 与现有 call-issued/unknown 占用；结构不可行直接阻断，依赖未来真人/来源时显示 `forecast_conditional`，部分健康时 `running_partial`。其中评论继续由真实 Daily Cap 与 allocation epoch 拥有容量，点赞由必填 `daily_reaction_cap + ReactionCapacityAllocationEpoch` 拥有任务日容量；不得用超大常量绕过。任何容量不足都不降低原业务目标或删除未分配需求。
...
| 触发依据 | 当前账号/目标的权威失败原因；旧连续失败次数仅作观测，不要求继续失败凑阈值 |
...
当前管理员及救活合同以`ai-group-membership-epoch-and-target-recovery-prd.md`§14为准，本段同步产品口径：

- 本Task指定管理员优先，否则使用Tenant默认管理员。Tenant默认专职账号仍从普通业务候选排除；仅Task指定的管理员只在对应Task/target保留，不从其他Task或整个租户覆盖分母移除。Task覆盖沿用受保护修改入口；普通设置的权限与界面不变。
- 真实当前准入/发送权限事实立即进入按原因分类的恢复，不再要求制造超过3次失败才开始救援；连续失败数保留为观测，每Action/Attempt只计一次。Listener读取失败不冒充普通账号发送失败，网络unknown只对账。
- 未入群、被限制、待审批、未过群管分别走正常加入/授权邀请、授权解除、原申请审批、当前验证依赖；已在群但禁言不得以重复邀请代替解除。实际Session须具备该目标对应invite_users/ban_users等权限，明确人工禁止恢复的决定保持。
- 救援开关关闭、管理员不可用、目标错绑、无权限或账号不明时显示具体原因，不创建假成功；不影响无此依赖的健康账号。按已启用救援范围执行常规处置，不新增逐次确认流程。
- 同一当前账号/目标恢复原因最多一个active救援owner。邀请、导出链接、解除限制、普通账号自行加入分别保存实际actor与步骤调用证据，各自经过适用节奏、真实账号互斥和共享限流；救援不消耗普通发言数量，不以救援类型跳过全部容量检查。
- 已入群的邀请幂等成功只表示成员/邀请阶段；必须回到配置频道/当前bot验证/观察/can_send复检，ready后唤醒原Task的合法剩余工作。准入恢复与实际可见发言分列，邀请成功不是最终救活。
- 旧call-issued/unknown继续原identity对账，换管理员、重试或TTL都不构成重放许可；明确未调用步骤才按当前配置/版本安全续接。存量preview/apply/readback与权限范围按专项§13–§14执行。
...
- AI 活群和频道评论的**所有 normal contextual 正文**都必须做无正文远端可见性核验：普通内容使用冻结 policy 的 15 秒窗口，首次通过准入、`admission_version` 变化或近期命中 moderation/bot 风险使用 90 秒窗口。Action 进入 `pending_visibility`，与 `unknown_after_send` 共用 hold 占位 1，禁止对同一义务规划替代发送。`visible_confirmed` 前不得增加数量、发言覆盖或 speaking participation。可信删除/拒绝写 `post_send_intercepted`；Observer 不可用、watermark gap 或 deadline 时远端状态不确定写 `visibility_observation_unknown`，进入 reconcile-only，超时不得当成功或补发。
- 待可见性业务事实统一命名为 `pending_visibility_hold`；现有模型/表 `PendingVisibilityCredit/pending_visibility_credits` 只是兼容物理名。Attempt 即使已有 remote id，也只能证明 Gateway delivery；`PostSendVisibilityObservation` 必须引用冻结 `PostSendVisibilityPolicyRevision` 与 Observer route/watermark。`visible_confirmed` 在同一短事务关闭 hold、完成 Action、确认主义务和 coverage；`post_send_intercepted` 进入 `recovering|abandoned_for_day`，但 abandoned 不缩小当日必达分母；`visibility_observation_unknown` 保留原 identity，三者都不得改写 Gateway-started/pending/unknown。
...
| 模式 | 字段 |
| --- | --- |
| unified 四类互动任务 | `engagement_contract_version=unified_engagement_v1`、`account_selection_mode=group`、`account_group_ids[]`（1..N）、`concurrency_limit_per_group`；唯一正常生产入口；absent 只读映射为 `legacy_v0`，必须经等价性预览和下一完整任务日 successor 才能切换 |
| legacy 全部账号 | `selection_mode=all`；只读收口。迁移时绑定全部 enabled、用途兼容的普通运营分组，且这些分组按同一稳定资格规则得到的 policy-eligible 并集 account-id set/hash 必须精确等于冻结 legacy scope；默认组映射、未分组 eligible 账号或集合不等均阻断自动迁移 |
| legacy 单账号分组 | `selection_mode=group`、`account_group_id`；迁为一个元素的 binding set |
| legacy 手动选择 | `selection_mode=manual`、`account_ids`；仅兼容/诊断，不作为 unified 新任务配置 |
...
所有按自然日履约的任务必须先建立不可变 `task_day_ledger_id`，冻结 `timezone_snapshot/timezone_revision/obligation_local_date/period_start_at/deadline_at/day_phase`；账号 coverage、频道消息日目标、纯搜索 click 义务、Action 和 Attempt 都以该 ID 归属，本地日期只用于展示。四类 unified 互动任务的 timezone 固定为 `Asia/Shanghai`，不接受中途修改；暂停跨过 deadline 时旧 ledger 如实 missed，resume 只恢复仍在原 period 的同一义务，不从 `resume_at` 新建缩量 ledger。非 unified/legacy IANA 任务若仍保留 timezone successor，只能在旧 period 完整结束后建立首尾相接且不重叠的迁移 ledger，并按其冻结 DST 事实只读收口；该兼容路径不得进入 unified 新计划。task-day ledger 切换不得重置账号/关键词安全额度、Telegram 限流、授权锁、代理/内容冷却或 unknown hold。存量 `legacy_mixed_search_join` 的历史 membership/admission 事实维持原 ledger 绑定，但不进入纯点击新合同。
...
- 面具支持账号以独立真人、真实群友或订阅读者人设参与互动，包含目标受众身份感与生活经历设定，全面服务于制造社群互动与频道用户热度；但不得冒充群管理员、官方认证身份或特定自然人，不得提供虚假资质背书。
...
> 本段保留已上线/存量 AI route 的现状合同；其中 `all_accounts_daily`、动态账号范围、签到兜底、Planner 私有批次与提前内容准备不得进入 `unified_engine_route_v1`。unified 新 route 的账号分组、参与计划、JIT、互动所有权、韧性和完成口径以本文顶部 2026-09-03 终审及统一引擎 PRD 为准。
...
频道点赞运行时必须先守卫关注关系、任务日容量与来源内容意图。账号没有关注 / 加入目标频道时，系统生成或复用 `ensure_target_membership`，当前点赞延后到准入后重试；不得在未关注状态下调用 Telegram reaction 接口。频道监听器采集 Telegram Reaction 能力 `unknown/all/some/none + available_reactions`；`reaction_scope=all_available` 只表示 capability candidate pool，不授权从全部 emoji 等概率随机。每个账号—来源先用冻结 `ReactionIntentPolicyRevision` 对 text/caption/approved transcript/typed media metadata 分类，再建立 `SourceReactionIntentDecision`，最终候选固定为 `allowed_reactions ∩ intent_safe_emojis ∩ task_policy_emojis ∩ account_behavior_constraints`。严肃、负向或高风险内容禁止庆祝类 reaction；内容不可解释为 `reaction_intent_unproven`，语义安全交集为空为 `reaction_intent_no_match`，Telegram 能力不支持才是 `reaction_capability_blocked`，三者均不发送、不降级默认表情、不计 completed。Premium-only Reaction 在没有逐账号 Premium 能力事实前不得进入能力池。所有动态来源先进入 `ReactionCapacityAllocationEpoch`，在 `daily_reaction_cap`、账号行为预算与 source density 内按轮次公平分配；未分配目标保留 shortfall。Gateway call-issued 前必须 CAS current source revision/content hash、intent decision 与 capability；来源已编辑/删除时旧 reaction 不得发送或换表情补发。点赞任务的 active/rolling window、稳定选择和 typed fact 仍按既有合同；specific 模式也必须通过 intent hard-deny，不得以运营指定绕过严重负向互斥。能力探测失败不阻塞同频道浏览或评论。
...
- legacy 频道浏览的 `task_daily_view_safety_cap/max_views_per_account_per_day` 与 legacy 频道点赞的 `max_likes_per_account_per_hour` 可继续以 `1_000_000` 只读收口，但该常量不得迁入 unified route。unified 浏览由 day cohort、allocation edge 与账号行为预算限量；unified 点赞由必填 `daily_reaction_cap`、`ReactionCapacityAllocationEpoch`、账号行为预算和 source density 共同限量。dynamic_new 仍形成完整需求事实，但超出容量的部分必须保留 `*_unallocated` shortfall，不能静默丢来源或无限建单。
...
- 频道评论 / 回复创建和配置更新必须支持 `reply_min_per_message` 或等价字段，表示每条频道消息本轮补计划时最少 Telegram 原生引用回复数；unified current 不允许 Unicode/图片 fallback 结算评论，存量 legacy v1.2 fallback 只按原 direct/reply relation 与 identity 收口。
...
生成恢复以已持久化 unknown 作业为权威，收口旧 generating 投影不得重放未知调用。频道浏览/点赞的未绑定冻结义务按原窗口具体预约空隙恢复，不能被远期最大 release 饿死，原 due/目标/已冻结下限保持。克隆在正式 Stop 后可通过预览校验刷新同账号控制授权，再由 Start 建立新实时起点；旧 epoch 事实与未知保留。细则分别见 AI 群供给专项、频道成员前置设计和群克隆专项的 2026-09-11 修复切片。

2026-09-11 频道执行恢复补充：浏览/点赞运行时来源预约按具体合法时间点查找原窗口内空隙，点赞消息 period 使用冻结账号预约截止，避免远期来源尾游标饿死当前动作；保留原间隔、释放下限和 unknown。详见频道/群准入前置设计末节。

2026-09-11 跨日点赞归属修复：按义务创建时、同来源和同账号的冻结参与版本核对资源；随机 due 跨日不能切换到次日参与账号。频道先完成账号资源准入，来源尚未到时则结算本次资源后延后，不提前消耗来源调用间隔。详见频道/群准入前置设计末节。

### 2026-09-11 AI活群健康供给履约修复

本轮按[健康供给履约修复合同](../03-feature-designs/ai-group-healthy-supply-fulfillment-repair-20260911-prd.md)推进。不可用账号不派发不等于活群执行故障；当前可执行供给、消息目标、独立账号覆盖分列，原冻结目标不缩小。修复候选公平性、资源等待、原时间窗口、生成/准入及消息事实投影；远端未知不重放。设计完成不等于实现、发布或Telegram履约完成。

### 2026-09-11 owner 认证连接断开

本地 IPC 认证未完成即断开的连接须显式记录并终结该连接，不得退出整个 owner 或触发业务重放。已提交 unknown 与授权身份保持原合同，详见账号授权 owner 专项 PRD 的同日修复节。整机大量磁盘读取根因另行取证。

### 2026-09-11 素材引用读取性能修复

素材引用统计保留完整、精确、租户隔离的现有口径；Action 引用采用数据库索引筛选和聚合，页面不加载全量历史 Action。详见素材库设计的「素材引用统计查询修复」。本切片不改变素材可用性、任务发送、账号授权或业务目标。
