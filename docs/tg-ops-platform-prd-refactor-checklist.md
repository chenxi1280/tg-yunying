...
本文件是后续代码重构的执行清单。2026-05-23 的目标是先把文档面升级到同一口径，不代表下面代码实施项已经完成；代码改造仍按 P0-P6 分批执行和验收。

| 文档面 | 本轮更新要求 | 完成判定 |
| --- | --- | --- |
| PRD | `docs/tg-ops-platform-prd.md` 的日期、更新记录、实施优先级和验收口径同步到本清单 | PRD 使用 P0-P6：基线口径、汇总读模型、账号可用性、运营方案、运营中心、任务中心、系统设置/手册闭环 |
...
| 区域 | 当前文件 | 当前状态 | PRD 差距 |
| --- | --- | --- | --- |
| 导航壳 | `frontend/src/app/AppShell.tsx`、`frontend/src/app/routes.ts` | 已有 13 个主导航；`overview` 显示为“运营概览”，路由 `/dashboard` | 需要升级文案为“运营中心”，并让首屏从概览卡片变成目标工作台 + 运营方案 |
...
| 系统设置 | `SystemConfigView.tsx` | 已有 TG 开发者应用、AI 供应商、AI 黑话、提示词与素材、账号与权限 Tab | 方向和 PRD 一致；需要补按钮权限矩阵、敏感动作原因和系统设置边界文案 |
...
| 权限 | `permission_middleware.py`、`frontend/src/app/utils.ts` | 已有菜单 / 接口权限；部分权限名为旧口径，如 `accounts.view_codes` | 需要对齐 PRD 权限矩阵，如 `accounts.codes.read`、`operation_plans.manage`、`operation_issues.manage`、`tasks.dispatch_control` |
...
| P1 汇总读模型与运营异常 | 建表、增量汇总、stale 语义、冷热边界 | 后端优先 | P2 / P3 依赖读模型 |
| P2 账号可用性中心 | `account_runtime_summary`、可用性 API、账号列表和详情展示 | 可与 P3 并行 | 依赖 P1 的账号读模型 |
| P3 运营方案中心 | 方案模型、生成预览、生成任务、关联任务调整 | 可与 P2 并行 | 依赖任务创建服务和权限 |
| P4 运营中心重构 | 目标工作台、异常聚合、方案模板、下钻任务失败 | 依赖 P1 / P3 | P1 和 P3 基础完成后做 |
| P5 任务中心收敛 | 列表读摘要、详情分页、attempt 下钻、向导字段简化 | 可与 P4 部分并行 | P1 摘要表和 P2 可用性 |
| P6 手册、权限、验收闭环 | 操作手册、权限矩阵、审计、回归测试、构建 | 最后收敛 | 所有功能阶段完成后 |
...
- [x] 确认 `frontend/src/app/routes.ts` 不新增 `素材库与 AI` 一级路由；AI、提示词、素材、后台账号权限继续归入 `/system-config`。
...
- [x] 空库迁移成功。
- [ ] 老库迁移成功，不删除 `actions`、`execution_attempts`、`runtime_metric_snapshots`。
- [x] 汇总表允许重算，不作为唯一事实源。
...
P3 已落地方案主表、目标绑定、任务关联和生成运行记录；运营中心可按目标快捷创建默认方案、编辑方案、生成预览、生成草稿、生成并启动任务，并在应用到关联任务前展示影响预览、要求填写原因和二次确认。生成预览已返回预计任务、目标、账号容量、准入动作和阻塞原因，并在运营中心用抽屉展示。
...
- [x] `GET /api/operation-issues/{issue_id}`：异常详情、关联任务失败、账号影响、建议动作。
...
- [x] `GET /api/tasks/{task_id}` 返回 `membership_subtask`、`task_runtime_summary`、目标、规则、账号摘要。
...
- [x] 保持 `SystemConfigView.tsx` Tab：TG 开发者应用、AI 供应商、AI 黑话、提示词与素材、后台账号权限、运行配置。
...
2026-05-23 PostgreSQL 验收说明：`backend/tests/conftest.py` 会在测试开始前 `DROP SCHEMA public CASCADE` 并重建 schema，随后应用启动按 `AUTO_MIGRATE_ON_START=true` 自动迁移到当前模型，已覆盖空库迁移和全量后端回归。老库迁移“不删除历史大表”和 1000 账号容量报告未在本轮单独执行。
...
| 风险 | 影响 | 处理 |
| --- | --- | --- |
| 一次性改运营中心、任务中心和数据模型 | 回归面过大 | P1 先建读模型，P4 再改页面 |
