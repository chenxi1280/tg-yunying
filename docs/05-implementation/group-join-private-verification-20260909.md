...
- 发布后直接调用生产已部署的`screening_inventory`（只读）：扫描877条频道消息、2,326,838条群上下文；确定性规则分别命中121条与6,216条，清单fingerprint=`e0b343403f4d8695bc3343182506897d06936b54eb6f437980eda039dd4b906e`。仅输出计数/hash，未删除历史证据；这些是规则命中数，不是人工确认或媒体全覆盖结论。
- 随后独立读取当时正在运行的backend/worker容器，17个全部RELEASE_SHA匹配`8fc01975`且healthy；未发现运行中应用容器版本漂移。此计数是瞬时运行集合，不替代预期角色完整性验收。
