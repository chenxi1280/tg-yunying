...
- 第七次 release `c0691f43` 的 run `29394248123` 成功，11 个 worker healthy 且 JSON `GROUP BY` 未再出现；同一标准 planner drain 的 fresh E4 又暴露 ai-memory 历史候选扫描：按 tenant、group task/action、success/unknown、30 天窗口和 created_at 排序后 anti-join memory，完整 Action ORM 查询运行 29 秒。0099 增加 `(tenant_id, created_at, id)` partial concurrent index，predicate 固定 `group_ai_chat + send_message + success/unknown_after_send`，与已存在的 memory.action_id 索引共同覆盖候选与 anti-join。迁移幂等/可逆/并发 DDL、既有 query shape 与唯一 migration head 共 `26 passed in 1.68s`；待第八次发布重跑 planner 和楼凤远端验收。
- 第八次 release `779e1da1` 的 run `29397882686` 成功，生产切到 `20260715074316_779e1da`，11 个 worker healthy，0099 索引 `valid/ready`、大小 `640 kB`。但对应用精确 SQL 的 `EXPLAIN ANALYZE` 仍在 20 秒超时；非执行计划证明虽命中 0099，查询仍在 anti-join 前读取宽度约 1103 字节的完整 Action，需堆读取约一万条已有记忆的 payload/result 才找到最多 100 条缺失记录。第九层 rework 保持 30 天、NOT EXISTS、created_at 顺序和 100 条上限不变，将单 SQL 拆成候选子查询先只投影 `id/created_at`，外层再加载最终 Action；0100 partial concurrent covering index 在原 key 上 `INCLUDE (executed_at, scheduled_at)`，使时间窗口过滤无需读取宽行堆页。旧查询形状红测先失败，修复后迁移、query shape、action_id 与 merge 组合 `29 passed in 2.36s`，ruff 与 diff check 通过；待第九次发布做生产执行计划、planner drain 和楼凤远端验收。
- 第九次 release `2bb59312` 的 run `29399258289` 完整成功，生产切到 `20260715080842_2bb5931`，Alembic 0100，backend 与 11 个 worker healthy；0099/0100 两个 ai-memory 索引均 `valid/ready`，大小分别 `640 kB/776 kB`。发布后主机仍处于约 `load 24-26`、CPU `0% idle`，容器新进程启动会超过 20 秒，因此不能把服务 healthy 当作查询性能通过。持续会话的 fresh `pg_stat_activity` 显示另一个群准入最新历史 Action 查询运行 14 秒并等待 `BufferMapping`：按 action type、channel target、task 排序每账号最新记录，但缺 tenant 过滤且无匹配索引。第十层 rework 为所有调用补 tenant 隔离，0101 增加 `(tenant_id, channel_target_id, task_id, account_id, created_at DESC, id DESC)` partial concurrent index；跨租户最新记录、SQLite 迁移幂等/回滚、PostgreSQL DDL 和 migration head 共 `7 passed in 2.68s`，ruff/diff check 通过；待第十次发布后重新核对整体负载、慢 SQL、planner 与楼凤远端成功。
- 第十次 release `44c96536` 的 run `29401368052` 完整成功，生产切到 `20260715084349_44c9653`，Alembic 0101，backend 与 11 个 worker healthy；0101 `valid/ready`、大小 `3896 kB`。ai-memory 精确两阶段 SQL 在生产重载下命中 0100 covering `Index Only Scan` 和 0092 action-id `Index Only Scan`，扫描 8283 条已回填候选、返回 0 条缺失，`Execution Time 7780.523ms`，后续活动 SQL 样本无超过 10 秒记录。四群远端确认由第五次发布时 `55/2320` 增长为天津 `21/580`、石家庄 `49/580`、郑州师范 `15/580`、郑州楼凤 `4/580`，合计 `89/2320`；楼凤已从 0 恢复真实远端成功。阿哥日记任务最新 16:10/16:17 北京时间 Action 与 ExecutionAttempt 均 success，remote id `329/330`，评论任务判定 PASS；17:00 至 20:00 仍为未来 pending。完整业务仍 blocked：未确认项主要有天津 `account_offline=180`、石家庄 `325`、楼凤 `79`，另有少量账号不可用/过期 bucket/质量过滤；标准 planner drain 运行约 3 分钟后 SSH 会话被高负载主机断开，未取得完成结果，保持 unproven。运行时查询事故判定 fixed，但 2320 自然日覆盖未完成，整体不得写 `production_fixed`。
