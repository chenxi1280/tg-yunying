...
- `account_profile_duplicate_reconcile.py`：精确重复组、keeper、全目标预检、分批旧值复核、manifest SHA。
...
- 完整 `no_postgres` 分区遵守 60 秒硬超时，约 31% 时未完成；不能写全分区通过。
...
- `postgres_concurrency`: unproven locally, required in CI
- `release`: pending
