...
## 2026-09-06 06:10 全量直接切换实现与发布检查

20dfc23bc66558436f965506cb684ca5774e4e6e的Deploy33992421714终态success，五组后端、前端、三镜像及部署均通过。05:30:55独立确认current=20260905212341_20dfc23b、10个核心后端/worker同SHA且running/healthy、APIok，schema为0225；回执runtime_20df_verify.json保留。本次0226和直接切换候选尚未部署，22个旧Task尚未退役，不能将成员基础发布当作引擎切换。

05:32:19使用正式bootstrap_account_group_revisions只读预览tenant 1全部12组，hash=95c97d2b136e8d6e7a628c957109cc11ab42ddb32d48a2a1ca410ee4820494a2。05:32:50按同SHA/hash、actor=codex、audit_reference=user-direct-cutover-20260906初始化缺失的成对revision1，独立读回true，after hash=33b788fb5aad49a9b4b3717abb876d8d8cf9c498b798349857a57dc7f55bbb5d。原成员、组用途、Task配置及运行状态未改变；membership_20df_full_preview.json、membership_20df_full_receipt.json已复制本地并以0600保存，后续部署不会丢失操作依据。

PRD§19.58按用户“全量放弃旧工作并直接切换”完成设计与反查。实现使用独立Task退役字段和新UUID映射，正式创建全部draft与旧Task退役同事务；先取得成员tenant锁，再锁Task并重验完整配置/成员/授权预览hash。迟到旧对象不能把退役Task改回running；配置、启动、reset/retry入口拒绝旧身份。Planner事务重新锁Task，Gateway真正call-issued前重新读取Task并与退役串行；旧called/unknown/success保持，未调用Action/Job owner/安全预约按原资源服务分批清理。新draft首次启动将binding同步到最终epoch，不宣称旧暂停任务计划继承已闭合。授权复制保留原scope/actor/evidence/attested_at/expiry，仅绑定新Task配置revision1，已撤销或到期授权明确拒绝。

真实PG红测曾复现锁竞争先把Attempt写skipped后旧defer不接受，以及已被发前检查标记skipped的Action仍有未释放预约；均在正式入口修补后通过，未把锁竞争写成unknown。全服务PG验证退役→清理→正式启动→独立读回及原called/unknown/success不变；并发测试覆盖缓存旧Task、双方先后取得行锁、旧身份拒绝恢复和已开始调用保留。迟到生成ready、原未知缺call-start、授权变更、集合/配置/SHA/成员范围漂移、二次创建回滚与重复执行均有定向验证。

QA证据：direct_cutover_release_verified_qa.log为285 passed/53.57秒；直接切换、授权、发前检查及Provider helper最后提取后的direct_cutover_final_structure_qa.log为48 passed/16.02秒；配置、来源选择、旧设置/角色回归为156 passed/2 xfailed/28.36秒（direct_cutover_final_settings_source_qa.log）。两个xfail是原已废弃硬小时上限用例，本次未添加或掩盖失败。0226及现有0196/0218增量升级、bootstrap/迁移图、非空退役证据降级保护15 passed/33.57秒（retirement_migration_verified_qa.log），迁移代码随后无变化。以上集合存在重叠，不合计为唯一测试总数。所有后台测试使用backend/.venv、每次硬60秒，PG仅显式55439/tg_yunying_test及原advisory lock；新模块/测试小于500行，改动生产函数不超过50非空行且复杂度不超过10。既有公共入口位置参数签名保持兼容，既有迁移图测试仅更新head期望，未作无关重构；语法及diff检查通过。

06:02只读生产候选输入验证覆盖全部22个配置及原账号范围，分类容量读取1.60936秒；原单组天津音乐学院保持536人、天津一品楼保持304人，成都排除原救援账号后1631人，其他全组范围稳定成员1632。已知初始来源下：活群需求上界20700、分类可用60384；评论4个逻辑来源合计上界4244、分类可用16320；点赞上界1296、分类可用81600；5个浏览仍new_only、初始来源0。评论每帖按898–1061、浏览未来按1306–1550参与账号形成预览，不将未来帖子或历史量当完成量。此为分类容量观察，不代表总预算/历史物理占用已通过完整规划，也不是生产吞吐或E4。候选源码hash及回执direct_cutover_capacity_candidate_readonly.json/.stderr保留；没有冻结计划或写生产配置。
...
## 2026-09-06 06:52 28bb发布完成与历史原容器退出对账候选

Deploy33995737955在28bbfb355a91a1889c3d38979e3f1bc9b21b3ac1上已终态success，五组后端、前端、三镜像和部署全部通过。06:50:21独立核验current为20260905223248_28bbfb35，20个规定服务全部存在、running/healthy、零重启/OOM、APIok。19个后端角色由RELEASE_SHA核验，图片识别服务按其既有Compose合同使用完整SHA镜像标签及实际image ID核验；它原本不注入RELEASE_SHA，首个只检查环境变量的诊断因此报missing，随后修正诊断并独立验证，不更改或缩小20角色集合。回执runtime_all_roles_28bb_final_verify.json/.stderr保存，stderr为空。

06:39:17生产READ ONLY/REPEATABLE READ确认schema=0226_task_retirement，任务停止CHECK、自引用RESTRICT外键及唯一替代约束均存在。12组基础hash仍33b788fb5aad49a9b4b3717abb876d8d8cf9c498b798349857a57dc7f55bbb5d，当前9活群/2评论/6点赞/5浏览共22个running、retired=0、unified_running=0。发布和基础验证通过不代表已经切换；话题占比上限的用户选择仍未收到，未使用只读样例0.3替代授权。

06:24:39原全范围1632业务成员中，776个有旧物理在途、108个有证据问题，两者并集779，只有853个不受这两项历史问题阻塞；因此评论最低898和浏览最低1306当前不能达到，不减分母/目标掩盖。06:28读取1633个普通组成员（比业务集合多一个救援资格排除项）的诊断得到2774条原Attempt、216个Docker前缀，原worker PID全部为1。跨两台已登记生产宿主的Docker全容器与/proc cgroup扫描均未找到这些前缀、读取错误为0；此缺失观察仅留审计，未写ACK。

宿主journal只读核验找到165个容器的TaskDelete与Docker实际exitStatus，均能对应晚于原调用的退出；随后轮转messages日志补到179个容器，合计2391条原调用。每个12位前缀在已采集原件中只对应一个完整64位ID。其他383条/90个诊断账号没有同等级原容器退出证明，继续保留；不能把它们按同批其他容器退出外推。原件legacy_owner_exit_prod-silicon-root.json、legacy_owner_rotated_log_probe.json和两机inventory受0600保护；不含Telegram正文或Session。已反查Gateway本进程执行路径自2026-05-18目录调整以来未改变，历史与现行Compose没有自定义hostname/host PID/外部Telegram执行服务，原worker_id明确为hostname:pid。

先完成PRD§19.59与反查，再实现原进程退出证据校验、精确preview/CAS/apply/AuditLog/幂等读回及正式CLI。只追加Attempt的transport元数据，原业务unknown、日归属、due、Action、journal及预算保持。首轮真实PG和单元测试暴露北京时间naive与UTC aware直接比较的问题，改用项目现有as_beijing_aware，并增加PG连接UTC时原时刻及hash相同的验证；另明确拒绝未来采集/退出时间，不把日志收集时间当实际退出时间。

QA：worker_exit_second_qa.log为28 passed/23.86秒；worker_exit_legacy_regression_qa.log为78 passed/12.27秒，含原本地runner ACK、历史占用投影与共享准入；最终时间校验及真实PG/共享原日预算为37 passed/22.90秒（worker_exit_final_postgres_qa.log）。集合重叠，不合计唯一测试总数。真实PG覆盖锁竞争零写、等待后原snapshot变化、回滚、同preview重入、跨时区及独立readback；统一三件套已有调用明确拒绝进入旧调用补证入口，空容器观察没有正面事件时拒绝。全部backend/.venv、硬60秒、显式55439/tg_yunying_test与原advisory lock。新文件均小于500行、函数不超50非空行/3位置参数/复杂度10，语法与diff检查通过。
