...
本能力属于账号中心和风控中心之间的账号安全治理与账号资料初始化，不属于任务中心的普通运营任务创建入口。但资料初始化、设置二步密码和清理登录设备一旦进入后台 worker 执行，执行状态必须能被任务中心读取；尤其资料初始化批次必须以系统执行任务形式展示进度、失败原因和头像缓存状态。
...
| 安全处理批次 | 从“设置二步密码”或“清理登录设备”入口选择账号后创建的处理批次。 |
| 安全处理项 | 设置二步密码或清理登录设备批次里针对单个账号的一条处理记录。 |
...
| 设置二步密码 | 只为未设置 2FA 的账号设置 Telegram 二步验证密码 |
| 清理登录设备 | 只退出非平台授权会话，保留平台可信 Session |
...
前端不新增独立一级导航，仍然放在“TG 账号管理”里。这里的边界是账号资产和账号维护：资料初始化、设置二步密码、清理登录设备、同步安全状态都属于账号维护；消息发送、联系人发送、运营方案和风控处置不放在账号管理里。
...
| 列 | 内容 |
| --- | --- |
| 登录状态 | 在线、待登录、等待验证码、等待扫码、等待 2FA、需重新登录 |
...
选择账号
  ├─ 账号组
  ├─ 搜索手机号 / 昵称 / username / 账号 ID
  ├─ 状态、健康分、在线状态、2FA、头像、资料完整度、安全状态、最近批次筛选
  ├─ 当前页勾选 / 跨页累计选择
...
- 禁止在同一个抽屉里混选动作。资料初始化、设置二步密码、清理登录设备必须是三个独立入口。

### 7.3.1 设置二步密码 / 清理登录设备抽屉
...
account_security 服务
  负责设置二步密码批次、清理登录设备批次、资料初始化批次、AI 命名、头像分配
```
...
每个批次只允许来自一个入口的动作集合：设置二步密码批次只包含 `set_two_fa`，清理登录设备批次只包含 `cleanup_devices`，资料初始化批次可以包含 `update_profile`、`update_username`、`update_avatar`。
...
- 投影 `type=account_profile_init`，`target_summary=账号资料初始化 / N 个账号`。
- 普通任务控制接口不接受投影 ID；启动、暂停、停止、重置、删除只适用于真实 `tasks` 表任务。
- 详情响应扩展 `profile_batch`，包含批次状态、账号项列表、头像缓存汇总、头像来源、缓存状态、失败原因和更新后的头像预览。
...
| accounts.security.batch | 创建设置二步密码 / 清理登录设备批次 |
