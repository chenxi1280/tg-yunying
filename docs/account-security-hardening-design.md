...
- 昵称、姓名和 `@username` 默认调用 AI 随机生成，生成结果需要预览、去重、可编辑、可重抽；序号递增只作为手工兜底策略。
...
| AI 随机命名 | 调用平台 AI 能力，按运营画像、语言风格、性别倾向、地区倾向和禁用词生成随机昵称、姓名、简介和 username 候选。 |
...
| 设置平台设备显示名 | 让平台 Session 在 Telegram 登录设备列表里更容易识别 |
...
配置 AI 随机生成策略、头像策略或导入映射表
  ↓
调用 AI 生成随机姓名、昵称、简介和 username 候选
  ↓
...
| 规则 | 示例 | 说明 |
| --- | --- | --- |
| AI 随机生成 | 顾北辰、林知夏、许清和 | 默认方案，减少批量账号资料过度规律 |
...
- AI 生成失败时允许切换到导入名单或模板兜底，不自动改用序号批量灌入。
  ...
  "items": [
    {
      ...
      "last_name": "林",
      "bio": "喜欢分享日常和行业观察",
      "username_candidates": ["linzhixia_28", "zhixia_daily", "lzx_notes"]
    }
  ]
}
```
...
- 一次批量生成后必须本地去重。
- 输出不合格时最多自动重试一次。
- AI 不可用时，批次进入“AI 生成失败”，由前端提示切换导入名单或模板兜底。
  ...
  "items": [
    {
      ...
      "generated_display_name": "林知夏",
      "generated_first_name": "知夏",
      "generated_last_name": "林",
      "generated_bio": "喜欢分享日常和行业观察",
      "username_candidates": ["linzhixia_28", "zhixia_daily", "lzx_notes"],
...
- 邮箱未确认进入待确认状态。
- 批量昵称默认按 AI 随机生成成功。
- AI 生成失败时能提示切换导入名单或模板兜底，不自动静默改成序号。
