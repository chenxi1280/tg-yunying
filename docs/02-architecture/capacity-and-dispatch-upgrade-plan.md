...
- 缺面具、授权代理路线切换，或主 AI 3 轮加不同备用 AI 3 轮均无可用候选时，AI 使用绑定原义务的精确 `签到`；coverage 与 extra-volume 普通数量槽都由 `primary_quantity_slot_id` 进入该收口，`coverage_ledger_id` 只标识覆盖归属，不是兜底资格。未显式选模型的主阶段复用任务/租户健康 Provider 解析，默认 Provider 失效后继续选择健康 Provider，不能硬编码具体 MiniMax 模型而漏掉健康 MiMo；备用阶段与主阶段独立。评论使用单个 Unicode 表情文本。无可用授权传输路线时进入 `waiting_transport`，不得直连、假成功或吞错。
...
搜索 carrier 的两层计数含义不得混用：Reservation `available=reserved-bound-claimed-released` 只表示还能绑定新 assignment 的 unit；ShardAllocation/Window `unclaimed=Σ(reserved-claimed-released)=Σ(available+bound)` 还必须包含已 action-bound、尚未进入 active claim 的 unit。绑定只执行 available→bound；confirm 执行 bound→claimed、unclaimed→active；bound release 执行 bound→released、unclaimed 减 1；claimed release执行 claimed→released、active 减 1。普通 reconciliation 与 release quarantine/reconciler 必须使用相同公式，不能由前者把 bound 从 unclaimed 提前扣除。
...
若某个中央 search fulfillment 份额无法求解或绑定，不在原份额重试。系统把本 epoch 全部未领取 `(dispatch_claim_reservation_id, fulfillment_lane_claim_ordinal)` 组成 release set；同一 outcome finalize 事务为每个 unit 写 `DispatchAllocationExclusion(window,source_reservation_id,source_ordinal,task,lane,shard,resource_snapshot_hash,reason,evidence,release_carrier_type,release_carrier_id)`，按 Reservation 汇总增加 `released_count`、按原 ShardAllocation/Window 汇总扣减 unclaimed，把 `SearchClickAssignmentEpoch` 从 open CAS 为 finalized。release set 非空且 Window 尚可领取时：`ready` 只递增一次 `dispatch_allocation_epoch`、把 Window 置为 `allocation_state=rebuild_required`、递增 `rebuild_input_version` 并保存 pending epoch；Window 已为 `rebuild_required` 时只递增输入版本并加入现有 wave。Window 已结束时只收口释放事实，不生成新 epoch。集合为空时中央状态不变。claimed/active/bound unit、其他有效旧 Reservation 与公平 cursor 不回退，业务 click 欠额不减少。
