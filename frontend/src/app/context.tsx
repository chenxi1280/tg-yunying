...
export function AppProvider({ children }: AppProviderProps) {
  ...
  const value: AppState = {
    ...
    loadVerificationChallengeContext: (task) => runWithLoading(`verification:${task.id}:context`, '读取验证聊天', () => loadVerificationChallengeContext(task)),
    refreshVerificationChallengeContext: (task) => runWithLoading(`verification:${task.id}:context-refresh`, '重新加入并读取验证', () => refreshVerificationChallengeContext(task)),
    ...
    submitVerificationTaskResponse: (task, responseText) => runWithLoading(`verification:${task.id}:submit-response`, '提交验证回复', () => submitVerificationTaskResponse(task, responseText)),
    ...
    saveTenantGroupRescueSettings: (tenantId, payload) => runWithLoading(`tenant:${tenantId}:group-rescue:save`, '保存群聊救援配置', () => saveTenantGroupRescueSettings(tenantId, payload)),
    openAdminUserCreate,
    openAdminUserEdit,
