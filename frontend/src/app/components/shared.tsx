...
export function statusTone(status: string | null | undefined): BadgeTone {
  ...
  if (['等待验证码', '等待扫码', '等待2FA', '排队中', '发送中', '同步中', '只读归档', '冷却中', '等待处理', '待确认', '执行中', '待处理', '运行中', '启动校验中', '准入补齐中', '等待 AI', '等待上下文', '等待冷却 / 下一轮', '等待下一轮', '容量不足', '需人工处理', '收尾中', 'unused', 'pending', 'running', 'executing', 'wrapping_up'].includes(value)) return 'warning';
