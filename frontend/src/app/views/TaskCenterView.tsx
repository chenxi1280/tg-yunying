...
export default function TaskCenterView({
  ...
  const simpleSearchClickTask = isSimpleSearchClickTask(taskType);
  const wizardSteps = wizardStepsForTask(taskType);
  ...
  async function ensureTaskFormData(type: TaskCenterTaskType, requestSeq: number): Promise<boolean> {
    ...
    try {
      const requests = taskFormSupportRequests(type);
  ...
  function editValuesFromTask(task: TaskCenterTask): Record<string, any> {
    ...
    return {
      ...
      source_capacity_v2_enabled: Boolean(pacing.source_capacity_v2_enabled),
      source_capacity_policy_version_id: pacing.source_capacity_policy_version_id ?? '',
  ...
  function pacingConfig(values: any, type: TaskCenterTaskType = taskType) {
    ...
    const config: Record<string, any> = {
      ...
      source_capacity_v2_enabled: systemFulfillmentGate && Boolean(values.source_capacity_v2_enabled),
      source_capacity_policy_version_id: systemFulfillmentGate && values.source_capacity_v2_enabled
        ? values.source_capacity_policy_version_id?.trim() ?? ''
        : '',
    };
  ...
  function channelCommentPayload(values: any, base: Record<string, any>, includeScope: boolean) {
    const payload: Record<string, any> = {
      ...
      ai_content_route_v2_enabled: Boolean(values.ai_content_route_v2_enabled),
  ...
  const recordColumns: ColumnsType<TaskCenterAction> = [
    ...
    { title: '引用回复', key: 'reply_target', width: 260, render: (_, action) => actionReplyTarget(action) },
    { title: '内容', key: 'content', ellipsis: true, render: (_, action) => actionContent(action) },
    { title: '账号/目标原因', key: 'failure_diagnosis_summary', width: 260, ellipsis: true, render: (_, action) => action.status === 'success' ? '-' : failureDiagnosis(action)?.operator_summary || action.failure_reason || action.result?.error_message || action.result?.detail || '-' },
        ...
        <Form form={form} layout="vertical" preserve initialValues={taskTypeInitialValues(taskType)}>
          ...
          <Space className="modal-actions">
            ...
            {wizardStep < wizardSteps.length - 1 ? (
              ...
              taskType === 'search_rank_deboost' ? (
                <Button type="primary" onClick={() => createTask({ start: false })}>创建草稿</Button>
              ) : simpleSearchClickTask ? (
