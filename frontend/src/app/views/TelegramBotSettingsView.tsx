...
function TenantBotCard({ tenant, settings, canManage, isActionPending, onSave, onTest }: {
  tenant: Tenant;
  ...
  return (
    <Card className="developer-card status-accent neutral" size="small" title={tenant.name} extra={<Badge tone={configured ? 'positive' : 'neutral'}>{configured ? '已配置' : '未配置'}</Badge>}>
      <Space direction="vertical" size={10} style={{ width: '100%' }}>
        <Alert type="info" showIcon message="Bot Token 保存后不会明文回显；AI 活群 Bot 设置开关只控制 bot 内修改任务配置，不影响 Web 任务设置。" />
        <div className="policy-grid">
          <label>Admin Chat ID<Input disabled={!canManage} value={adminChatId} onChange={(event) => setAdminChatId(event.target.value)} placeholder="Telegram 管理员 chat id" /></label>
        ...
        <Space wrap>
          <Tag color={webhookStatus === 'active' ? 'green' : 'default'}>Webhook：{webhookStatus}</Tag>
          <Typography.Text type="secondary">{settings?.telegram_bot_webhook_url || '保存后生成 webhook 地址'}</Typography.Text>
        </Space>
...
export default function TelegramBotSettingsView({ tenants, botSettings, canManageBotSettings, onSaveTenantBotSettings, onTestTenantBotMessage, isActionPending }: Props) {
  return (
