...
const moduleGuides = [
  {
    ...
    steps: ['新增账号并选择登录方式', '按页面提示提交验证码、扫码或 2FA', '同步群、频道和联系人', '在账号详情的账号安全页刷新设备和 2FA 状态', '需要批量处理时勾选账号后执行安全加固或资料初始化', '检查账号状态、健康分和可操作范围'],
  ...
  },
  {
    ...
    steps: ['选择任务类型和运营目标', '使用默认节奏创建，必要时展开高级设置', '创建并启动，或先保存草稿再启动', '频道任务会先让未关注账号按抖动节奏加入频道', '全部账号都未关注时，系统会先安排全部可用账号关注频道', '部分账号已关注时，系统只补齐未关注账号', '在任务详情查看前置关注、执行项、账号分配和失败原因'],
...
const recentFeatureGuides = [
  ...
  },
  {
    title: '批量资料初始化',
    tag: 'TG账号管理',
    detail: '在账号列表勾选多个账号后，可以批量生成或编辑昵称、简介、头像等资料预览，确认后按批次执行初始化。',
  },
  {
    title: '频道目标手动添加',
    tag: '运营目标',
  ...
  },
  {
    title: '频道任务前置关注',
    tag: '任务中心',
    detail: '频道浏览、点赞、评论启动前会先检查账号是否关注频道；未关注账号按抖动节奏先关注，关注成功后才进入主互动阶段。',
  },
];
...
const taskTypes = [
  ...
  ['频道浏览', '为频道消息安排查看动作；未关注账号会先进入关注频道前置阶段。'],
  ['频道点赞', '为频道消息安排点赞动作；全部或部分账号未关注时，系统先补关注再执行点赞。'],
  ...
  },
  {
    ...
    children: (
      <>
        ...
        频道任务启动后会先生成关注频道前置动作；如果没有账号已关注，会让全部可用账号按抖动节奏加入；如果只有部分账号已关注，只补齐未关注账号；全部加入失败时不会进入浏览、点赞或评论主阶段。
      </>
    ),
  },
];
