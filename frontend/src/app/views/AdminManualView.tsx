...
const quickStartSteps = [
  ...
  {
    title: '处理目标和账号能力',
    description: '从运营中心跳转到运营目标、TG 账号管理、规则中心或风控中心，补齐授权、账号可用性、规则版本和风险处置。',
  },
  {
    title: '发起并复盘执行',
...
const moduleGuides = [
  {
    ...
    steps: ['先看目标工作台中的 open issue、失败任务、影响账号和最近失败', '点开目标查看关联任务失败、代表执行项和建议动作', '按建议跳转到任务详情、TG 账号管理、运营目标、规则中心或风控中心处理', '在下半部分查看运营方案模板、关联任务和生成入口', '处理后回到运营中心刷新汇总状态'],
    checks: ['目标异常能定位到关联任务或账号', '方案调整前有影响预览和确认', '汇总数据展示最近更新时间或 stale 提示', '系统设置只维护底座配置，不处理运营方案'],
  },
  {
    key: 'accounts',
    icon: <Smartphone size={18} />,
    title: 'TG账号管理',
    scenario: '账号接入、登录恢复、资料同步、安全加固、资料初始化、可用性与容量准备。',
    steps: ['新增账号并选择登录方式', '按页面提示提交验证码、扫码或 2FA', '同步群、频道和联系人', '在账号详情的账号安全页刷新设备和 2FA 状态', '查看账号可发送、可监听、可加入、可评论、可修改资料和可读取验证码能力', '需要批量处理时勾选账号后分别执行资料初始化、设置二步密码或清理登录设备', '检查账号状态、健康分、剩余容量和不可用原因'],
    checks: ['账号状态为在线或可恢复', '开发者应用可用', '代理与验证码任务无未处理异常', '查看或同步验证码已填写原因', '外部设备、2FA 和资料初始化结果已在账号安全记录中确认', '不可用原因和下次可重试时间可解释'],
  },
  {
    key: 'targets',
    icon: <Users size={18} />,
...
const recentFeatureGuides = [
  ...
  },
  {
    title: '运营中心日常入口',
    tag: '运营中心',
    detail: '登录后先看运营中心目标工作台，默认按目标聚合 open issue 和失败任务；点开后查看关联任务失败、代表执行项和建议动作。',
  },
  {
    title: '运营方案模板',
    tag: '运营中心',
  ...
  },
  {
    title: '账号可用性',
    tag: 'TG账号管理',
    detail: '账号中心展示可发送、可监听、可加入、可评论、可修改资料、可读取验证码、剩余容量、不可用原因和下次可重试时间。',
  },
  {
    title: '数据汇总与延迟',
    tag: '运营数据',
  ...
  },
  {
    title: '导航升级',
    tag: '系统设置',
    detail: '菜单统一使用“运营中心”；AI、提示词、素材和后台账号权限位于系统设置 Tab，不作为独立一级菜单。',
  },
];
...
const exceptionPlaybook = [
  {
    ...
    children: (
      <>
        ...
        先按目标展开关联任务失败和代表执行项，再按建议跳转账号、目标、规则、风控或任务详情；处理后回到运营中心刷新汇总状态。
      </>
    ),
  },
  {
