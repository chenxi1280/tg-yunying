...
const moduleGuides = [
  {
    ...
    title: 'TG账号管理',
    scenario: '账号接入、登录恢复、资料同步、分组与容量准备。',
    steps: ['新增账号并选择登录方式', '按页面提示提交验证码、扫码或 2FA', '同步群、频道和联系人', '检查账号状态、健康分和可操作范围'],
    checks: ['账号状态为在线或可恢复', '开发者应用可用', '代理与验证码任务无未处理异常'],
  },
  {
    key: 'targets',
    icon: <Users size={18} />,
    ...
    steps: ['按目标类型筛选群、频道或私聊', '确认目标授权、关联账号和最近同步时间', '从目标详情进入消息发送或任务创建', '目标异常时回到账号或风控中心处理'],
    checks: ['目标已确认', '关联账号具备查看、发言或评论能力', '目标没有被风控拦截'],
  },
  {
    key: 'message',
  ...
  },
  {
    ...
    steps: ['选择任务类型和运营目标', '使用默认节奏创建，必要时展开高级设置', '创建并启动，或先保存草稿再启动', '在任务详情查看执行项、账号分配和失败原因'],
    checks: ['预检查通过', '账号池容量满足任务规模', '暂停、继续、停止、重试都有审计记录'],
  },
  {
    key: 'listeners',
  ...
  },
  {
    ...
    icon: <ShieldAlert size={18} />,
    title: '风控中心',
    scenario: '处理账号风险、目标能力、限流、冷却、代理异常和处置队列。',
    steps: ['先看风险总览和账号评分', '处理待处置队列中的异常', '需要时调整策略并填写原因', '回到任务中心重试受影响任务'],
    checks: ['账号不过度集中使用', '目标慢速模式和禁止发言状态已识别', '代理异常不会继续放大任务失败'],
  },
  {
    key: 'archive',
...
const taskTypes = [
  ...
  ['频道浏览', '为频道消息安排查看动作，适合新消息持续触达。'],
  ['频道点赞', '为频道消息安排点赞动作，需确认账号容量和目标权限。'],
  ['频道评论/回复', '在频道讨论区执行评论或回复，需关注讨论组权限。'],
];
        ...
        <Card className="panel manual-section" title="上线前检查">
          <div className="manual-checklist">
            {[
              ...
              'TG 账号已登录并同步资产，账号健康分和代理状态正常。',
              '运营目标已确认，群、频道或讨论组具备所需权限。',
