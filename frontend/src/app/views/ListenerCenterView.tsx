...
export default function ListenerCenterView({ canManageListeners = false, onOpenTargetProfile }: { canManageListeners?: boolean; onOpenTargetProfile: () => void }) {
    ...
    <Space direction="vertical" size={12} style={{ width: '100%' }}>
      <Space direction="vertical" size={6} style={{ width: '100%' }}>
        ...
        <Space wrap>
          ...
          <Button size="small" onClick={onOpenTargetProfile}>打开目标画像</Button>
          {canManageListeners && <Button size="small" danger onClick={() => onResetWatermark(row)}>重置水位</Button>}
        </Space>
      </Space>
      <Space direction="vertical" size={6} style={{ width: '100%' }}>
        <Typography.Text strong>全站目标画像</Typography.Text>
        <Typography.Text type="secondary">监听对象只作为学习来源候选；画像摘要、样本过滤、版本恢复统一在目标画像页面配置。</Typography.Text>
      </Space>
      <Space direction="vertical" size={6} style={{ width: '100%' }}>
