...
@pytest.mark.no_postgres
def test_search_join_group_safe_navigation_executor_only_allows_pre_join_navigation() -> None:
    """executor 只允许目标确认前的 navigate_only 行为。"""
    config = {
        "pre_join_decoy_click_max": 2,
        "post_join_safe_navigation_max": 0,
        "decoy_join_enabled": False,
    ...
    assert safe_nav["total_max"] == 2
    assert safe_nav["pre_join_decoy_click_max"] == 2
    assert safe_nav["post_join_safe_navigation_max"] == 0
    assert safe_nav["decoy_join_enabled"] is False


@pytest.mark.no_postgres
def test_search_join_group_schema_rejects_all_post_join_navigation() -> None:
    """本期不支持目标确认后的安全浏览；仅允许入群前最多三次导航。"""
    for post_max in (1, 2, 3):
        with pytest.raises(ValidationError, match="post_join_safe_navigation 本期不支持"):
            _safe_nav_payload(post_join_safe_navigation_max=post_max)

    boundary_payload = _safe_nav_payload(pre_join_decoy_click_max=3, post_join_safe_navigation_max=0)
    assert boundary_payload.pre_join_decoy_click_max == 3
    assert boundary_payload.post_join_safe_navigation_max == 0
