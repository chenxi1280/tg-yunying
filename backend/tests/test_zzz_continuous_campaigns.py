...
from app.models import AiDraft, AiUsageLedger, AppUser, Campaign, ContentKeywordRule, GroupContextMessage, MessageTask, TaskStatus, TgGroup
...
def test_mirror_forward_multi_source_deduplicates_messages(monkeypatch):
    with TestClient(app) as client:
        ...
        assert first == len(source_groups)
        assert second == 0
        assert task_count == len(source_groups)
