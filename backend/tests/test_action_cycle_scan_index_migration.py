...
MIGRATION_PATH = Path(__file__).resolve().parents[2] / "backend/migrations/versions/0097_action_cycle_scan_index.py"


def test_cycle_scan_index_migration_is_idempotent_and_reversible_on_sqlite() -> None:
...
def test_cycle_scan_index_migration_uses_concurrent_postgres_ddl(monkeypatch) -> None:
    migration = _migration_module()
...
def _migration_module():
    spec = importlib.util.spec_from_file_location("action_cycle_scan_index_0097", MIGRATION_PATH)
