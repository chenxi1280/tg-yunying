...
def test_prd_permission_vocabulary_for_ai_prompt_and_proxy_controls():
    ...
    assert required_permission("GET", "/api/ai-account-voice-profile-generation-jobs") == ("account_masks.view",)
    assert required_permission("GET", "/api/ai-account-voice-profile-generation-jobs/job-1") == ("account_masks.view",)
    assert required_permission("POST", "/api/ai-account-voice-profile-generation-jobs") == ("ai_voice_profiles.manage",)
    assert required_permission("POST", "/api/ai-account-voice-profile-generation-items/item-1/retry") == ("ai_voice_profiles.manage",)
...
def test_sensitive_read_routes_have_explicit_least_privilege_rules():
    ...
    assert required_permission("GET", "/api/account-environment-bindings/proxy-airport-nodes") == ("account_masks.view",)
    assert required_permission("POST", "/api/account-environment-bindings/refresh-observations") == ("account_environment.manage",)
    assert required_permission("POST", "/api/account-environment-bindings/batch-proxy-bind") == ("account_environment.manage",)
