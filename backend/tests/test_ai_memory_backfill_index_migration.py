...
MIGRATION_PATH = Path(__file__).resolve().parents[2] / "backend/migrations/versions/0099_ai_memory_backfill_index.py"


def test_ai_memory_backfill_index_is_idempotent_and_reversible_on_sqlite() -> None:
...
def test_ai_memory_backfill_index_uses_concurrent_partial_postgres_ddl(monkeypatch) -> None:
    migration = _migration_module()
    ...
    assert "task_type = 'group_ai_chat'" in migration.POSTGRES_CREATE
    assert "status IN ('success', 'unknown_after_send')" in migration.POSTGRES_CREATE


def _migration_module():
    spec = importlib.util.spec_from_file_location("ai_memory_backfill_index_0099", MIGRATION_PATH)
