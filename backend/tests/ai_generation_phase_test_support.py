...
def barrier_generator(barrier: Barrier, label: str):
...
def profile_generator(session: Session, observed: dict[str, int]):
...
def account_content_generator(session: Session, observed: dict[str, int], *, rejected_content: str):
    def generate(_session, _tenant_id, config, *, count, **_kwargs):
        assert session.in_transaction() is False
        observed["provider_calls"] += 1
        slots = list(config["generation_slots"])
...
def profile_sender(session: Session, observed: dict[str, int]):
...
def seed_reserved_reply_action(session: Session, now_value):
...
def invalid_reply_dependencies(session: Session, invalidation: str) -> GenerationDependencies:
...
def forbidden_normal_generation(*_args, **_kwargs):
...
def reply_generator(observed: dict[str, object]):
...
def reply_sender(session: Session, observed: dict[str, object]):
...
def reply_probe(session: Session):
