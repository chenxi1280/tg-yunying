...
def test_content_surface_serializes_two_writers_already_holding_group_foreign_keys(database):
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        futures = [executor.submit(_group_planner, database, account_id, barrier) for account_id in (11, 12)]
        assert sorted(future.result(timeout=WAIT_SECONDS) for future in futures) == [11, 12]
    with Session(database) as session:
