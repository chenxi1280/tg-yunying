...
def test_remote_reconcile_apply_requires_preview_fingerprint_and_approval() -> None:
    ...
    assert "membership-probe" in source
    assert "accept_fresh_conflict_evidence" in source
    assert "--accept-fresh-conflict-evidence" in source
    assert "--expected-conflict-evidence-hash" in source
