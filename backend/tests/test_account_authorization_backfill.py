...
from app.services import account_authorization_backfill
...
def test_backfills_missing_standby_hash_and_api_snapshot(monkeypatch) -> None:
    with _session() as session:
        ...
        session.add(asset)
        session.commit()
        monkeypatch.setattr(account_authorization_backfill, "credentials_for_developer_app", lambda *_args: SimpleNamespace())
        monkeypatch.setattr(
            account_authorization_backfill.gateway,
            "list_authorizations",
