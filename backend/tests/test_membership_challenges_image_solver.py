...
def test_search_join_image_solver_uses_only_first_healthy_provider(monkeypatch) -> None:
    providers = [
    ...
    def solve(provider, *_args, **kwargs):
        ...
        if provider.id != 1:
            raise AssertionError("must not wait for another provider")
        return SimpleNamespace(answer="10", confidence=0.92)
...
@pytest.mark.no_postgres
def test_search_join_image_solver_does_not_request_a_second_model_answer(
    monkeypatch,
) -> None:
    provider = SimpleNamespace(
        id=1,
    ...
    assert solver(_request(["7", "8"])) == ("7", 0.95)
    assert next(answers) == "8"
...
@pytest.mark.no_postgres
def test_search_join_image_solver_exposes_only_selected_provider_unsafe_outcome(
    monkeypatch,
) -> None:
    providers = [
        SimpleNamespace(id=1, provider_name="MiMo", model_name="mimo-v2.5"),
    ...
    assert "MiniMax(MiniMax-M3)" not in str(raised.value)
