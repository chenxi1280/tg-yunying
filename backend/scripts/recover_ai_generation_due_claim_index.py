...
def _apply_requested() -> bool:
    value = os.getenv("AI_GENERATION_INDEX_RECOVERY_APPLY", "false").lower()
    if value not in {"true", "false"}:
        raise ValueError("AI_GENERATION_INDEX_RECOVERY_APPLY must be true or false")
    return value == "true"
...
def _require_apply_guard(snapshot: dict, expected_hash: str) -> None:
    if _state_hash(snapshot) != expected_hash:
        raise RuntimeError("AI generation index recovery state hash changed")
    ...
    if snapshot["index_exists"] and not snapshot["index_valid"]:
        raise RuntimeError("invalid AI generation due-claim index already exists")


def main() -> None:
    apply = _apply_requested()
    expected_hash = os.getenv(
        "AI_GENERATION_INDEX_RECOVERY_EXPECTED_STATE_HASH", "",
    ).strip()
    ...
    if apply and not after["index_valid"]:
        raise RuntimeError("AI generation due-claim index is not valid after apply")
    print("AI_GENERATION_INDEX_RECOVERY=" + json.dumps({
