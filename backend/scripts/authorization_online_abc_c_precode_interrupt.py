...
def main() -> int:
    ...
    print("AUTHORIZATION_ONLINE_ABC_C_PRECODE_INTERRUPT=" + json.dumps(result, sort_keys=True))
    return 0


def _execute(session, args) -> dict:
    if args.mode == "readback":
        return readback_c_precode_interrupt(
    ...
    if args.mode == "preview":
        return preview_c_precode_interrupt(session, args.batch_id, args.account_id, **common)
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Close a control-plane-interrupted pre-code C as manual debt")
