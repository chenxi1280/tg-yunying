...
def upgrade() -> None:
    if not _has_table("target_membership_challenge_attempts"):
        op.create_table(
...
def downgrade() -> None:
    if _has_table("target_membership_challenge_attempts"):
        op.drop_index("ix_membership_challenge_attempt_identity", table_name="target_membership_challenge_attempts", if_exists=True)
