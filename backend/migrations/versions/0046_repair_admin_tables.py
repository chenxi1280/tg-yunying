"""repair admin user tables for migrated legacy databases

Revision ID: 0046_repair_admin_tables
Revises: 0045_operation_issue_context
...
revision = "0046_repair_admin_tables"
down_revision = "0045_operation_issue_context"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    if not _table_exists("app_users"):
        ...
        _add_column_if_missing("app_users", sa.Column("role_template", sa.String(40), nullable=False, server_default="运营管理员"))
        _add_column_if_missing("app_users", sa.Column("permission_version", sa.Integer(), nullable=False, server_default="1"))
        _add_column_if_missing("app_users", sa.Column("last_login_at", sa.DateTime(), nullable=True))
        _add_column_if_missing("app_users", sa.Column("is_active", sa.Boolean(), nullable=False, server_default=sa.true()))

    indexes = _index_names("app_users")
    if "ix_app_users_email" not in indexes:
        op.create_index("ix_app_users_email", "app_users", ["email"], unique=True, if_not_exists=True)
    if "ix_app_users_phone" not in indexes:
    ...
    if "ix_user_token_ledgers_user_created" not in _index_names("user_token_ledgers"):
        op.create_index("ix_user_token_ledgers_user_created", "user_token_ledgers", ["user_id", "created_at"], if_not_exists=True)


def downgrade() -> None:
    # Repair migration: these tables may predate this revision on migrated
    # databases, so downgrade must not remove authentication data.
    return
