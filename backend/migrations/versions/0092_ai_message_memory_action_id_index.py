"""index AI message-memory action lookups

Revision ID: 0092_ai_memory_action_idx
Revises: 0091_ai_memory_index
...
revision = "0092_ai_memory_action_idx"
...
def upgrade() -> None:
    ...
    if op.get_bind().dialect.name == "postgresql":
        with op.get_context().autocommit_block():
            op.execute(f"CREATE INDEX CONCURRENTLY {INDEX_NAME} ON {TABLE_NAME} (action_id)")
        return
    op.create_index(INDEX_NAME, TABLE_NAME, ["action_id"], unique=False)
