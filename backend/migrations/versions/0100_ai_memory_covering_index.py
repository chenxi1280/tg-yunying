"""cover AI message memory candidate timestamps

Revision ID: 0100_ai_memory_covering_index
Revises: 0099_ai_memory_backfill_index
...
revision = "0100_ai_memory_covering_index"
down_revision = "0099_ai_memory_backfill_index"
branch_labels = None
depends_on = None

INDEX_NAME = "ix_actions_ai_memory_backfill_covering"
INDEX_PREDICATE = (
...
POSTGRES_CREATE = (
    ...
    f"INCLUDE (executed_at, scheduled_at) WHERE {INDEX_PREDICATE}"
)
SQLITE_CREATE = (
    f"CREATE INDEX IF NOT EXISTS {INDEX_NAME} "
    f"ON actions (tenant_id, created_at, id, executed_at, scheduled_at) WHERE {INDEX_PREDICATE}"
)
