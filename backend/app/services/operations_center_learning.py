...
from app.services.tenant_learning_samples import GROUP_CHAT_SCENE
from app.services.tenant_target_profile import get_target_profile_overview, list_samples


def listener_learning_profile(session: Session, tenant_id: int, object_type: str, object_id: int) -> dict:
    _ensure_learning_object_exists(session, tenant_id, object_type, object_id)
...
def listener_learning_samples(session: Session, tenant_id: int, object_type: str, object_id: int, filters: dict | None = None) -> dict:
    _ensure_learning_object_exists(session, tenant_id, object_type, object_id)
    return list_samples(session, tenant_id, filters)
...
def _refresh_group_learning(session: Session, tenant_id: int, group_id: int, actor: str) -> dict:
    ...
    session.commit()
    return {"inserted": inserted, "learning_profile": get_target_profile_overview(session, tenant_id)}
...
def _ensure_learning_object_exists(session: Session, tenant_id: int, object_type: str, object_id: int) -> None:
    if object_type == "channel":
