...
from .channel_membership_attempts import membership_reprobe_snapshot
...
def _mark_stale_executing_action(
    *,
    ...
    if latest_attempt:
        ...
        latest_attempt.result_snapshot = (
            membership_reprobe_snapshot(latest_attempt, action.result)
            if action.action_type in MEMBERSHIP_ACTION_TYPES else dict(action.result or {})
        )
    return {
        "previous_lease_owner": previous_lease_owner,
        "recovery_reason": recovery_reason,
...
def _mark_unknown_membership_reprobe_timeout(
    *,
    ...
    if latest_attempt:
        ...
        latest_attempt.after_call_at = now
        latest_attempt.result_snapshot = membership_reprobe_snapshot(latest_attempt, action.result)
...
def _clear_deleted_action_admission_references(
    ...
    action_fields = (
        ...
        TaskMembershipAdmissionItem.delete_action_id,
        TaskMembershipAdmissionItem.rescue_action_id,
    )
    updated_at = _now()
    for field in action_fields:
        session.execute(
            update(TaskMembershipAdmissionItem)
            .where(field.in_(action_ids))
            .values({field.key: None, "updated_at": updated_at})
        )
