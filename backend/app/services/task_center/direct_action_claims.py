...
from .direct_action_candidates import _candidate_rows
from .direct_action_candidates import _ranked_candidate_query as _ranked_candidate_query
...
def claim_fact_first_candidates(
    ...
    *,
    ...
    token = str(uuid4())
    options = dict(limit=limit, now=now, exclude_task_ids=exclude_task_ids,
        execution_lane=execution_lane, shard_total=shard_total, shard_index=shard_index)
    expired = _candidate_rows(session, phase="expired", **options)
    unexpected = _claim_rows(session, expired, owner=owner, token=token,
        now=now, lease_seconds=lease_seconds)
    if unexpected:
        raise RuntimeError("expired_candidate_became_executable")
    candidates = _candidate_rows(session, phase="live", **options)
    claimed = _claim_rows(
        session,
        candidates,
        owner=owner,
