...
def guard_attempt_call_start(session: Session, attempt: ExecutionAttempt) -> None:
    ...
    if _gateway_task_is_current(current, attempt, action):
        return
    ...
    try:
        with session.begin_nested():
            return session.execute(select(Task.status, Task.retired_at, Task.deleted_at,
                Task.task_lifecycle_epoch, Task.type, Task.tenant_id, Task.stats, Task.type_config).where(Task.id == action.task_id,
...
def _gateway_task_is_current(current, attempt, action):
    return (current is not None and task_allows_action_execution(current, action) and current.retired_at is None
