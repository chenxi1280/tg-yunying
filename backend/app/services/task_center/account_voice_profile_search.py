...
def _search_values(row: dict[str, Any]) -> list[str]:
    ...
    return [
        ...
        str(row.get("mask_name") or "").lower(),
        str(row.get("audience_archetype") or "").lower(),
        str(row.get("identity_frame") or "").lower(),
        " ".join(str(item).lower() for item in row.get("preference_tags") or []),
