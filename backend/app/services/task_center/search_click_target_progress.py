...
class SearchClickTargetProgress:
    ...
    def completed(self) -> bool:
        return self.target_count is not None and self.confirmed_count >= self.target_count
...
def search_click_target_progress(session: Session, task: Task) -> SearchClickTargetProgress:
...
def reconcile_search_click_target_progress(session: Session, task: Task) -> SearchClickTargetProgress:
...
def _action_count(session: Session, task: Task, action_type: str, statuses: tuple[str, ...]) -> int:
...
def _confirmed_action_count(session: Session, task: Task, action_type: str) -> int:
