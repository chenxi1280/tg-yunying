...
from .semantic_review_contract import (
    SEMANTIC_REVIEW_PROMPT_VERSION, TwoStageRealizeError, _parse_semantic_review,
)
...
def _run_semantic_review(
    ...
    *,
    ...
    try:
        ...
        raise TwoStageRealizeError(exc.code, evidence=exc.evidence,
            tokens=int(tokens or 0)) from exc
    evidence["candidate_hash"] = hashlib.sha256(content.encode("utf-8")).hexdigest()
    return _accepted_semantic_review(evidence, tokens)


def _accepted_semantic_review(evidence: dict, tokens: int) -> tuple[dict, int]:
