...
class AuthorizationDrRuntimeContract(Base):
    ...
    required_node_capability_version: Mapped[str] = mapped_column(String(80), default="")
    required_node_runtime_image_sha: Mapped[str] = mapped_column(String(64), default="")
    claim_scope_operation_id: Mapped[str] = mapped_column(String(36), default="")
...
class TgAuthorizationDrOperation(Base):
    ...
    code_source_authorization_id: Mapped[int | None] = mapped_column(
    ...
    expected_authorization_generation: Mapped[int] = mapped_column(Integer, default=0)
    expected_authorization_fact_generation: Mapped[int] = mapped_column(Integer, default=0)
    expected_connection_generation: Mapped[int] = mapped_column(Integer, default=0)
    expected_code_source_fact_version: Mapped[int] = mapped_column(Integer, default=0)
    expected_code_source_user_id_digest: Mapped[str] = mapped_column(String(64), default="")
    expected_code_source_auth_key_digest: Mapped[str] = mapped_column(String(64), default="")
    developer_app_id: Mapped[int] = mapped_column(ForeignKey("telegram_developer_apps.id"))
    ...
    login_flow_id: Mapped[int | None] = mapped_column(Integer, nullable=True)
    login_challenge_sent_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    login_code_message_id: Mapped[str] = mapped_column(String(64), default="")
    login_code_received_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
